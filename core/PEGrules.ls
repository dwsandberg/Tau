...
/br If wn rn contains /1 then any word w in str not eq to one of w1.. w (n-i) is replace with wn rn with the /1 replaced by w. }
...
for isT = e ∉ "any Any" ∨ e ∈ "!", r ∈ g
while isT
do e ≠ leftside.r,
not.isT

Function NTcount(p:pegpart, gin:seq.pegrule) int
for count = 0, e ∈ part.p
do if isNonTerminal(e, gin) then count + 1 else count,
count
