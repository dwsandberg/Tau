...
emptyUTF8
 ...
 do
  let chars = decodeword.this,
   if n.chars = 1 then
    let ch = 1#chars,
     if ch ∈ (decodeword.merge."+-.:^#" + char.10 + char.32) then
     {no Space before or after} next(prefix, stk, cmd, result + chars, false, w)
     else if ch ∈ decodeword.merge.",]})^(dq)" then
     {no Space before but Space after} next(prefix, stk, cmd, result + chars, true, w)
     else if ch ∈ decodeword.merge."([{" then
      {Space before but no Space after}
      next(prefix, stk, cmd, if not.Space then result + chars else result + char.32 + chars, false, w)
    ...
    next(prefix, stk, cmd, paragraph.result, false, w)
     ...
     if kind.top.stk ∈ "block" then
      ...
      let z =
       ...
       UTF8(toseqbyte.result >> endbreak.toseqbyte.result) + prefix >> 1
       else block(R0, R1),
     ...
     if w ∈ "table block" then
      let new =
       if w ∈ "block" ∧ newway then
       if not.haslinebreak.toseqbyte.result then result + prefix + char.32 else result
       else result,
      next(
   ...
   next(prefix, stk, not.cmd, result, Space, w)
   else if not.cmd then
    let chars2 = if not.Space then result + chars else result + char.32 + chars,
    next(prefix, stk, cmd, chars2, true, w)
   else if this ∈ "/keyword /em /strong /cell" then
   next(prefix, stk, cmd, result, Space, w)
   else if this ∈ "/sp" then
    if not.isempty.toseqbyte.result ∧ 1^toseqbyte.result = tobyte.32 then
    next(prefix, stk, cmd, result, false, w)
    ...
    else let z = UTF8.linebreak.toseqbyte.result, next(prefix, stk, cmd, z, false, w)
   else if this ∈ "/tag" then
   next(prefix, stk, cmd, result + decodeword.w, false, 1#"/cell")
   else
    let chars2 = if not.Space then result + chars else result + char.32 + chars,
    next(prefix, stk, cmd, chars2, true, w),
 result + char.32
...
for a = tobyte.32, count = 0, e ∈ reverse.b while a = tobyte.32 do next(e, count + 1),
...
function linebreak(a:seq.byte) seq.byte if not.haslinebreak.a then a + [tobyte.10] else a 