...
for
 layers = empty:seq.seq.layernode
 , b = 1
 , idx = 1
 , e ∈ toseq.nodes.g + layernode(0, 0, 0)
do
  ...
  if node ≠ currentlayer sub n.currentlayer then next(acc, k0, l)
   ...
   if isempty.p then next(acc, k0, l)
    ...
    do
     ...
     acc3
     + if k < k0 ∨ k > k1 then [arc(start, {k1}iis)] else empty:seq.arc.layernode,
    next(acc + acc3, k1, l1),
...
let tmp = asset.all
for more = blocks, e ∈ toseq.nodes.gin do if no.e ∈ tmp then more else more + [no.e]
{assert false report %n.blocks+"/br
"+%.vertarcs+"/br
"+%.toseq(nodes.g \ asset.all)}
 ...
 next(
  if n.classlist < class.val2 then classlist + [val2]
   ...
   replace(
    classlist
    , class.val2
    , subseq(blks, 1, place - 1) + [val2] + subseq(blks, place, n.blks)
   )
