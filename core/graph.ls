...
for acc = graph(empty:set.arc.T, empty:set.arc.T, nodes)
, @e ∈ toseq.arcs.g
do
...
graph(arcs.g + a
, backarcs.g + arc(head.a, tail.a)
, nodes.g + tail.a + head.a
)
...
if isempty.new then old
 ...
 let a = 
  for acc = empty:set.T, @e ∈ toseq.new do acc ∪ successors(g, @e) /for (acc)
 let b = old ∪ new
