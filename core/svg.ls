...
for state = 0, acc = "", acc2 = "", w ∈ s
do
 let new =
  if w ∈ "/tag /sp" then acc2 + merge("?" + w) else acc2 + "::(state)" + w,
 if state = 0 ∧ w ∈ "=" then next(1, acc + "/sp", new)
 else if state = 1 ∧ w ∈ "=" then next(2, acc + "/nsp =" + dq + "/nsp", new)
 else if state = 2 ∧ w ∈ "=" then next(1, acc + "/nsp" + dq + "/sp", new)
 else if state = 2 ∧ w ∈ "/br" then next(0, acc + "/nsp" + dq, new)
 else next(state, acc + w, new)
assert state = 0 report
 {for txt2 ="", w ∈ s do if w ∈"/tag /sp"then txt2+merge("?"+w)else txt2+w}
 "incorrect format on atts"
 + acc2,
acc
...
/tag ></rect><rect = pointer-events = none = fill = white = opacity = 0.0 = x =:(nodex)= y =:(nodey)= height = 1 = width = 100 /br
/tag ></rect><text = pointer-events = none = class = nodes = x =:(nodex)= y =:(nodey)= opacity = 0.0 = x =:(nodex)= y =:(nodey)/br >:(hovertext)/tag </text></g>"
 + encodeword.[char.10]
