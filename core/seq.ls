...
Function pseq2seq(a:pseq.T) seq.T toseq.a

Function =(a:seq.T, b:seq.T) boolean
{OPTION COMPILETIME}
for isequal = n.a = n.b, i = 1, e ∈ a while isequal do next(e = b sub i, i + 1),
isequal
...
for found = empty:seq.T, e ∈ s while isempty.found do if a = e then found + e else found,
found

type pseq is sequence, a:seq.T, b:seq.T, start:int

Function seqseg(sin:seq.T, ii:int) pseq.T
let s = to:pseq.T(sin),
if n.toseq.s = 0 then pseq(n.sin + 1, sin, empty:seq.T, ii - 1)
else
 let i = ii + start.s
 let len = n.a.s,
 if i > len then
  let x = to:pseq.T(b.s),
  if n.toseq.x = 0 then pseq(n.b.s - i + len + 1, b.s, empty:seq.T, i - len - 1)
  else seqseg(b.s, i - len)
 else
  let x = to:pseq.T(a.s),
  if n.toseq.x = 0 then pseq(n.a.s - i + 1, a.s, empty:seq.T, i - 1)
  else seqseg(a.s, i)
