...
do
 if last = none then
  ...
  if not.isempty.stk then
   ...
   else if isempty.top.stk then
    {block without html escape} next(acc + toUTF8(result, true, true), 0, "", pop.stk, this)
   else
  ...
  if last = dq_1 then
   if state = seconddq then
    ...
    next(acc, seconddq, result + "/sp" + last + "/nosp", stk, this)
 ...
 if last = none then
  ...
  if not.isempty.stk then
   if state ∈ [seconddq, firstdq] then
    next(acc, indent - 1, 0, result, pop.stk, this)
   else if top.stk ∈ "block" then
    next(acc, indent - 1, pendingLine, result, pop.stk, this)
   else
    next(acc, indent - 1, 0, result, pop.stk, this)
  else
   next(acc, indent, 0, result, stk, none)
 else if state ∈ [seconddq, firstdq] then
  if last = dq_1 then
   if state = seconddq then
    next(acc, indent, firstdq, result + last, stk, this)
   else
    next(acc, indent, seconddq, result + "/sp" + last + "/nosp", stk, this)
  else if last = "/br"_1 then
   next(acc, indent, state, result + LF, stk, this)
  else
   next(acc, indent, state, result + last, stk, this)
 else if last = "/br"_1 then
  next(acc
   , indent
   , 0
