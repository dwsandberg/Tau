...
do
 let nextclass = if not.between(toint.b, 1, n.classify) then toint.b else idxNB(classify, toint.b)
 ...
 else if KEY = KEY(Kother, Klf) ∨ KEY = KEY(Kother, Kspace) ∨ KEY = KEY(Kother, Kstandalone) then next(result, words + encodeword.decodeUTF8.UTF8(bytes + this), empty:seq.byte, nextclass, b)
