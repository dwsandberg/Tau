...
else if i#a >1 j#b = GT then
union(a, b, i, j + 1, result + j#b)
else if i#a >1 j#b = EQ then
union(a, b, i + 1, j + 1, result + i#a)
else
...
result + subseq(a, i, n.a)
 ...
 let c = ai >1 j#b,
  if c = EQ then
  replace(a, b, i + 1, j + 1, result + [j#b])
  else if c = LT then
...
if l > 1 then
if (l - 1)#a >2 n = EQ then expandrangedown(a, n, l - 1) else l
else l

function expandrangeup(a:seq.T, n:T, u:int) int
if u < n.a then
if (u + 1)#a >2 n = EQ then expandrangeup(a, n, u + 1) else u
else u
