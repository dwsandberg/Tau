...
Export_(seq.T, int)T{From seq.T}
...
Function asset(s:seq.T)set.T set.for acc = empty:seq.T, @e ∈ s do setinsert(acc, @e)/for(acc)
...
else if(a_i >1 b_j) = GT then union(a, b, i, j + 1, result + b_j)
else if(a_i >1 b_j) = EQ then union(a, b, i + 1, j + 1, result + a_i)
else
...
else if(a_i >1 b_j) = EQ then diff(a, b, i + 1, j + 1)
else if(a_i >1 b_j) = LT then[a_i] + diff(a, b, i + 1, j)else diff(a, b, i, j + 1)
...
else if(ai >1 b_(j + k)) = GT then skipahead(b, j, k + k, ai)else j + k / 2 + 1
