...
Function asset(s:seq.T) set.T for acc = empty:seq.T, @e ∈ s do setinsert(acc, @e), set.acc
...
Function _(i:int, s:set.T) T i_toseq.s

Function lookup(s:set.T, val:T) set.T
let i = binarysearch(toseq.s, val),
if i > 0 then set.[i_toseq.s] else empty:set.T
...
if i > n.a then
result + subseq(b, j, n.b)
else if j > n.b then
result + subseq(a, i, n.a)
else if i_a >1 j_b = GT then
union(a, b, i, j + 1, result + j_b)
else if i_a >1 j_b = EQ then
union(a, b, i + 1, j + 1, result + i_a)
else
 let p = binarysearch(a, i + 1, n.a, j_b),
  if p > 0 then
  union(a, b, p + 1, j + 1, result + subseq(a, i, p))
  else union(a, b,-p, j + 1, result + subseq(a, i,-p - 1) + [j_b])
...
result + subseq(a, i, n.a)
 ...
 let c = ai >1 j_b,
  if c = EQ then
  replace(a, b, i + 1, j + 1, result + [j_b])
  else if c = LT then
  replace(a, b, i + 1, j, result + ai)
  else replace(a, b, i, skipahead(b, j, 1, ai), result)

function skipahead(b:seq.T, j:int, k:int, ai:T) int
if j + k > n.b then
j + k / 2 + 1
else if ai >1 (j + k)_b = GT then
skipahead(b, j, k + k, ai)
else j + k / 2 + 1
...
Function n(a:set.T) int {set cardinality} n.toseq.a
...
if l > 1 then
if (l - 1)_a >2 n = EQ then expandrangedown(a, n, l - 1) else l
else l

function expandrangeup(a:seq.T, n:T, u:int) int
if u < n.a then
if (u + 1)_a >2 n = EQ then expandrangeup(a, n, u + 1) else u
else u
