...
finalmix32.for acc = hashstart32.0, @e ∈ tointseq.a do hash32(acc, @e) /do acc
...
encodeword.for acc = empty:seq.char, @e ∈ a do acc + decodeword.@e /do acc
...
for acc = empty:seq.alphaword, @e ∈ a do acc + alphaword.@e /do acc
...
for acc = empty:seq.word, @e ∈ a do acc + toword.@e /do acc
...
/do for acc2 = empty:seq.seq.word, s2 ∈ sort.acc do acc2 + towordseq.toseq.s2 /do acc2
...
if length.l > 2 ∧ l_1 = char1."0" ∧ l_2 ∈ decodeword.first."xX" then
 if for acc = true, @e ∈ l << 2 do acc ∧ binarysearch(validhex, @e) > 0 /do acc then
  "INTEGER"_1
 else
  "ILLEGAL"_1
else
