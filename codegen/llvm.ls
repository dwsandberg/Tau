...
let a = asseq.t
let tp = typeop.a_1
let b = for acc = empty:seq.llvmtype, @e ∈ a do acc + inttollvmtype.@e /do acc,
if tp = INTEGER then
 [merge("i" + toword.a_2)]
 ...
 "function (
  $(for acc = "", @e ∈ subseq(b, 3, length.a) do acc + %.@e + "," /do acc >> 1))"
else if tp = TVOID then "VOID" else if tp = DOUBLE then "double" else "?"

Function typerecords seq.seq.int
for acc = empty:seq.seq.int, @e ∈ encodingdata:llvmtypeele do acc + toseq.@e /do acc
...
llvmtype.for acc = [toint.FUNCTION, 0], @e ∈ para do acc + typ.@e /do acc
...
let c = 
 if name = "" then
  ...
  let chars = tointseq.for acc = empty:seq.char, @e ∈ name do acc + decodeword.@e /do acc,
...
slot(addorder.llvmconst(typ.t
 , [toint.CAGGREGATE] + for acc = empty:seq.int, @e ∈ data do acc + toint.@e /do acc)
- 1)
...
for acc = empty:seq.slotrecord, @e ∈ encodingdata:llvmconst do
 acc + slotrecord.@e
/do acc
