...
for acc = "", e ∈ parts.bc do acc + %.val.e /do acc
...
if val < 32 then
 ...
 add6bits(toint(bits.val ∧ bits.31 ∨ bits.32), add(val / 32, b))
...
if bits < 58 then
 ...
 if arg < 0 then
  ...
  [templatepart(arg - ibcsubslot.t - deltaoffset + 1, Reloc)]
...
if a < 0 then
 ...
 if d < 0 then
  ...
  addplaceholder(templatepart(d, slot, Substitue), b)
...
if a ≤ 0 then
 ...
 addplaceholder(templatepart(a - slot + 1, Relocsigned), b)
...
for acc = emptyinternalbc, @e ∈ arithseq(nopara, 1, 1) do
 acc + phiinst(slot, typ, tailphi, nopara, @e)
/do acc
...
if bitstoadd > 58 then
 ...
 if toint.leftover < 32 then
  ...
  addvbr6(b ∨ (leftover ∧ bits.31 ∨ bits.32) << bitstoadd
   , bitstoadd + 6
   , leftover >> 5
   , s
   , r
 ...
 if v < 32 then
  ...
  addvbr6(b ∨ (bits.v ∧ bits.31 ∨ bits.32) << bitstoadd
   , bitstoadd + 6
   , bits.v >> 5
   , s
   , r
...
for acc = bits, @e ∈ s do addrecord(acc, abbrevlength, @e) /do acc
...
for acc = a2, @e ∈ subseq(a, 2, length.a) do addvbr6(acc, @e) /do acc
...
let a7 = for acc = a6.p, @e ∈ bodytxts do addbody(acc, offset, @e) /do acc
{sym table}
...
let offset = length.constantrecords
let a7 = for acc = a6.p, @e ∈ bodies do addbody(acc, @e) /do acc
{sym table}
let symtabheader = addblockheader(a7, MODABBREVLEN, toint.VALUESYMTABLE, TYPEABBREVLEN)
let a8 = finishblock(symentries(symtabheader, constantrecords, 1), length.symtabheader, TYPEABBREVLEN),
{finish module block} toseqseqbyte.finishblock(a8, length.h.p, MODABBREVLEN)
...
let a = 
 addrecords(h
  , MODABBREVLEN
  , [[1, 1]
   , [toint.TRIPLE] + for acc = empty:seq.int, @e ∈ triple.info do acc + toint.@e /do acc
   , [toint.LAYOUT]
   + for acc = empty:seq.int, @e ∈ datalayout.info do acc + toint.@e /do acc]
  )
{type block}
 ...
 finishblock(
  addrecords(pgh
   ...
   , [
    ...
    + tointseq.for acc = empty:seq.char, @e ∈ "no-frame-pointer-elim-non-leaf" do
     acc + decodeword.@e
    /do acc /for
    + [0]]
   )
...
{const block}
let g = 
 for acc = trackconst(a5,-1, 0)
  , @e ∈ subseq(constantrecords, length.deflist + 1, offset)
 do
  constrecords(acc, @e)
 /do acc
