...
for acc = empty:seq.slot, @e ∈ encodingdata:const3 do acc + flds.@e /do acc
...
hash.for acc = empty:seq.int, @e ∈ flds.a do acc + toint.@e /do acc
...
/do
 ...
 /do loadbcwords.bcwords

Function wordstoadd(baselibwords:seq.seq.char) seq.encoding.word3
let have = 
 for acc = empty:set.encoding.word3, @e ∈ baselibwords do acc + encode.word3.@e /do acc
let used = 
 for acc = empty:set.encoding.word3, @e ∈ encodingdata:word3 do acc + encode.@e /do acc
,
toseq(used \ have)
...
for acc = empty:seq.char, c ∈ bcwords do
 if c = bcwordsep then
...
let ll = 
 global(name
  , objtype
  , AGGREGATE.for acc = empty:seq.slot, @e ∈ data do acc + asi64.slot.@e /do acc)
,
...
let flds = for acc = empty:seq.slot, @e ∈ fldsin do acc + asi64.slot.@e /do acc
...
addobject.for acc = [toint.C64.0, toint.C64.length.a], @e ∈ a do acc + wordref.@e /do acc 