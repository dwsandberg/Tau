...
Function groupparagraphs(break:seq.word, a:seq.seq.word)seq.seq.seq.word
 ...
 for acc = empty:seq.seq.seq.word, @e = finishpair(for acc = empty:seq.seq.int, @e = arithseq(length.a, 1, 1)do addpair(acc, findparagraph(break, a, @e))end(acc), length.a)do
  acc + extractblock(a, @e)end(acc)
...
Function getlibrarysrc(libname:seq.word)seq.seq.word
 ...
 for acc = empty:seq.seq.word, @e = filelist do acc + gettext.[ merge([ first.libname] + "/" + @e + ".ls")]end(acc)
