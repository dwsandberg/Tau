...
function +(p:place, r:encodingpair.seq.char)place p + valueofencoding.code.r + tointseq.data.r + hash.r
...
if isempty.t then 0
 ...
 let p =
  if isempty.have then place(empty:seq.int, 0, empty:seq.int)
  else
...
/Function loaddict(file:fileresult)int if size.file >-1 then let data = data.file // deepcopy. // get2(data, length 
.data)@ +(0, primitiveadd.@e)else 0

function get2(data:seq.int, i:int)seq.encodingpair.seq.char
{ file is built by append new data to the end followed by two words. The first is the start of the new data and the second is the 
 size of the data before the new data was appended. To read the file the appended segements are combined into one long sequence 
. }
...
Function +(p:place, c:seq.int)place place(this.p + (next.p + 1), offset.p, data.p + [ 0, length.c] + c)
...
assert between(i, 1,(data.a)_(offset.a + 1))
report"out of bounds2"
