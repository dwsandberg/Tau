...
function +(p:place,r:encodingrep.seq.char) place  p+valueofencoding.code.r+tointseq.data.r+hash.r
 
Function writedict(tin:seq.encodingrep.seq.char) int
  let have=asset.initialdict
  let t=toseq(asset.tin-have)
  if isempty.t then 0 else
  let p=if isempty.have then 
   place(empty:seq.int,0,empty:seq.int)
  else
