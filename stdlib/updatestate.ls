...
for acc = "", a ∈ breakparagraph.data.first.input do
 if subseq(a, 1, 1) ∈ ["-", "comment"] then
  ...
  for txt = "", w ∈ a do
   if w ∈ "define:" ∧ not.isempty.txt then txt + "/br define"
   else txt + w
  /for (acc + "/p define" + txt << 1)
 else
  for txt = "", have = "", lastbreak = 0, p ∈ a do
   if p ∈ "+=" then
    next(if p ∈ "+" then txt + "/br+"
    else txt >> 1 + "/br" + last.txt + p
    , have
    , 0
    )
   else
...
if i > length.s then s
 ...
 subseq(s, 1, i - length.b) + replacement
 + substitute(s << i, b, replacement)
...
let unchanged = 
 if isempty.lines then empty:seq.filename
  ...
  for txt = empty:seq.filename, line ∈ lines do
   txt
   + tofilename.UTF8 (toseqbyte.line << (2 + i + length.decodeword.first."./"))
  /for (txt)
let r = buildgraph(input << 2, builddir)
let g = newgraph.arcs.r
let changed = 
 toseq (asset.sinks.g \ asset.unchanged
 \ asset.[filename (builddir + "orgstdlib.lib")
 , filename (builddir + "shell.lib")
 ])
...
do
 ...
 if first.aa ∈ "comment" then next(acc, defs2, defines)
  ...
  let tmp2 = 
   getfilenames (builddir + cmdlib + subseq(aa, 2, length.aa - length.rest + 3))
  let tail = tmp2_2
...
if lib.cmd ∈ "shell" then "/br $(cmds.cmd + parts << 1) >" + node
 ...
 /for (
  ...
  "
   ...
   /br libexe $(lib.cmd) makebitcode+$build $([name.out] + "
   ." + ext.out + libinfo)
