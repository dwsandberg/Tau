...
let a = breakblocks(p, code, 1, 1, empty:seq.symbol, empty:stack.block)
 ...
 ∧ for acc = false, @e = a do acc ∨ tailrecursion(self, @e)end(acc)then
 \\ tail recursion \\
  let nopara = nopara.self
  let a2 = for acc = empty:seq.block, @e = a do acc + preparetail(nopara, self, continue.nopara, @e)end(acc)
  ...
  + Loopblock.for acc = empty:seq.mytype, e3 = paratypes.self do acc + getbasetype(alltypes, e3)end(acc))
...
function cvtlabels(blks:seq.block, i:int, result:seq.block)seq.block
 ...
 let b = blks_1
  ...
  assert length.code.b = 3 report"OPT P"
    for acc = empty:seq.block, @e = subblks.b do acc + convertexits(blkno.blks_(value.(code.b)_1), blkno.blks_(value.(code.b)_2), @e)end(acc)
  else [ blks_i]
...
function convertexits(label1:int, label2:int, b:block)block
 if kind.b ∈ "BR JMP"then b
  ...
  assert kind.b = "EXIT"_1 report"unexpected block type" + kind.b
   ...
   let target = if Lit0 = (code.b)_1 ∨ Litfalse = (code.b)_1 then label2 else label1
...
function gathercase(l:seq.block, blk:block, exp:seq.symbol, caseblks:seq.caseblock)gathercaseresult
 ...
 let t = if fsig.code_(len - 3) ∈ ["=(int, int)","=(word, word)"]then
 [ caseblock(label, blkno.blk, code_(len - 4))]
 else
  let rep = code_(len - 4)
   if module.rep = "$words"then
   for acc = empty:seq.caseblock, e6 = fsig.rep do acc + caseblock(label, 0, e6)end(acc)
   else
    assert length.constantcode.rep > 2
    ∧ ((constantcode.rep)_1 = Lit.0 ∨ (constantcode.rep)_1 = Lit.1)report"not a standard seq" + print.code
     for acc = empty:seq.caseblock, @e = subseq(constantcode.rep, 3, length.constantcode.rep)do acc + caseblock(label, 0, @e)end(acc)
  ...
  let newblock = findblk2(l, 1, label2.blk)
   ...
   ∧ 1 = for acc = 0, e5 = l do acc + indegree(blkno.newblock, e5)end(acc)then
...
function print(b:block)seq.word
 ...
 "(" + for acc ="", @e = subblks.b do acc + print.@e end(acc)
  + ")"
