...
Function rotateleft(x:bits,n:int) bits
  {(x << n ) &or (x >> (64-n)  )
}
   ...
   let PRIME1 = 11400714785074694791
    let PRIME2 = 14029467366897019727
         let   PRIME4 =  9650029242287828579
            bits(   toint.rotateleft(xor (acc,bits(toint.rotateleft(bits(toint.acc+ x * PRIME2),31) * PRIME1)),27) * PRIME1+PRIME4)
               
Function hashstart(seed:int) bits let   PRIME5 =  2870177450012600261   bits(seed+PRIME5+64)
