...
Function postorder(a:tree.T)seq.tree.T for acc = empty:seq.tree.T, @e = sons.a do acc + postorder.@e /for(acc)+ a

Function replace(with:T, a:T, t:tree.T)tree.T let newlabel = if label.t = a then with else label.t tree(newlabel, for acc = empty:seq.tree.T, @e = sons.t do acc + replace(with, a, @e)/for(acc))
