 ...
 function abstractarcs(s: seq.symdef ) seq.arc.symbol
   ...
   if isspecial.codesym ∨ not.isabstract.module.codesym ∨ sym = codesym ∨ isBuiltin.codesym then arcs
    ...
    if name.codesym ∈ "name for"then arcs
     ...
     arcs + arc(sym, symbol(moduleref("stdlib seq", resulttype.codesym),"_", seqof.resulttype.codesym, typeint, resulttype.codesym))
...
function removesinks(sinkstokeep:set.symbol, g:graph.symbol, toprocess:seq.symbol)seq.arc.symbol
 ...
 /for(let newsinks = for acc = empty:seq.symbol, p = toseq.pred do
...
Function compile(allmods:set.passsymbols, modlist:set.passsymbols, lib:word, src:seq.seq.word, mode:word, requireUnbound:set.symdef)seq.symdef
...
Function passparse(abstractmods:set.passsymbols, simplemods:set.passsymbols, lib:word, prg1:seq.symdef, src:seq.seq.word, mode:word)set.symdef
...
{ graph g3 has three kinds of sinks.
 ...
 { change many-to-one relation defined by arcs in g5 into format of set.symdef }
  let requireUnbound = if isempty.arcs.g4 then empty:set.symdef
  else
   ...
   let newacc = if last ≠ tail.a then if isempty.list then acc else acc + symdef(last, list)else acc
 ...
 asset(  prga+prg1+  prg )
   ...
   if islocal.sym then result + Local.value.sym
    ...
    { if isconst.sym /or isspecial.sym then result + sym else assert subseq(print.sym.p, 1, 5)/ne"tree.T:="/or module.sym.p = module.sym report"KL"+ print.sym.p + print.sym }
    result
...
/ function arcinfo(l:seq.arc.symbol)seq.arcinfo.seq.word for acc = empty:seq.arcinfo.seq.word, a = l do acc + arcinfo(print.tail.a, print.head.a,"")/for(acc)
