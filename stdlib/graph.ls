...
function reachable(g:graph.T, old:set.T, new:set.T, count:int)set.T
 assert count < 1000 report"fal" + toword.cardinality.old + toword.cardinality.new
  if isempty.new then old
   ...
   let a = @(∪, successors.g, empty:set.T, toseq.new)
   let b = old ∪ new
...
Function transitiveClosure(g:graph.T)graph.T
 // add arcs to graph so if node is reachable, it can be reached with single arc // @(addclosure, identity, g, toseq.nodes.g)

function addclosure(g:graph.T, n:T)graph.T
 // add arcs to graph so path does not need to go through n // g
 + toarcs(toseq.predecessors(g, n), toseq.successors(g, n))
