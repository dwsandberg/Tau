...
Function createfile(filename:seq.word, s:seq.seq.word)int
 createfile(filename, for acc = empty:seq.byte, @e = s do
  acc + toseqbyte.toUTF8.@e + [ tobyte.10, tobyte.10]
 /for(acc))
...
Function createfile(name:seq.word, a:seq.byte)int
 createfile3(length.a, packed.bits.for acc = empty:bitstream, @e = a do add(acc, bits.toint.@e, 8)/for(acc), tocstr.name)

Function createlib(b:seq.bits, libname:word, dependlibs:seq.word)int
 createlib2(tocstr.[ libname], tocstr.for acc ="", @e = dependlibs do acc + [ @e] + ".dylib"/for(acc), length.b * 8, packed.b)
