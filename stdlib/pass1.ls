...
Function pass1(allsrc:seq.seq.seq.word, exports:seq.word, librarymods:seq.firstpass)linkage
 let alllibsym = librarymods @ ∪(empty:set.symbol, defines.@e)
 ∪ (librarymods @ ∪(empty:set.symbol, exports.@e))
...
function processtypedef(defined:typedict, undefined:seq.myinternaltype, i:int, newundefined:seq.myinternaltype)typedict
 ...
 if length.newundefined = 0 then defined
  ...
  let td = undefined_i
  let fldtypes = if kind.td ∈ "record"then subflds.td @ +(empty:seq.mytype, parameter.@e)
  else if kind.td ∈ "sequence"then subflds.td @ +([ typeint], parameter.@e)else subflds.td
  ...
  if orgcount = 0 then modset
   ...
   let newcount = toseq.newset @ +(0, totalunbound.@e)
    ...
    assert orgcount = 0 report"unresolved exports"
     + u @ +(empty:seq.symbol, unboundexports.@e) @ +("", print.@e)
      modset
