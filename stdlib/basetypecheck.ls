...
let codeonly = removeoptions.removeoptions.removeoptions.code
 if length.codeonly = 0 then""
  ...
  let localdict = for acc = emptyworddict:worddict.mytype, @e = arithseq(nopara.s, 1, 1)do addpara(acc, alltypes, paratypes.s, @e)/for(acc)
 ...
 if i > length.code then
  ...
  let s = code_i
   assert not.isempty.module.s report"Illformed module on symbol"
    ...
    else if(fsig.s)_1 = "bitcast"_1 ∧ module.s ≠ "interpreter"
    ∧ nopara.s ≠ 0 then
