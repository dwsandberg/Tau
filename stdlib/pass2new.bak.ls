...
Function firstopt(p:program, rep:symbol, code:seq.symbol) program
 ...
 let code2 = code.yyy(p, deepcopy.code, 1, nopara + 1, pdict)
 ...
 let a2=code.yyy(p,a,1,nopara+1,pdict)
...
function yyy(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol)expandresult
 ...
 if module.sym in ["$constant","$fref","$word","$words","int $"] then yyy(p, s, i + 1, nextvar, map)
   ...
   if  module.sym="local" then 
    ...
    if not.isempty.dd &and "INLINE"_1 in options.dd then
      ...
      inline(p, s, i, nextvar,nopara,code,map)
...
function optoneop(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)expandresult
...
function opttwoopbuiltin(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)expandresult
 if s_i = IDXUC   then
    let j = value.s_(i - 1)
    let x =  s_(i - 2)
...
function islit(s:symbol) boolean   module.s ="int $" 
 ...
 function inline(p:program, s:seq.symbol, i:int, nextvar:int,nopara:int,code:seq.symbol, map:worddict.seq.symbol)
  expandresult
  ...
  // assert not((print.[ s_i])_1 = "message"_1)report print.[ s_i] + if issimple.k then"SIMPLE"else"" //
   ...
   // function just returns result // 
       yyy(p,replace(s, i, 1, empty:seq.symbol),i,nextvar,map)
    else
    let t = backparse(s, i - 1, nopara, empty:seq.int) + i
     ...
     let new = if issimple(p,nopara,code) then 
         ...
         code.yyy(p, code, 1, nextvar, pmap)
    else 
   ...
   // assert not(fsig.s_i="z") &or i in [3,4,9]    report "P"+toword.i+print.s
       ...
       yyy(p, replace(s, t_1, i - t_1 + 1, new),t_1 + length.new,nextvar + nopara,map)
...
function expandinline(s:seq.symbol, t:seq.int, pmap: worddict.seq.symbol, i:int, newcode:seq.symbol, nextvar:int, inlinecode:seq.symbol, p:program)seq.symbol
 ...
 if i = 0 then newcode+code.yyy(p, inlinecode, 1, nextvar, pmap)
 else
...
function applycode(p:program, nextvar:int, code:seq.symbol, index:int,map:worddict.seq.symbol)  expandresult
 ...
 ∧ not((fsig.term2_1)_1 = "deepcopy"_1)
  ...
  // assert not(subseq(code, t_2, index - 4)=[var.1]) report "XXXX"+print.code+"/new/"+print.new //
     yyy(p, new + subseq(code, index + 1, length.code),length.new + 1,nextvar,map)
  else
...
yyy(p, replace(code, t_1, index - t_1 + 1, new), t_1 + length.new, nextvar.body, map)
