...
Function isexternal(s:symbol) boolean
   isbuiltin.module.s &and not((fsig.s)_1 in    "aborted  loadlib createlib unloadlib allocatespace 
 addencoding createfile getinstance dlsymbol getfile  addresstosymbol2
 randomint getmachineinfo currenttime callstack initialdict   createthread assertptr assertreal assert  ")
...
function addtemplate(sym:symbol, length:int, parts:seq.templatepart, action:word, arg:int, code:seq.symbol, functype:llvmtype) 
 match5 
...
function addtemplate(sym:symbol, length:int, b:internalbc)match5
  addtemplate(sym, length, getparts.b,"TEMPLATE"_1, slot.nopara.sym)
 
function findtemplate(d:symbol) seq.match5 
findencode.match5(d, 0, empty:seq.templatepart,"NOTFOUND"_1,  0,empty:seq.symbol,i64)
...
, addtemplate(symbol("isnull(ptr)","builtin","boolean"), 3, CAST(r.1, slot.ibcsub1, i64, ptrtoint) + CMP2(r.2, r.1, C64.0, 32)
+ CAST(r.3, r.2, i64, zext))
, addtemplate(symbol("nullptr","builtin","ptr"), 1, CAST(r.1, C64.0, ptr.i64, inttoptr))
, addtemplate(symbol("IDX(int seq,int)","builtin","int"), 2, GEP(r.1, i64, slot.ibcsub1, slot.ibcsub2) + LOAD(r.2, r.1, i64))
, addtemplate(symbol("IDX(int seq seq,int)","builtin","int seq "), 3, GEP(r.1, i64, slot.ibcsub1, slot.ibcsub2) + LOAD(r.2, r.1, i64)
+ CAST(r.3, r.2, ptr.i64, inttoptr))
, addtemplate(symbol("IDX(ptr seq,int)","builtin","ptr"), 3, GEP(r.1, i64, slot.ibcsub1, slot.ibcsub2) + LOAD(r.2, r.1, i64)
+ CAST(r.3, r.2, ptr.i64, inttoptr))
, addtemplate(symbol("IDX(real seq,int)","builtin","real"), 3, GEP(r.1, i64, slot.ibcsub1, slot.ibcsub2) + LOAD(r.2, r.1, i64)
+ CAST(r.3, r.2, double, bitcast))
  , // addtemplate(1,"getseqtypeZbuiltinZTzseq"_1, 1, LOAD(r.1, slot.ibcsub1, i64))  
...
function buildtemplate( theprg:program,  alltypes:typedict,xx:symbol)seq.symbol
  let pkg = module.xx
   if pkg = "$constant"then [xx]
      ...
      addtemplate(xx, 0, empty:seq.templatepart,"ACTARG"_1, if pkg = "$real"then Creal.toint.(fsig.xx)_1 
      else C64.toint.(fsig.xx)_1)
      else if islocal.xx then
      addtemplate(xx, 0, empty:seq.templatepart,"LOCAL"_1, slot.toint.(fsig.xx)_1)
      else if isdefine.xx then
      addtemplate(xx, 0, empty:seq.templatepart,(fsig.xx)_1, slot.toint.(fsig.xx)_2)
