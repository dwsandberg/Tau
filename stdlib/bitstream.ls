...
/ function singlebit(a:bitstream, i:int)int // does format checking and gets bit i // assert between(i, 1, length.a)report"out bound singlebit"assert(length.a + 63)/ 64 = length.fullwords.a + 1 report"rep problem"+ stacktrace let j =((i-1)/ 64 + 1)let b = if j > length.fullwords.a then let partbits =(length.a mod 64)assert partbits = 0 &or bits.0 = endpart.a >>(partbits)report"stray bits"+ stacktrace endpart.a else(fullwords.a)_j toint(b >>((i-1)mod 64)&and 0x1)

Function empty:bitstream bitstream bitstream(0, 0x0, empty:seq.bits)

function =(a:bitstream, b:bitstream)boolean
 length.a = length.b ∧ endpart.a = endpart.b ∧ fullwords.a = fullwords.b

Function patch(a:bitstream, i:int, val:int)bitstream
 subseq(a, 1, i - 1) + bitstream(32, bits.val)
 + subseq(a, i + 32, length.a)
...
function print(x:bitstream)seq.word
 if length.x = 0 then"empty"
  ...
  let part = if k = 16 then start else [ toword.k] + "bits of" + start
   part + for acc ="", @e = reverse.fullwords.x do acc + print.@e end(acc)

function firstword(x:bitstream)bits if isempty.fullwords.x then endpart.x else first.fullwords.x

function ithword(x:bitstream, i:int)bits if i > length.fullwords.x then endpart.x else(fullwords.x)_i
...
Function add(a:bitstream, b:bits, nobits:int)bitstream
 ...
 let firstwordb = b ∧ bits.-1 >> (64 - nobits)
  if partbitsa = 0 ∧ length.a > 1 then
...
Function +(a:bitstream, b:seq.byte)bitstream for acc = a, @e = b do acc + @e end(acc)
