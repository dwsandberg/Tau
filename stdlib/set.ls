...
else if(ai ? b_(j + k)) = GT then skipahead(b, j, k + k, ai)else j + k / 2 + 1
...
if l > 1 then if ?2(a_(l - 1), n) = EQ then expandrangedown(a, n, l - 1)else l
else l

function expandrangeup(a:seq.T, n:T, u:int)int
if u < length.a then if ?2(a_(u + 1), n) = EQ then expandrangeup(a, n, u + 1)else u
else u
