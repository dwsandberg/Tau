...
function codeforbuiltin(alltypes:typedict, newsym:symbol, oldsym:symbol, org:seq.word)seq.symbol
 let newmodpara = parameter.modname.newsym
  ...
  else if fsig.oldsym = "allocatespace:T(int)"then
  [ Local.1, symbol("allocatespace(int)","builtin","int seq")]
  ...
  else if fsig.oldsym = "packed(T seq)"then [ Local.1] + blocksym.gettypeinfo(alltypes, newmodpara)
