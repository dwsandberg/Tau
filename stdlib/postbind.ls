 ...
 if length.aa = last then
  ...
  let symz = data.pp
   if isspecial.symz ∨ isconst.symz ∨ isBuiltin.symz ∨ isGlobal.symz ∨ inModFor.symz then accZ
    ...
    let sd = if not.isempty.lr1 ∨ iscompiled(lr1, symz)then symdef(symz, lr1)
    ...
    {------------}
     ...
     let pdict = for pmap =  empty:set.localmap2  , parano =   arithseq(nopara.sym.sd,1, 1)  do 
      ...
      if name.module.sym ∈ "$define"then
       ...
       let encodenocode = if typ = typeref."typename tausupport stdlib"then [ Lit.2]
        ...
        let discard = symbolref.encodenosym
         ifthenelse([ gl, Lit.0, Getfld.typeint, Define.nextvar, Local.nextvar, Lit.0, EqOp], [ gl, Words.print.typ, encodenosym, setSym.typeint, Define(nextvar + 1), gl, Lit.0, Getfld.typeint], [ Local.nextvar], typeint)
...
if isspecial.sym then
 ...
 if a = b then empty:seq.symbol else [ symbol(internalmod,"bitcast", a, b)]
  ...
  let typ = para.module.sym
   if typ = typeint ∨ typ = typereal ∨ typ = typeptr ∨ isseq.typ then
    Lit.1
   else
  ...
  let typ = resulttype.sym
   if iscoretype.typ then Getfld.typ
    ...
    assert not.isempty.a report"cannot find type getfld" + print.typ
      if length.a > 1 then symbol(internalmod,"GEP", seqof.typeptr, typeint, typeptr)else Getfld.first.a
...
function deepcopybody(type:mytype, typedict:typedict)seq.symbol
 ...
 { base types are int real boolean ptr seq.int seq.real seq.boolean seq.ptr seq.byte seq.bit seq.packed2 seq.packed3 seq.packed4 seq.packed5 seq.packed6 or $base.x where x is a integer }
 ...
 [ Local.1, symbol(modTausupport,"blockIt", seqof.typeint, seqof.typeint)]
  ...
  if length.subflds = 1 then
    { only one element in record so type is not represent by actual record }[ Local.1] + deepcopySym.first.subflds
   else
...
use set.localmap2
 ...
 Export _(set.localmap2,int) localmap2
...
Function ?(a:localmap2,b:localmap2) ordering key.a ? key.b 
 ...
 Export _(seq.localmap2,int) localmap2
  ...
  let tablesize = length.table.h
    ...
    for acc=empty:seq.T,   idx=arithseq( tablesize,1,1)     do
      for acc2=acc,   e=  (table.h)_idx do 
          if (bits.hash.e ∧ mask) =bits(idx-1)  then
         acc2+data.e  else acc2 /for(acc2) 
    /for(acc)
...
Function +(h:hashset.T, ele:T)hashset.T
 ...
 for acc = empty:seq.hashelement.T, found=false,e =(table.h)_dataindex do
     if data.e = ele  then next(acc+e,true)  else
    if  notsamehash (ele,hash, hash.e, mask)then next(acc,found)
...
Function  /cup(ele:T,h:hashset.T) hashset.T
   replace(h,hashelement(ele,hash.ele)) 
...
function replace(h:hashset.T, ele:hashelement.T)hashset.T
 ...
 for acc = [ ele],found=false, e =(table.h)_dataindex do
    ...
    else if  notsamehash (data.ele,hash, hash.e, mask) then next(acc,found)
    else next(acc + e,found)
   /for(  
