...
Function print(s:modref) seq.word
 if issimple.s then [ name.s]
 else [ name.s,"."_1] + print.para.s
...
Function TypeFromOldTyperep(m:seq.word) mytype  
 mytype(for acc=empty:seq.typedef,e=m do  [typedef(e,"?"_1,"?"_1)]+acc  /for(acc)
)
...
let t=first.typerep.typ
 modref(library.t,modname.t,if length.typerep.typ > 1  then  typeT  else
mytype.empty:seq.typedef)
...
Function printrep(s:mytype) seq.word
  for acc =[toword.length.typerep.s], t = typerep.s do acc + [ name.t,modname.t,library.t] 
  /for(acc )
...
Function resolvetypes(t:seq.seq.word, lib:word)set.passtypes
 for knownmods = empty:set.modref, s = empty:set.passtypes, defines = empty:set.mytype, unresolveduses = empty:seq.seq.word, unresolvedexports = empty:seq.seq.word, mref = internalmod, m = t do
  ...
  else if first.m ∈ "type"then next(knownmods, s, defines + 
   newtype(mref, m_2), unresolveduses, unresolvedexports, mref)
