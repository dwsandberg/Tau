...
let libdesc= libdesc(cinfo,prg4 )
  let bc = codegen( prg4, roots.cinfo ∪ asset.liblibflds.libdesc, last.libname, libdesc, typedict.cinfo, isempty.dependentlibs)
...
let p2=process.compilerfront("pass1", "runit",src,"stdlib" + lib,"$X")
  if aborted.p2 then message.p2
   ...
   let p3 = process.interpret( typedict.result.p2, getCode(theprg, symbol(moduleref."runit $X","runitx", seqof.typeword)))
