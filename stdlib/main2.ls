...
let aa = 
 for acc = empty:seq.seq.word, modname = "?"_1, p ∈ breakparagraph.input do
  if subseq(p, 1, 1) = "Function"
  ∧ subseq(p, 3, 8) = "(input:seq.file" then
   let t = "use" + modname
...
for txt = "", useclauses = "", p ∈ aa do
 if first.p ∈ "use" then next(txt, useclauses + p + "/p")
  ...
  do
   ...
   else if last ∈ "," then next(para, w, w, "")
   else next(para, name, w, type)
  /for (
   txt
   + "
    /br else if cmd /in $(dq.subseq(p, 2, 2)) then $(subseq(p, 2, 2)) (input
    $(para))")
  , useclauses
  )
/for (
 if isempty.txt then txt
  ...
  "Module $(merge (libname + "$root"))
   ...
   /p use seq.file
   /p use standard
   /p $(useclauses) Function $(merge (libname + "
   $EP")) (args:seq.word, input:seq.file) seq.file
...
, "use seq.file"
, "use process.UTF8"
, "Function entrypoint (args:UTF8) UTF8 let p = process.entrypoint2 (args) if aborted.p then
 finishentry.[file ($(dq."error.html"), message.p)] else result.p"
...
let entrymod = 
 if allsrc_2 = "Module" + merge (libname + "$root") then
...
if aborted.p then
 [file("error.html"
 , "COMPILATION ERROR in libray:$(libname) /br $(message.p)"
 )
 ]
else result.p
