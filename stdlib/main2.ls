...
function subcompilelib(option:seq.word, libname:seq.word)seq.seq.word
 ...
 let prg3 = for acc = prg2, @e = for acc = empty:seq.seq.word, @e = allsrc do acc + @e end(acc)do
  processOption(acc, @e)end(acc)
   if option = "pass1"then
  for acc = empty:seq.seq.word, @e = toseq.prg3 do acc + print(prg3, @e)end(acc)
   else
    ...
    for acc = empty:seq.seq.word, @e = defines do acc + print(prg4, @e)end(acc)
...
Function testcomp(s:seq.seq.word)seq.seq.word
 ...
 for acc = empty:seq.seq.word, @e = toseq.result.r do acc + print(result.r, @e)end(acc)
...
Function print(a:seq.seq.word)seq.word
 for acc ="", @e = a do acc + " &br  &br" + @e end(acc)
