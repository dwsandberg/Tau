...
let b = parse(bindinfo(empty:set.symbol, empty:seq.symbol, empty:seq.mytype,""), ' function a int f("hi")')
 "OK"
...
function  apply(initseq:bindinfo, accsym:bindinfo, initacc:bindinfo, args:bindinfo, input:seq.token.bindinfo, place:int)bindinfo
...
Function action(ruleno:int, input:seq.token.bindinfo, R:reduction.bindinfo)bindinfo
 ...
 assert(tokentext.R_4)_1 in "record sequence"report errormessage("Expected record or sequence after is in type definition got:" + tokentext.R_4, input, place.R)
 ...
 bindinfo( types.R_1  @@ addparameter(dict.R ,cardinality.dict.R, input, place.R,@e), empty:seq.symbol, types.R_1,"")
 ...
 else if ruleno = // E unused // 19 then R_1
 else if ruleno = // E E_E // 20 then opaction(R, input)
 else if ruleno = // E-E // 21 then unaryop(R, input, tokentext.R_1, R_2)
 else if ruleno = // E W.E // 22 then unaryop(R, input, tokentext.R_1, R_3)
 ...
 bindinfo(dict.R, code.R_3, types.R_3, tokentext.R_1)
 else if ruleno = // K N // 50 then R_1
 else if ruleno = // K NM(L)// 51 then
 bindinfo(dict.R, code.R_3, types.R_3, tokentext.R_1)
 else if ruleno = // K NM // 52 then R_1
 else if ruleno = // NM W // 53 then R_1
 else if ruleno = // NM W:T // 54 then
 bindinfo(dict.R, empty:seq.symbol, empty:seq.mytype, tokentext.R_1 + ":" + print.(types.R_3)_1)
 else if ruleno = // E @(K, K, E, E)// 55 then
  assert false report errormessage("oldformat", input, place.R)
 R_1
 else if ruleno = // D E @@ // 56 then declareapplyvars(R_1, input, place.R)
 else if ruleno = // E D NM(E, L)// 57 then
 apply(R_1, R_2, R_4, R_6, input, place.R)
 else
