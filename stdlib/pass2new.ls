...
function yyy(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol)ipair.seq.symbol
 ...
 if module.sym in ["$constant","$fref","$word","$words","int $"] then yyy(p, s, i + 1, nextvar, map)
 else
   if  (module.sym)_1="para"_1 &and last.module.sym="$"_1 then  
      yyy(p, replace(s,i,1,[Local.(module.sym)_2]),i,nextvar,map)
    ...
    //  assert isempty.dd &or ( isinline.s_i = "INLINE"_1 in options.dd) 
       &or subseq(fsig.sym,1,1)=[merge."to:pseq.T"] report "XX"+options.dd+print.s_i+print.dd  
    //  if not.isempty.dd &and "INLINE"_1 in options.dd then
         let code =   if (last.dd=Optionsym) then subseq(dd,1,length.dd-2) else   dd
         if isempty.code then yyy(p, s, i + 1, nextvar, map)
         else
  ...
  //  two parameters with   constant  args //
   if module.sym="builtin" then opttwoopbuiltin(p, s, i , nextvar, map,sym)
    ...
    if module.arg1 = "$words" &and  between(idx, 1, length.fsig.arg1) then
      yyy(p, replace(s, i - 2, 3, [ Word.(fsig.arg1)_idx]), i - 1, nextvar, map)
 ...
 let arg1 =  s_(i - 2)
  let arg2 =  s_(i - 1)
   if module.arg1 = "$words" ∧ module.arg2 = "$words"then
     yyy(p, replace(s, i - 2, 3, [ Words(fsig.arg1 + fsig.arg2)]), i - 1, nextvar, map)
   else yyy(p, s, i + 1, nextvar, map)
 else yyy(p, s, i + 1, nextvar, map)
 
function optoneop(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)ipair.seq.symbol
 ...
 let arg1 =  s_(i - 1)
   if module.arg1 = "$words"then
     yyy(p, replace(s, i - 1, 2, [ Word.merge.fsig.arg1 ]), i, nextvar, map)
   else yyy(p, s, i + 1, nextvar, map)
else yyy(p, s, i + 1, nextvar, map)

function opttwoopbuiltin(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)ipair.seq.symbol
 if s_i = IDXUC   then
    let j = value.s_(i - 1)
    let x =  s_(i - 2)
...
function optmodule$(p:program, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)ipair.seq.symbol
if  fsig.rep="BLOCK 3" then
    ...
    yyy(p, replace(s, i - 1, 2, empty:seq.symbol), i - 1, nextvar, replace(map, thelocal, [ s_(i - 1)]))
   else
...
function islit(s:symbol) boolean   module.s ="int $" 
 ...
 function inline(p:program, s:seq.symbol, i:int, nextvar:int,nopara:int,code:seq.symbol)expandresult
   assert length.code > 0 report "INLINE PROBLEM"+print.s_i
  // assert not((print.[ s_i])_1 = "message"_1)report print.[ s_i] + if issimple.k then"SIMPLE"else"" //
    if length.code = 1 ∧ code = [ var.1]then
   // function just returns result // expandresult(nextvar, i, replace(s, i, 1, empty:seq.symbol))
   else
    let t = backparse(s, i - 1, nopara, empty:seq.int) + i
     ...
     let new = if issimple(p,nopara,code) then 
         expandsimpleinline(s, t, emptyworddict:worddict.seq.symbol, nopara, nextvar, code, p)
    else 
          expandinline(s, t, emptyworddict:worddict.seq.symbol, nopara, empty:seq.symbol, nextvar, code, p)
   // assert not(fsig.s_i="z") &or i in [3,4,9]    report "P"+toword.i+print.s
         +"&br -------------------"+print.replace(s, t_1, i - t_1 + 1, new) //
      expandresult(nextvar + nopara, t_1 + length.new, replace(s, t_1, i - t_1 + 1, new))



function expandsimpleinline(s:seq.symbol, t:seq.int, pmap: worddict.seq.symbol, i:int, nextvar:int, inlinecode:seq.symbol, p:program)seq.symbol
...
type expandresult is record nextvar:int, index:int, code:seq.symbol

function applycode(p:program, nextvar:int, code:seq.symbol, index:int)expandresult
...
/Function toprg(p:program) program 
 @(cvt,identity,emptyprogram,toseq.toset.p)
    
/function cvt(p:program,s:symbol) program
