...
do
 if nodeno ∉ casenodes then next(cases, e, nodeno + 1, nextcase - 1, first, eqivs)
  ...
  if isempty.first then
   ...
   else next(cases + casenode(code_(e - 2), nodeno, brt), e, nodeno + 1, brf.sym, b, empty:seq.symbol)
  else if nextcase = 1 ∧ (subseq(code, last + 1, e - 3) = first ∨ last = e - 4 ∧ code_(e - 3) ∈ eqivs)then
   next(cases + casenode(code_(e - 2), nodeno, brt), e, nodeno + 1, brf.sym, first, eqivs)
  else if nextcase = 1 ∧ islocal.code_(e - 3) ∧ Define.value.code_(e - 3) = code_(e - 4)
