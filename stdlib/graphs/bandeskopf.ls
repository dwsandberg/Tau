...
function iis(g:graph.T, dummy:set.T, node:T)seq.T
 ...
 if not(node ∈ dummy)then empty:seq.T
  ...
  let p = for acc = empty:set.T, @e = toseq.predecessors(g, node)do acc ∪ contains(dummy, @e)end(acc)
...
Function type1conflicts(g:graph.T, dummy:set.T, layers:seq.seq.T)seq.arc.T
 ...
 for acc = empty:seq.arc.T, @e = arithseq(length.layers - 2, 1, 2)do acc + marklayer(g, dummy, layers, @e)end(acc)
...
function crossings(g:graph.T, upperlayer:seq.T, k0:int, k1:int, currentlayer:seq.T, l:int, l1:int)seq.arc.T
 ...
 for acc = empty:seq.arc.T, @e = arithseq(l1 - 1 - (l + 1) + 1, 1, l + 1)do
  acc + w1(g, upperlayer, k0, k1, currentlayer, @e)end(acc)
...
function w1(g:graph.T, upperlayer:seq.T, k0:int, k1:int, currentlayer:seq.T, l0:int)seq.arc.T
 ...
 for acc = empty:seq.arc.T, @e = toseq.predecessors(g, currentlayer_l0)do acc + w1(upperlayer, k0, k1, currentlayer_l0, @e)end(acc)
...
Function findvertarcsUL(g:graph.T, currentlayer:seq.T, lastlayer:seq.T, r:int, x:int, assigned:seq.T)seq.arc.T
 if x > length.currentlayer then empty:seq.arc.T
  ...
  let preds = toseq.predecessors(g, node)
   ...
   let upperidx = for acc = empty:seq.int, @e = preds do acc + findindex(@e, lastlayer)end(acc)
...
Function findvertarcsUL(g:graph.T, layers:seq.seq.T)seq.arc.T
 for acc = empty:seq.arc.T, @e = arithseq(length.layers - 1, 1, 2)do acc + findvertarcsUL(g, layers, @e)end(acc)

Function findvertarcsUR(g:graph.T, layers:seq.seq.T)seq.arc.T
 for acc = empty:seq.arc.T, @e = arithseq(length.layers - 1, 1, 2)do acc + findvertarcsUR(g, layers, @e)end(acc)
...
Function assignvert(RtoL:boolean, layers:set.nodeinfo.T, vertarcs:seq.arc.T, assigned:set.nodeinfo.T, q:nodeinfo.T, x:int, result:seq.nodeinfo.T)set.nodeinfo.T
 ...
 let lastassignedx = if RtoL then for acc = x, @e = toseq.assigned do min(acc, findx(RtoL, q, @e))end(acc)
 else for acc = x, @e = toseq.assigned do max(acc, findx(RtoL, q, @e))end(acc)
 let newq = for acc = empty:seq.nodeinfo.T, @e = vertarcs do acc + findy(q, @e)end(acc)
  if isempty.newq then for acc = assigned, @e = result + q do acc + setx(lastassignedx, @e)end(acc)
...
Function layerarcsR(arcstoroots:set.arc.T, layer:seq.T)seq.arc.T
 for acc = empty:seq.arc.T, @e = arithseq(length.layer - 1,-1, length.layer)do acc + layerarcsR(arcstoroots, layer, @e)end(acc)
...
Function layerarcs(arcstoroots:set.arc.T, layer:seq.T)seq.arc.T
 for acc = empty:seq.arc.T, @e = arithseq(length.layer - 1, 1, 2)do acc + layerarcs(arcstoroots, layer, @e)end(acc)
...
let arcstoroots = asset.for acc = empty:seq.arc.T, @e = toseq.nodes.g3 do acc + isroot(g3, @e)end(acc)
let layerarcs = asset.for acc = toseq.arcstoroots, @e = layers do acc + layerarcsR(arcstoroots, @e)end(acc)
...
let g3 = newgraph.vertarcs
let arcstoroots = asset.for acc = empty:seq.arc.T, @e = toseq.nodes.g3 do acc + isroot(g3, @e)end(acc)
let layerarcs = asset.for acc = toseq.arcstoroots, e2 = layers do acc + layerarcs(arcstoroots, e2)end(acc)
let a = newgraph.toseq.layerarcs
let b = sinksfirst.a + singlenodelayers.layers
...
function singlenodelayers(a:seq.seq.T)seq.T
 for acc = empty:seq.T, @e = a do acc + singlenodelayers.@e end(acc)
...
let m = for acc = 0, @e = toseq.UR do min(acc, x.@e)end(acc)
 asset.for acc = empty:seq.nodeinfo.T, @e = arithseq(cardinality.UL, 1, 1)do acc + merge(UL, UR, m, @e)end(acc)
