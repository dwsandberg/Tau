...
function iis(g:graph.T, dummy:set.T, node:T)seq.T
 ...
 if not(node ∈ dummy)then empty:seq.T
  ...
  let p = for acc = empty:set.T, @e = toseq.predecessors(g, node)do acc ∪ contains(dummy, @e)/for(acc)
...
Function assignvert(RtoL:boolean, layers:set.nodeinfo.T, vertarcs:seq.arc.T, assigned:set.nodeinfo.T, q:nodeinfo.T, x:int, result:seq.nodeinfo.T)set.nodeinfo.T
 ...
 let lastassignedx = if RtoL then
...
let layerarcs = asset.for acc = toseq.arcstoroots, @e = layers do acc + layerarcsR(arcstoroots, @e)/for(acc)
...
let g3 = newgraph.vertarcs
let arcstoroots = asset.for acc = empty:seq.arc.T, @e = toseq.nodes.g3 do acc + isroot(g3, @e)/for(acc)
let layerarcs = asset.for acc = toseq.arcstoroots, e2 = layers do acc + layerarcs(arcstoroots, e2)/for(acc)
let a = newgraph.toseq.layerarcs
let b = sinksfirst.a + singlenodelayers.layers
