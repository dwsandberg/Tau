...
Function subgraph(g:graph.T, nodes:set.T)graph.T
 for acc = newgraph.empty:seq.arc.T, @e = toseq.nodes do acc + subgraph1(g, nodes, @e)end(acc)

function subgraph1(g:graph.T, nodes:set.T, n:T)seq.arc.T
 for acc = empty:seq.arc.T, @e = toseq(successors(g, n) ∩ nodes)do acc + arc(n, @e)end(acc)

Export arc(a:T, b:T)arc.T

Function successors(g:graph.T, n:T)set.T
 for acc = empty:set.T, @e = toseq.findelement2(arcs.g, arc(n, n))do acc + head.@e end(acc)
...
Function predecessors(g:graph.T, n:T)set.T
 for acc = empty:set.T, @e = toseq.findelement2(backarcs.g, arc(n, n))do acc + head.@e end(acc)
...
Function deletearcs(g:graph.T, a:set.arc.T)graph.T
 for acc = g, @e = toseq.a do deletearc(acc, @e)end(acc)
...
Function +(g:graph.T, a:seq.arc.T)graph.T for acc = g, @e = a do acc + @e end(acc)

Function +(g:graph.T, node:T)graph.T graph(arcs.g, backarcs.g, nodes.g + node)

Function reachable(g:graph.T, a:seq.T)set.T let d = asset.a
 reachable(g, d, d, 1)
...
function reachable(g:graph.T, old:set.T, new:set.T, count:int)set.T
 assert count < 1000 report"fal" + toword.cardinality.old + toword.cardinality.new
  if isempty.new then old
   ...
   let a = for acc = empty:set.T, @e = toseq.new do acc ∪ successors(g, @e)end(acc)
   let b = old ∪ new
...
Function sinks(g:graph.T, b:set.T)seq.T
 for acc = empty:seq.T, @e = toseq.nodes.g do acc + sinks(g, b, @e)end(acc)
...
let a = for acc = result, @e = toseq(nodes.g - b)do addnodup(acc, sinks(g, b, @e))end(acc)
...
let a = for acc = result, @e = toseq(nodes.g - b)do addnodup(acc, sources(g, b, @e))end(acc)
