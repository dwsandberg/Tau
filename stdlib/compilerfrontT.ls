...
let prg4=  for  acc=prg.m, s /in toseq.prg.m do 
...
let js=symbol(internalmod,"jsHTTP",constantseq(8,typereal),typereal)
 { let prg6= for  acc=prg5,    s /in toseq.prg5 do 
   if js /in code.s  /and "INLINE"_1 /nin getoption.code.s  then
      addoption(acc,sym.s,"INLINE") else acc
  /for(pass2:T( acc,typedict.m,"addpass") ) }
...
/Function finish:T(m:midpoint, prg5:set.symdef)compileinfo
