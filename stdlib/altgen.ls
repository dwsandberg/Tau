...
Function addfuncdef(info:geninfo6, a:altresult, f:func)altresult 
  let consts = l.a 
  let l = Lcode( emptyinternalbc, consts, empty:seq.localmap, 1, nopara.f + 1, empty:seq.int, empty:seq.Lcode, empty:seq.int, 0)
  ...
  let r = @(processnext.g5, identity, l, prepa(nopara.f, codetree.f))
  altresult(bodies.a + [ BLOCKCOUNT(1, noblocks.r)+ code.r + RET(regno.r + 1, last.args.r)], lst.r)

type Lcode is record   code:internalbc, lst:linklists2, lmap:seq.localmap, noblocks:int, regno:int, args:seq.int, blocks:seq.Lcode, tailphi:seq.int, loopblock:int
...
Function geninfo6(f:func, wordstype:encoding.llvmtype, conststype:encoding.llvmtype, profiletype:encoding.llvmtype, tab:seq.match5, profile:word)geninfo6 
 export

function push(l:Lcode, code:internalbc, regno:int, arg:int)Lcode 
 Lcode( code, lst.l, lmap.l, noblocks.l, regno, args.l + [ arg], blocks.l, tailphi.l, loopblock.l)

function push(l:Lcode, arg:int)Lcode 
 Lcode( code.l, lst.l, lmap.l, noblocks.l, regno.l, args.l + [ arg], blocks.l, tailphi.l, loopblock.l)

function processnext(lib:geninfo6, l:Lcode, m:match5)Lcode 
  let inst = inst.m
  ...
  then let noargs = arg.m 
   let args = top(args.l, noargs)
    if  profile.lib ="noprofile"_1 ∨  profile.lib=inst
   then  
    let c = usetemplate(m, regno.l, empty:seq.int)+ CALLFINISH(regno.l + 1, [ -1]+ args)
    Lcode( code.l + c, lst.l, lmap.l, noblocks.l, regno.l + 1, poppush(args.l, noargs,-(regno.l + 1)), blocks.l, tailphi.l, loopblock.l)
   else // if callee ="PROCESS2"_1 then let discard = if noargs = 1 ∧ nosons(t_1)= 5 then profile(profile.lib, arg.label(t_1_3))else Case of CONST insteand of record as arg 0 addcode(l.sons, c,-(regno.l.sons + 1), 1)else // 
   profilecall(profiletype.lib, l, args, C.[ inst], profile(profile.lib, inst))
  ...
  then let newcode = usetemplate(m, regno.l, args.l)
    // assert inst &ne "CALLIDX"_1 report astext.addtobitstream(10000, empty:bitpackedseq.bit, newcode)  //
   let noargs = arg.m
   Lcode( code.l + newcode, lst.l, lmap.l, noblocks.l, regno.l + length.m, poppush(args.l, noargs,-(regno.l + length.m)), blocks.l, tailphi.l, loopblock.l)
  else if action ="SPECIAL"_1 
  then if inst ="ELSEBLOCK"_1 
   then 
    Lcode( emptyinternalbc, lst.l, lmap.l, noblocks.l + 1, regno.l, empty:seq.int, [ l]+ blocks.l, tailphi.l, loopblock.l)
   else if inst ="THENBLOCK"_1 
   then let newcode = CAST(regno.l + 1, last.args.l, typ.i1, CASTTRUNC)
    let cond = Lcode( code.l + newcode, lst.l, lmap.l, noblocks.l, regno.l + 1, poppush(args.l, 1,-(regno.l + 1)), blocks.l, tailphi.l, loopblock.l)
    Lcode( emptyinternalbc, lst.l, lmap.l, noblocks.l + 1, regno.l + 1, empty:seq.int, [ cond]+ blocks.l, tailphi.l, loopblock.l)
   else if inst ="if"_1 
   then let exp3 = l 
    ...
    Lcode( newcode, lst.l, lmap.l, noblocks.l + 1, regno.exp3 + 1, poppush(args.exp1, 1,-(regno.exp3 + 1)), subseq(blocks.l, 3, length.blocks.l), tailphi.l, loopblock.l)
   else if inst ="DEFINE"_1 
   then Lcode( code.l, lst.l, lmap.l + localmap(toint.instarg, last.args.l), noblocks.l, regno.l, subseq(args.l, 1, length.args.l - 1), [ l]+ blocks.l, tailphi.l, loopblock.l)
   else if inst ="SET"_1 
   then Lcode( code.l, lst.l, lmap(blocks(l)_1), noblocks.l, regno.l, args.l, subseq(blocks.l, 2, length.blocks.l), tailphi.l, loopblock.l)
   ...
   then let varcount = toint.instarg - 1 
    ...
    let k = Lcode( code.l, lst.l, lmap.l, noblocks.l, regno.l, subseq(args.l, 1, length.args.l - varcount - 1), blocks.l, tailphi.l, loopblock.l)
    Lcode( emptyinternalbc, lst.l, bodymap, noblocks.l + 1, regno.l + varcount, [ varcount], [ k]+ blocks.l, tailphi, noblocks.l)
   ...
   then let b = blocks(l)_1 
    ...
    Lcode( code.b + newcode, lst.l, lmap.b, noblocks.l, regno.l, args.b + [ last.args.l], subseq(blocks.l, 2, length.blocks.l), tailphi.b, loopblock.b)
   ...
   then let noargs = toint.instarg 
    ...
    Lcode( code.l + BR(regno.l, loopblock.l), lst.l, lmap.l, block + 1, regno.l, poppush(args.l, noargs, -1), blocks.l, tailphi, loopblock.l)
   ...
   Lcode( value.@(setnextfld, identity, ipair(regno.l + 2, code.l + newcode), args), lst.l, lmap.l, noblocks.l, regno.l + 2 + noargs, poppush(args.l, noargs,-(regno.l + 1)), blocks.l, tailphi.l, loopblock.l)
  else assert action in"CONSTGROUP"report"CHECK"+ action 
      assert inst ="CONSTANT"_1 report"CONSTGROUP"+ inst 
  let tt = addconst(lst.l,fullinst.m) 
  let newcode = GEP(regno.l + 1, 1, typ.conststype, C."list", C64.0, C64(index.tt + 1))+ CAST(regno.l + 2,-(regno.l + 1), typ.i64, 9)
  Lcode( code.l + newcode, value.tt, lmap.l, noblocks.l, regno.l + 2, args.l + [-(regno.l + 2)], blocks.l, tailphi.l, loopblock.l)

exp1 exp2 exp2 FIRSTVAR <firstvar> LOOPBLOCK 4 <loop body> FINISHLOOP 2

/function print(t:tree.seq.word) seq.word  @(+,print,"",sons.t)+label.t
...
Function prepa(nopara:int, t:tree.cnode)seq.match5 
  let inst = inst.label.t 
  if inst ="if"_1 
  then prepa(nopara, t_1)+ lookuptemplate("THENBLOCK 1")+ prepa(nopara, t_2)+ lookuptemplate("ELSEBLOCK 1")+ prepa(nopara, t_3)+ lookuptemplate("if 3")
  else if inst ="SET"_1 
  then prepa(nopara, t_1)+ lookuptemplate("DEFINE"+ arg.label.t)+ prepa(nopara, t_2)+ lookuptemplate([inst, arg.label.t])
  else if inst ="LOOP"_1 
  then @(+, prepa.nopara, empty:seq.match5, subseq(sons.t, 3, nosons.t))+ lookuptemplate("FIRSTVAR"+ arg.label(t_1))+ lookuptemplate("LOOPBLOCK"+toword(nosons.t - 1))+ prepa(nopara, t_2)+ lookuptemplate("FINISHLOOP 2")
  else if inst ="PARA"_1 
  then [ lookuptemplate("PARAM"+ toword(toint.arg.label.t - nopara - 2))]
  else if inst in"LIT LOCAL FREF WORD"
  then [ lookuptemplate([inst, arg.label.t])]
  else if inst ="CRECORD"_1 
  then [lookuptemplate("CONSTANT"+prep3.t)] 
  else @(+, prepa.nopara, empty:seq.match5, sons.t)+ lookuptemplate([inst, toword.nosons.t])
...
function profilecall(profiletype:encoding.llvmtype, l:Lcode, args:seq.int, callee:int, idx:int)Lcode 
 let base = regno.l 
  ...
  Lcode( code.l + c, lst.l, lmap.l, noblocks.l + 3, regno.l + 21, poppush(args.l, length.args,-base - 19), blocks.l, tailphi.l, loopblock.l)
...
Function profilearcs seq.word 
  @(+, callarc,"", mapping.statencoding)
