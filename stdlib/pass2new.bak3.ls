...
Function firstopt(p:program, rep:symbol, code:seq.symbol) program
 ...
 let t=deepcopy.code
 let code2 = code.yyy(p,t,1,empty:seq.symbol, t, 1, nopara + 1, pdict)
 ...
 let a2=code.yyy(p,a,1,empty:seq.symbol,a,1,nopara+1,pdict)
...
function yyy(p:program,org:seq.symbol,k:int,result:seq.symbol, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol)expandresult
 if k > length.org then expandresult(nextvar, s)
 else let sym=org_k
 assert result=subseq(s,1,i-1) report "DIFF"+print.result
  +"&br ----------- &br"+  print.subseq(s,1,i-1)
    +"&br ----------- &br"+print.org
 +stacktrace   
 if module.sym in ["$constant","$fref","$word","$words","int $"] then yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 else
   if  module.sym="local" then 
      let t = lookup(map, (fsig.sym)_1)
      if isempty.t then yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
       else yyy(p,org,k+1,result+t_1, replace(s, i, 1, t_1), i + length.t_1, nextvar, map)
     ...
     if   isbr.s_( condidx+3)   &and isconst(cond) then
        ...
        yyy(p,org,k+1,subseq(result,1,condidx-1)+new, replace(s, condidx, i-condidx+1, new), condidx+length.new, nextvar, map) 
     else    yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
   else  if (fsig.sym)_1="BR"_1   then
       if (fsig.sym)_2="3"_1 &and s_(i-3)=notOp then
          yyy(p,org,k+1,subseq(result,1,i-4)+[s_(i-1),s_(i-2),Br],replace(s,i-3,3,[s_(i-1),s_(i-2)]),i,nextvar,map)
      else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
   else if // isdefine //  (fsig.sym)_1="DEFINE"_1  then
      let thelocal=(fsig.sym)_2
      if i > 1 ∧ (isconst.s_(i - 1) ∨ islocal.s_(i - 1))then
         yyy(p,org,k+1,subseq(result,1,length.result-1), replace(s, i - 1, 2, empty:seq.symbol), i - 1, nextvar, replace(map, thelocal, [ s_(i - 1)]))
       else
         yyy(p,org,k+1,result+  Define.toword.nextvar, replace(s, i, 1, [ Define.toword.nextvar]), i + 1, nextvar + 1, replace(map, thelocal, [ var.nextvar]))
   else if  (fsig.sym)_1="LOOPBLOCK"_1 then
      let nopara = toint.(fsig.sym)_2 - 1
      let firstvar = value.s_(i - 1)
      yyy(p,org,k+1,subseq(result,1,i-2)+Lit.nextvar+sym, replace(s, i - 1, 1, [ Lit.nextvar]), i + 1, nextvar + nopara, addlooplocals(map, firstvar, nextvar, nopara, 0))
   else if (fsig.sym)_1="RECORD"_1 then
      let nopara = toint.(fsig.sym)_2
      let args = subseq(s, i - nopara, i - 1)
      if @(∧, isconst, true, args) then
        yyy(p,org,k+1,subseq(result,1,i - nopara-1)+constant.args, replace(s, i - nopara, nopara + 1, [ constant.args]), i - nopara+1, nextvar, map)
      else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
   else if  (fsig.sym)_1="APPLY"_1 then
       applycode(p,org,k,result, nextvar, s, i,map)
   else if  (module.sym)_1="para"_1   then  
      let sym2=Local.(module.sym)_2
       let t = lookup(map, (fsig.sym2)_1)
       if isempty.t then yyy(p,org,k+1,result+sym2, replace(s,i,1,[sym2]), i + 1, nextvar, map)
        else yyy(p,org,k+1,result+t_1, replace(s, i, 1, t_1), i + length.t_1, nextvar, map)
     else  yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
  else     
    ...
    if not.isempty.dd &and "INLINE"_1 in options.dd then
         ...
         if isempty.code then yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
         else
      inline(p,org,k,result, s, i, nextvar,nopara,code,map)
 else if nopara=0 &or nopara > 2 &or not(isconst.s_(i - 1)) then yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 ...
 " char seq erecord","words"])  &and not(last.module.sym="seq"_1)  
   then yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 else 
  // one or two parameters with last arg being constant //
 if nopara = 1 then optoneop(p,org,k,result, s, i , nextvar, map,sym)
 ...
 &and module.sym="char seq encoding" then
  let arg1 =s_(i - 1)
   ...
   let a1 = @(+, Lit, empty:seq.symbol, tointseq.decodeword.(fsig.arg1)_1)
    let d = constant.([ Lit.0, Lit.length.a1] + a1)  
     yyy(p,org,k+1,subseq(result,1,i-3)+d, replace(s, i - 2, 3, [d]), i + 1- 2, nextvar, map)
   else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
  ...
  let arg1 = s_(i - 1)
   if module.arg1 = "$constant"then
      ...
      yyy(p,org,k+1,subseq(result,1,i-3)+new, replace(s, i - 2, 3, [new]), i - 1, nextvar, map)
   else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 else if not.isconst.s_(i - 2) then  yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 else  
  //  two parameters with   constant  args //
   if module.sym="builtin" then opttwoopbuiltin(p,org,k,result, s, i , nextvar, map,sym)
   else  
    ...
    if module.arg1 = "$words" &and  between(idx, 1, length.fsig.arg1) then
      yyy(p,org,k+1,subseq(result,1,i-3)+Word.(fsig.arg1)_idx, replace(s, i - 2, 3, [ Word.(fsig.arg1)_idx]), i - 1, nextvar, map)
   else if    module.arg1 = "$constant" &and  between(idx, 1, length.constantcode.arg1-2) then
      yyy(p,org,k+1,subseq(result,1,i-3)+(constantcode.arg1)_(idx+2), replace(s, i - 2, 3, [ (constantcode.arg1)_(idx+2)]), i - 1, nextvar, map)
   else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 ...
 let arg1 =  s_(i - 2)
  let arg2 =  s_(i - 1)
   if module.arg1 = "$words" ∧ module.arg2 = "$words"then
     yyy(p,org,k+1,subseq(result,1,i-3)+Words(fsig.arg1 + fsig.arg2), replace(s, i - 2, 3, [ Words(fsig.arg1 + fsig.arg2)]), i - 1, nextvar, map)
   else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 else yyy(p,org,k+1,result+sym, s, i + 1, nextvar, map)
 
function optoneop(p:program,org:seq.symbol,k:int,result:seq.symbol, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)expandresult
 ...
 let arg1 =  s_(i - 1)
   ...
   let  x=Lit.representation.makereal.fsig.arg1 
     yyy(p,org,k+1,subseq(result,1,i-2)+x, replace(s, i - 1, 2, [ x]), i, nextvar, map)
   else yyy(p,org,k+1,result, s, i + 1, nextvar, map)
 else if fsig.rep = "merge(word seq)" &and module.rep="words" then
 let arg1 =  s_(i - 1)
   if module.arg1 = "$words"then
     yyy(p,org,k+1,subseq(result,1,i-2)+Word.merge.fsig.arg1, replace(s, i - 1, 2, [ Word.merge.fsig.arg1 ]), i, nextvar, map)
   else yyy(p,org,k+1,result+rep, s, i + 1, nextvar, map)
else yyy(p,org,k+1,result+rep, s, i + 1, nextvar, map)

function opttwoopbuiltin(p:program,org:seq.symbol,k:int,result:seq.symbol, s:seq.symbol, i:int, nextvar:int, map:worddict.seq.symbol,rep:symbol)expandresult
 if fsig.rep  =" IDXUC(int,int)"   then
    let j = value.s_(i - 1)
    let x =  s_(i - 2)
    if between(j, 0, length.constantcode.x - 1)then
      yyy(p,org,k+1,subseq(result,1,i-3)+[(constantcode.x)_(j + 1)], replace(s, i - 2, 3, [(constantcode.x)_(j + 1)]), i - 1, nextvar, map)
    else yyy(p,org,k+1,result+rep, s, i + 1, nextvar, map)
 else  if fsig.rep = "+(int,int)" then 
   if module.s_(i - 2)="$constant" then // address calculation //
     yyy(p,org,k+1,result+rep, s, i + 1, nextvar, map)
   else 
   let new= replace(s, i - 2, 3, [ Lit(value.s_(i - 2) + value.s_(i - 1))])
   yyy(p,org,k+1,subseq(result,1,i-3)+ Lit(value.s_(i - 2) + value.s_(i - 1)), new,i-1,nextvar,map)
 else if fsig.rep="*(int,int)" then
   let new=replace(s, i - 2, 3, [ Lit(value.s_(i - 2) * value.s_(i - 1))])
    yyy(p,org,k+1,subseq(result,1,i-3)+ Lit(value.s_(i - 2) * value.s_(i - 1)),new,i-1,nextvar,map)
 else if fsig.rep="-(int,int)" then
  ...
  yyy(p,org,k+1,subseq(result,1,i-3)+ if  s_(i - 2) =  s_(i - 1)  then Lit.1 else Lit.0,new,i-1,nextvar,map)
 else if fsig.rep=">(int,int)"  then
   let new=replace(s, i - 2, 3, [ if value.s_(i - 2) > value.s_(i - 1)  then Lit.1 else Lit.0])
  yyy(p,org,k+1,subseq(result,1,i-3)+if value.s_(i - 2) > value.s_(i - 1)  then Lit.1 else Lit.0,new,i-1,nextvar,map)
   else if fsig.rep="∨ (bits, bits)" then
  let new=replace(s, i - 2, 3, [ Lit.toint(bits.value.s_(i - 2) &or bits.value.s_(i - 1))])
  yyy(p,org,k+1,subseq(result,1,i-3)+[ Lit.toint(bits.value.s_(i - 2) &or bits.value.s_(i - 1))],new,i-1,nextvar,map)
  else if fsig.rep="∧ (bits, bits)" then
  let new=replace(s, i - 2, 3, [ Lit.toint(bits.value.s_(i - 2) &and bits.value.s_(i - 1))])
   yyy(p,org,k+1,subseq(result,1,i-3)+[ Lit.toint(bits.value.s_(i - 2) &and bits.value.s_(i - 1))],new,i-1,nextvar,map)
else if fsig.rep="<<(bits, int)" then
  let new=replace(s, i - 2, 3, [ Lit.toint(bits.value.s_(i - 2) << value.s_(i - 1))])
 yyy(p,org,k+1,subseq(result,1,i-3)+[ Lit.toint(bits.value.s_(i - 2) <<  value.s_(i - 1))],new,i-1,nextvar,map)
 else if  fsig.rep=">>(bits, int)"then
  let new=replace(s, i - 2, 3, [ Lit.toint(bits.value.s_(i - 2) >> value.s_(i - 1))])
 yyy(p,org,k+1,subseq(result,1,i-3)+[ Lit.toint(bits.value.s_(i - 2) >>  value.s_(i - 1))],new,i-1,nextvar,map)
  ...
  yyy(p,org,k+1,subseq(result,1,i-3)+[ Lit.representation(casttoreal.value.s_(i - 2) - casttoreal.value.s_(i - 1))], new,i-1,nextvar,map)
  else yyy(p,org,k+1,result+rep, s, i + 1, nextvar, map)
...
function islit(s:symbol) boolean   module.s ="int $" 
 ...
 function inline(p:program,org:seq.symbol,k:int,result:seq.symbol, s:seq.symbol, i:int, nextvar:int,nopara:int,code:seq.symbol, map:worddict.seq.symbol)
  expandresult
      if length.code = 1 ∧ code = [ var.1]then
   // function just returns result // 
       yyy(p,org,k+1,result,replace(s, i, 1, empty:seq.symbol),i,nextvar,map)
    else
    let t = backparse(s, i - 1, nopara, empty:seq.int) + i
     ...
     let new = if issimple(p,nopara,code) then 
         ...
         code.yyy(p,code,1,empty:seq.symbol, code, 1, nextvar, pmap)
    else 
          expandinline(s, t, emptyworddict:worddict.seq.symbol, nopara, empty:seq.symbol, nextvar, code, p)
        yyy(p,org,k+1,subseq(result,1,t_1-1)+new, replace(s, t_1, i - t_1 + 1, new),t_1 + length.new,nextvar + nopara,map)
...
function expandinline(s:seq.symbol, t:seq.int, pmap: worddict.seq.symbol, i:int, newcode:seq.symbol, nextvar:int, inlinecode:seq.symbol, p:program)seq.symbol
 ...
 if i = 0 then newcode+code.yyy(p,inlinecode,1,empty:seq.symbol, inlinecode, 1, nextvar, pmap)
 else
...
function applycode(p:program,org:seq.symbol,k:int,result:seq.symbol, nextvar:int, code:seq.symbol, index:int,map:worddict.seq.symbol)  expandresult
 ...
 ∧ not((fsig.term2_1)_1 = "deepcopy"_1)
  ...
  // assert not(subseq(code, t_2, index - 4)=[var.1]) report "XXXX"+print.code+"/new/"+print.new //
     ...
     yyy(p,org,k+1,subseq(result, 1, t_1 - 1) + subseq(result, t_2, index - 4), a,length.new + 1,nextvar,map)
  else
   let paras = adddefines2(code, t + (index - 3), 1, nopara1 + nopara2 + 2, empty:seq.symbol, nextvar)
   let a=applytemplate
   let body = yyy(p,a,1,empty:seq.symbol, a, 1, nextvar + nopara1 + nopara2 + 2, map5)
...
yyy(p,org,k+1,subseq(result,1,t_1-1)+new, replace(code, t_1, index - t_1 + 1, new), t_1 + length.new, nextvar.body, map)
