...
function frombits(a:bits,null.T)T unbound
...
Function_(a:bitpackedseq.T, idx:int)T 
 let nobits = sizeinbits.frombits(bits.0 ,null:null.T)
  ...
  frombits(d >> slot ∧ bits(-1)>> 64 - nobits,null:null.T)
...
Function add(a:bitpackedseq.T, b:T)bitpackedseq.T 
 let nobits = sizeinbits.b 
  ...
  then bitpackedseq(length.a + 1, data.a + frombits(newpart,null:null.T), bits.0)
...
Function align(a:bitpackedseq.T)bitpackedseq.T 
 let nobits = sizeinbits.frombits(bits.0 ,null:null.T)
  let slotsperword = 64 / nobits 
  let slot = length.a mod slotsperword 
  if slot = 0 then a else align.add(a, frombits(bits.0 ,null:null.T))
...
Function patch(a:bitpackedseq.T, j:int, val:int)bitpackedseq.T 
 assert(j - 1)mod 32 = 0 report"incorrect parameter to replace"
  ...
  else bitpackedseq(length.a, subseq(data.a, 1, i - 1)+ frombits(x,null:null.T) + subseq(data.a, i + 1, length.data.a), part.a)
...
Function add(a:bitpackedseq.T, b:bits, count:int)bitpackedseq.T 
 let nobits = sizeinbits.frombits(bits.0 ,null:null.T) 
  ...
  let d = if slot + slotstoadd = slotsperword 
   then bitpackedseq(length.a + slotstoadd, data.a + frombits(newpart,null:null.T), bits.0)
