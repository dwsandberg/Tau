...
Function patch(a:bitpackedseq.T, j:int, val:int)bitpackedseq.T
 ...
 b ∧ bits(toint(bits.1 << 32) - 1) << 32 ∨ bits.val
 else b ∧ bits(toint(bits.1 << 32) - 1) ∨ bits.val << 32
  // assert false report"PP"+ toword.toint.x //
  if i > length.data.a then bitpackedseq(length.a, data.a, x)
  else
   bitpackedseq(length.a, subseq(data.a, 1, i - 1) + frombits:T(x)
   + subseq(data.a, i + 1, length.data.a), part.a)
