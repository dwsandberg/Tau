...
Function compilerfront(option:seq.word, libname:seq.word, allsrc:seq.seq.word, dependentlibs:seq.word, exports:seq.word)compileinfo
 ...
 let libinfo = libmodules2.dependentlibs
  { assert isempty.mods.libinfo report for txt ="testingx", sd = prg.libinfo do if name.module.sym.sd /in"llvm"then txt + print.sym.sd + print.code.sd + EOL else txt /for(txt)}
  ...
  { assert isempty.libpasstypes report for txt ="types", t = types.libinfo do txt + print.first.t /for(txt)+"passtypes"+ for txt ="", p = toseq.libpasstypes do txt + print.p /for(txt)}
   let mode = if option = "text"then"text"_1 else"body"_1
    ...
    { figure out how to interpret text form of symbol }
     ...
     { assert false report for libs = empty:seq.word, p = toseq.modules.t5 do libs + library.modname.p + name.modname.p + EOL /for(libs)}
      ...
      if mode = "text"_1 then
       ...
       compileinfo(zz1, emptytypedict, empty:seq.libraryModule, allsrc, empty:set.symbol)
        ...
        let templates = for acc = empty:seq.symdef, p = toseq.prg10 do
        ...
        { let z = symbol(moduleref("stdlib standard"),"<", typeint, typeint, typeboolean)let c1 = getCode(prg.pb, z)let c2 = getCode(result, z)}
         { assert libname /ne"stdlib.testoptconfig"report"XXX"+ for txt ="", Z = toseq.result do txt + print.sym.Z + print.subseq(code.Z, 1, 10)+"ENDX"+ EOL /for(txt)}
         compileinfo(toseq.if option = "pass1"then result else pass2.result ∪ templates, typedict.pb, tolibraryModules(typedict, toseq.modules.t5, exports), empty:seq.seq.word, asset.roots)

Export type:compileinfo

type compileinfo is typedict:typedict, code:seq.seq.symbolref, src:seq.seq.word, symbolrefdecode:seq.symbol, mods:seq.libraryModule, Xroots:seq.symbolref
...
let symdecode = symbolrefdecode.s
 ...
 acc4
  + symdef(sym, for acc = empty:seq.symbol, r = c << 2 do acc + symdecode_(toint.r)/for(acc))
 /for(acc4)
 ...
 if i > length.x then in
  ...
  if isempty.f ∨ isempty.code.f_1 then acc
    ...
    + for acc2 = [ symbolref.data.p, symbolref.Lit.paragraphno.f_1], sym = code.f_1 do
     acc2
     + if isFref.sym then let discard = symbolref.basesym.sym
      symbolref.sym
...
function externalname(sym:symbol, ll:liblib, prg:set.symdef, idx:int)seq.word
 if library.module.sym = (libname.ll)_1 then
  [ merge(libname.ll + "$$" + toword.idx)]
...
let code = for acc2 = empty:seq.symbol, r = c << 1 do acc2 + (decoderef.ll)_(toint.r)/for(if isabstract.module.sym then acc2
else let externalname = externalname(sym, ll, orgprg, toint.c_1)
 ...
 for acc = empty:seq.libraryModule, typedec = empty:seq.seq.mytype, m2 = t5 do
  if name.module.m2 ∉ exports then next(acc, typedec)
   ...
   let exps = for acc3 = empty:seq.symbolref, e = toseq.d2 do
    if isunbound.e then acc3 else acc3 + symbolref.e
   /for(acc3)
