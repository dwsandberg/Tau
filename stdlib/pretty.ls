...
let totwidth = for acc = 0, @e = toseq.a do acc + width.@e /for(acc)
let noperline = if totwidth < 30 then 10000
else if for acc = 0, @e = toseq.a do max(acc, width.@e)/for(acc)
< 30 * 10 then
 10
else 1
 attribute2.[ prettyresult(0, if totwidth ≥ 30 then 10000 else totwidth, for acc ="", i = 1, e = toseq.a do
  next(acc + removeclose.text.e
  + if i = noperline then" /br,"else",", if i = noperline then 1 else i + 1)
 /for(acc >> 1))
 ]
...
let a = if prec.post > prec ∨ prec ≠ 3 ∧ prec = prec.post then
 prettyresult(prec, width.pre + width.x + width.post, pre1
 + if binary = "."then"("else x + "("/if
 + text.post
 + ")")
else
...
let exp =(toseq.a)_1
 attribute2.[ if width.exp < 30 then prettyresult(0, width.exp," /br  /keyword else" + text.exp)
  ...
  { assert"if"_1 ∉ text.exp report"here"+ text.exp }
