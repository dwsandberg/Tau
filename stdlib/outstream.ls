...
for nospace = false, result = x, stk = empty:stack.seq.word, last = none, this ∈ a + space do
 if last = match then
  if this = " /<"_1 then
   ...
   next(findindex(this, specialwords) < length.specialwords
   , addspace(nospace, result, this)
   , stk
   , last
   )
...
for needsLF = false, nospace = false, result = x, stk = empty:stack.word, last = none, this ∈ a + space do
 ...
 if last = match then
  if this = " /<"_1 then
   ...
   next(needsLF
   , findindex(this, specialwords) < length.specialwords
   , addspace(nospace, result, this)
   , stk
   , last
   )
  ...
  else if this = "noformat"_1 then next(needsLF, nospace, result, stk, match)
  else next(needsLF, nospace, result, push(stk, space), none)
 else if last = " />"_1 ∧ not.isempty.stk then next(needsLF, nospace, result, pop.stk, this)
 else if last ∈ " /keyword  /em  /strong"then next(needsLF, nospace, result, stk, this)
 else if last = " /p"_1 then
  next(true, true, addspace(nospace, result," /br  /br"), stk, this)
 else if last = space then next(needsLF, true, addspace(nospace, result, space), stk, this)
 else
  let i = findindex(last, specialwords)
  next(true, i < length.specialwords, addspace2(nospace, result, last, false, i), stk, this)
/for(if last = none then result else addspace(nospace, result, last)/if) 