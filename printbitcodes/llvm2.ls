...
toint(tobits.toint.(sbit.a)_((i - 1) / 8 + 1) >> ((i - 1) mod 8) ∧ 0x1)
...
first.toseqbits.subseq(sbit.a, bidx, bidx + (size + 15) / 8) >> ((idx - 1) mod 8)
∧ mask
...
if i = size - 1 then
 if b = 0 then
  pp(idx + size, toint.val)
 else
  getvbr(a, size, val, nobits, idx + size, 0)
else
 getvbr(a, size, bits.b << nobits ∨ val, nobits + 1, idx, i + 1)
...
if length.r > 0 then
 ...
 if noargs = 0 then
  ...
  let next = 
   if blockid = toint.CONSTANTS ∧ r_1 = toint.CINTEGER then
 ...
 {assert not (blockid = toint.CONSTANTS) report" KK"+toword.length.orderadded.blockabbre+@ (seperator
  ."
  /br", printabbr,"", abbrdef)}
 let t = {getvbr (b, abbrvlen, bits.0, 0, idx, 0)} getfixed(b, idx, abbrvlen),
 {assert val.t = val.getvbr (b, abbrvlen, bits.0, 0, idx, 0) report" XX"+toword.val.t+toword.abbrvlen
  +toword.val.getvbr (b, abbrvlen, bits.0, 0, idx, 0)}
 if val.t = 3 then
  {record}
  ...
  {assert not (val.newblockid = 0) report" abbrlen"+toword.val.abbrlen+" bits"+@ (+, toword,
   "", @ (+, toint, empty:seq.int, @ (+,_.b, empty:SBIT, arithseq (val.len * 32, 1, idx.len))))}
  let subblock = block(b, idx.len, val.len, val.abbrlen, val.newblockid),
  {let discard = if blockid.subblock = 0 then processabbr (recs.getinfo.subblock, 1, 0, empty:seq.seq
   .int) else 0}
  getinfo(b
   , 0
   , empty:seq.int
   , finish
   , recs + [-1, val.newblockid, length.blocks + 1, val.abbrlen, val.len, idx.len, finish]
   , blocks + subblock
   , abbrvlen
   , blockid
   , abbrdef)
  ...
  getinfo(b, 0, empty:seq.int, ops_1, recs + rec
   , blocks, abbrvlen, blockid, abbrdef + rec)
 else
  let rec = readabbrrecord(abbrdef_(val.t - 3), 2, empty:seq.int, b, idx.t),
  getinfo(b
   , 0
   , empty:seq.int
   , rec_1
   , recs + subseq(rec, 2, length.rec)
   , blocks
   , abbrvlen
   , blockid
   , abbrdef)
...
if len > 0 then
 ...
 subseq(s,-len + 1, length.s)

function readabbrrecord(def:seq.int, i:int, result:seq.int, b:SBIT, idx:int) seq.int
if i > length.def then
 [idx] + result
else
 let code = def_i,
 if code = ABBROPFixed then
  let arg = getfixed(b, idx, def_(i + 1)),
  readabbrrecord(def, i + 2, result + val.arg, b, idx.arg)
 else if code = 0 then
  readabbrrecord(def, i + 2, result + def_(i + 1), b, idx)
 else if code = ABBROPArray ∧ def_(i + 1) = ABBROPChar6 then
  let len = getvbr(b, idx, 6)
  let rec = readarrayfixed(6, b, idx.len, val.len, empty:seq.int),
  {assert false report" HEREH"+@ (+, toword,"", rec)}
  readabbrrecord(def, i + 2, result + subseq(rec, 2, length.rec), b, rec_1)
 else if code = ABBROPArray ∧ def_(i + 1) = ABBROPFixed then
  let len = getvbr(b, idx, 6)
  let rec = readarrayfixed(def_(i + 2), b, idx.len, val.len, empty:seq.int),
  {assert false report" HEREH"+@ (+, toword,"", rec)}
  readabbrrecord(def, i + 3, result + subseq(rec, 2, length.rec), b, rec_1)
 else
  assert code = ABBROPVBR
  report
   "not imp abbr" + toword.code + toword.def_(i + 1) + "idx" + toword.idx
   + printabbr.def
  let arg = getvbr(b, idx, def_(i + 1)),
  readabbrrecord(def, i + 2, result + val.arg, b, idx.arg)
