...
{ENTRYPOINT Print frequency of useage in bitcode file}
for content = empty:seq.content, @e ∈ findsubblock(toint.FUNCTIONBLK, blocks.getcontent.1_input)
...
{ENTRYPOINT Decode a a bitcode file generated by the compiler into a more readable format./br flags
...
let z = getcontent.1_input
{let z2 = getinfo.(blocks.z)_5 @ (seperator." /br", printrecord.toint.MODULE,"", recs.z)}
let functionblocks = findsubblock(toint.FUNCTIONBLK, blocks.z)
let typeblock =
 if blockid.2_blocks.z = toint.TYPES then
 2
 else assert blockid.1_blocks.z = toint.TYPES report "Expected TYPE Table as first block", 1
let types =
 for acc = empty:seq.seq.int, @e ∈ recs.getinfoB.typeblock_blocks.z do acc + removeabbr.@e,
 acc
let q1 =
...
let names = for acc = sparseseq.1_"undefinedname", @e ∈ symbols do processsymentry(acc, @e), acc
let slots2 = slotorder2(z, 4, empty:seq.slotdesc)
let checkslots = number.check(slots2, q1)
assert 1_"ERROR" ∉ checkslots
...
let str1 =
 ...
 "Module data /p use standard
  ...
  /p use file
  /p use seq.file
  /p Function test2 (input:seq.file, o:seq.word) seq.file {ENTRYPOINT}
  ...
  /br let bc = llvm ([^(
   for acc = "", @e ∈ subseq(recs.getinfoB.typeblock_blocks.z, 2, 3)
   do acc + printrecord(TYPES, @e) + ",",
   acc >> 1)]+subseq (typerecords, 3, length.typerecords), a)
...
function removeabbr(s:seq.int) seq.seq.int if 1_s = -2 then empty:seq.seq.int else [s]
...
do
 let rec = rec.sl,
  if typ.sl ≠ -1 ∨ 1_rec ≠ toint.FUNCTIONDEC ∨ {external function} 4_rec = 1 then
  next(result, codeblock, slotno + 1)
  else
   {MODULECODEFUNCTION}
   let typ = (2_rec + 1)_types
   let nopara = for acc = 0, e ∈ typ do if e ∈ "," then acc + 1 else acc, acc
   let offset = n.consts - 1
   let inst = recs.getinfoB.codeblock_blocks
...
do
 let tp = instop.1_d,
  ...
  {skip BLOCKCOUNT} next(slot, result, inblock)
   ...
   let reglabel = if slotinc = 1 then if llvm then [merge.[1_"%", ll]] else "r." + ll else ""
   let pattern =
    if tp = BINOP then
     let op1 = relocate(info, slot, 2_d)
     let op2 = relocate(info, slot, 3_d),
     "binaryop (^(reglabel),^(decode.binaryop.4_d),^(op1),^(op2))"
    else if tp = CMP2 then
     let op1 = relocate(info, slot, 2_d)
     let op2 = relocate(info, slot, 3_d),
     "cmp2 (^(reglabel),^(decode.cmp2op.4_d),^(op1),^(op2))"
    else if tp = CAST then
     let op1 = relocate(info, slot, 2_d),
      if llvm then
      "^(reglabel) =^(decode.castop.4_d),^(op1),^(typ(info, 3_d))"
      else "cast (^(reglabel),^(decode.castop.4_d),^(op1),^(typ(info, 3_d)))"
    else if tp ∈ [LOAD] then
     let op1 = relocate(info, slot, 2_d),
     "load (^(reglabel),^(op1),^(typ(info, 3_d)),^(decode.align.4_d))"
    else if tp = RET then
    if n.d = 1 then "ret" else "ret (^(relocate(info, slot, 2_d)))"
    else if tp ∈ [STORE] then
     let op1 = relocate(info, slot, 2_d)
     let op2 = relocate(info, slot, 3_d),
     "store (^(op1),^(op2),^(decode.align.4_d))"
    else if tp = GEP then
    "getelementptr (^(reglabel),^((3_d + 1)_types.info),^(for acc = "", @e ∈ subseq(d, 4, n.d) do acc + relocate(info, slot, @e) + ",", acc >> 1))"
    else if tp = CALL then
     let fconst = relocate(info, slot, 5_d),
      if llvm then
       let tt = break(1_",", (4_d + 1)_types.info >> 1)
       let args =
        for acc = "", types = tt << 1, @e ∈ subseq(d, 6, n.d)
        do next(acc + 1_types + relocate(info, slot, @e) + ",", types << 1),
        acc >> 1
       ,
       "^(reglabel) = call^(1_tt << 3)^(fconst) (^(if n.args = 0 then ")" else "^(args))")"
      else
       let args = for acc = "", @e ∈ subseq(d, 6, n.d) do acc + relocate(info, slot, @e) + ",", acc >> 1,
       "call (^(reglabel),^((4_d + 1)_types.info),^(fconst)^(if n.args = 0 then ")" else ", [^(args)])")"
    else if tp = PHI then
     "phi (^(reglabel),^((2_d + 1)_types.info)^(phi2(info, slot, d, 3, ""))"
     + ")"
    else if tp = ALLOCA then
     "alloca (^(reglabel),^((2_d + 1)_types.info),^((3_d + 1)_types.info)"
     + ",^(relocate(info, slot, 4_d)))"
    else empty:seq.word
   let inst =
    ...
    pattern
     ...
     if n.d = 4 then
      let op1 = relocate(info, slot, 4_d),
       if between(2_d, 1, n.blocks.info) ∧ between(3_d, 1, n.blocks.info) then
        "br (^(op1),"
        + toword.(2_d + 1)_blocks.info
        + ","
        + toword.(3_d + 1)_blocks.info
        + ")"
       else "branch problem"
     else if between(2_d, 1, n.blocks.info) then
     "br.^(
       if between(2_d, 1, n.blocks.info) then
       [toword.(2_d + 1)_blocks.info]
       else "{" + toword.2_d + "} 0" + "+/br label." + ll)"
     else "branch problem" + toword(2_d + 1)
    else reglabel + decode.instop.1_d + for acc = "", @e ∈ d do acc + toword.@e, acc
   ,
   next(
    slot + slotinc
...
result
 ...
 let blklab = toword.((i + 1)_d + 1)_blocks.info
 let x = i_d
 let y = x / 2
...
if {" ll"_1 in format.info} true then
 if slot - arg ≥ offset then
  ...
  if llvm.info then [merge.[1_"%", toword.x]] else "r.^(x)"
...
function typ(info:info, arg:int) seq.word (arg + 1)_types.info
...
result
 ...
 let i = j_recs.z,
  ...
  slotorder2(z, j + 1, result)
   ...
   {not a sub block}
   if 1_i ∈ [toint.FUNCTIONDEC, toint.GLOBALVAR] then
   slotorder2(z, j + 1, result + slotdesc(-1, i))
   else slotorder2(z, j + 1, result)
  else
   let blocks = blocks.z,
    if blockid.(3_i)_blocks = toint.CONSTANTS then
     let recs = recs.getinfoB.(3_i)_blocks,
...
result
 ...
 let a =
  result
  + (if i > n.labels then prefix + i_items else if i > 1 then "," else "")
  + prefix
  + "{"
  + i_labels
  + "}"
...
if isempty.list then 1 else let discard = encode.blockabbr(blockid, list), 1
 ...
 let ai = i_a,
  if 1_ai = SETBID then
   ...
   processabbr(a, i + 1, 2_ai, list)
   else
    let discard = encode.blockabbr(blockid, list),
    processabbr(a, i + 1, 2_ai, empty:seq.seq.int)
  else if 1_ai = -2 then
  processabbr(a, i + 1, blockid, list + i_a)
...
replaceS(s, w, [codefreq(count.w_s + 1, w)])
...
function count(s:seq.codefreq, w:seq.int) seq.codefreq count(s, 1_w)
