 ...
 result + printrecord(MODULE, rec.k)
  ...
  let z = printrecord(CONSTANTS, rec.k),
   if 1#rec = toint.CAGGREGATE then
    ...
    if
     (2#rec = 9 ∧ typ.k = 0 ∨ 2#rec = 11 ∧ (typ.k + 1)#types = "double")
      ∧ findtype(slots, 4#rec) ∈ [3#rec,-1]
    then
    result + z
    else
     result
      + (z + "ERROR" + ((typ.k + 1)#types + %.findtype(slots, 4#rec) + %.3#rec))
   else if 1#rec.k = toint.CINTEGER then
    if (typ.k + 1)#types ∈ ["i64", "i32"] then
    result + z
  ...
  if typ.sd = -1 then
   ...
   let new =
    if 1#a = toint.GLOBALVAR then
     ...
     let init =
      if 4#a = 0 then
      "0,"
      else if 4#a - 1 > n.result then
      "toint.slot (" + toword(4#a - 1) + ")+1,"
   ...
   let b =
    if 1#a = toint.CINTEGER then
     ...
     {if a#1 = AGGREGATE then let eletype = subseq (currenttype, 4, n.currenttype-1)" ["+@
      ...
      if 1#a = toint.CCAST ∧ castop.2#a = ptrtoint then
       let objno = checkobjref(s, types, sd),
        ...
        "obj.^(objno)"
         ...
         report "not expecting type in prttoint^(currenttype)^((3#a + 1)#types)",
         "ptrtoint (^(lookupconst(result, 4#a)),^((3#a + 1)#types)
         ^(if check then ",^(printrecord(CONSTANTS, a)))" else ")")"
      else if 1#a = toint.CGEP ∧ n.a = 8 ∧ check then
      "CGEP (^(if 2#a = 1 then "conststype" else (3#a + 1)#types),
      ^(
       ...
       assert constop.1#a ∈ [CAGGREGATE, CNULL, CDATA, CCAST]
       report "KL^(decode.constop.1#a)" + toword.1#a,
 ...
 let gep = rec.(4#a + 1)#slots,
  if
   1#gep = toint.CGEP
    ...
    ∧ 1#rec.(8#gep + 1)#slots = toint.CINTEGER
  then
  2#rec.(8#gep + 1)#slots
  else -1
else -1
