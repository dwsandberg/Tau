...
Function cindex(s:seq.T, i:int, idx:int)T 
 if idx = 1 then if s_i < 128 then s_i else s_(i + 1) - 128 +(s_i - 128 - 64)* 64 else cindex(s, i + if s_i < 128 then 1 else 2, idx - 1)

We need a constructor of our sequence.Note the use of a toseq function.This is defined implicitly by the sequence 
 type definition to change the type from myseq(T)to seq(T).
