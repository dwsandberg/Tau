...
Function test11a seq.word 
 // testcomp2.["module testit","use stdlib","function f1(a:int)boolean [ a +"]// 
 let z = [ compare("a + b + c","{(a + b)+ c }")
  ...
  // 11 // 
  testerror("ERR parse error expect:]), > got:end of paragraph", ["function f1(a:int)boolean [ a"])
  , testerror("ERR parse error expect:N(I @ W A $wordlist assert { E > comment if [ let got:end of paragraph"
  , ["function f1(a:int)boolean [ a +"])
  , testerror("ERR Function f1 is defined twice in module testit"
