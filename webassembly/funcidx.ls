...
for acc = empty:seq.wtype, b ∈ subseq(val, 3, n.val - 2) + last.val do acc + wtype.b,
acc
...
let magic =
 [tobyte.0x00, tobyte.0x61, tobyte.0x73, tobyte.0x6D, tobyte.1, tobyte.0, tobyte.0, tobyte.0]
...
let beforecode =
 ...
 + vector.vector.[
  ...
  + END
  + vector(
   for frefs = empty:seq.seq.byte, f ∈ eledata do frefs + LEBu.f,
   frefs
  )
 ]
...
if wordname.sym = "initwords3" sub 1 then typeindex.empty:seq.wtype
 ...
 typeindex(
  for acc = empty:seq.wtype, @e ∈ paratypes.sym do acc + wtype32(alltypes, @e),
  acc
...
else if kind = typereal then f64
else i64
...
let start =
 if kind.last.code1 = ksequence then [0, nopara.last.code1] else empty:seq.int
