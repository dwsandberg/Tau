   ...
   let newstack =
    ...
    stk
     ...
     stk
      ...
      report "type problem else^(toseq.stk)+^(top.blkstk)"
      ,
      pop.stk
    ...
    pop.stk
     ...
     stk
      ...
      report "type problem end^(toseq.stk)
       /br^(toseq.blkstk)
       /br^(text)"
      ,
      stk
  ...
  let c = tobits.byte ∧ 0x7F,
   if c = tobits.byte then
    let arg =
     ...
     toint.
      if (tobits.byte ∧ 0x40) = 0x0 ∨ op ∉ [i32const, i64const] then
      tobits.byte
      else c ∨ tobits.-1 << 7
    ...
    let newstk =
     ...
     stk
      ...
      assert top(stk, n.xx - 4) = subseq(xx, 3, n.xx - 2)
      report "types nomatch call stk:^(top(stk, n.xx - 4)) xx^(xx)
       /br^(text)"
      ,
...
report "type problem^(decodeop.op) stk: ^(toseq.stk)
 /br^(text)",
