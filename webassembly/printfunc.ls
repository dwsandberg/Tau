  ...
  if c = tobits.byte then
   ...
   let newstk = 
    if nocheck then stk
     ...
     report "type problem localset" + toword.arg + "locals" + locals + "/br"
     + text
     pop.stk
    ...
    ∈ [i32store, i64store, f64store, i64load, i64load8u, i32load, f64load, i32const, i64const] then
     newstack(op, stk, text)
     ...
     assert top(stk, 1) = "i32"
     report "XXK" + toword.arg + toseq.stk + "/br" + text
     pop.stk
    else if op = br then
     ...
     if blktype = "void"_1 then stk
      ...
      report "JK" + blktype + top.stk + decodeop.lastop + toword.arg + "/br"
      + toseq.blkstk
      + "/br"
      + text
      pop.stk
     ...
     assert top.stk = locals_(arg + 1)
     report "type problem localtee" + toword.arg + "locals" + locals + "/br"
     + text
     stk
...
report "type problem $(decodeop.op) $(toseq.stk) /br $(text)"
if isempty.d_2 then pop(stk, length.d_1)
else push(pop(stk, length.d_1), d_2_1) 