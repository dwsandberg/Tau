...
let a = for acc = empty:set.seq.word, f ∈ knownfuncs do acc + %.sym.f /do acc
let b = for acc = empty:set.seq.word, p ∈ encodingdata:coverage do acc + towfunc.p /do acc,
for txt = "", l ∈ toseq(a \ b) do txt + l + "/br" /do txt
...
let elements = 
 ...
 /do topackedbyteobject.outbytes:addrsym(acc)
let discardff = 
 addf(alltypes
...
createwasm(imp, exp + exportmemory."memory"_1, dataseg, startfuncidx, filename.o, info)

function topackedbyteobject(x:seq.byte) seq.int
for acc = [1, length.x], acc2 = 0x0, shift = 0, b ∈ x do
...
do
 ...
 ∨ sym = symbol(internalmod, "set", typeptr, typeptr, typeptr) then
  ...
  report
   "Exit type problem STK:$(for l = "", e ∈ toseq.typestk do l + %.e /do l)
    /br $(printcode.curblk)
    /br $(code)"
  ,
  next(last, sym, pop.typestk, push(blkstk, blkele(curblk, sym)), empty:seq.byte, localtypes)
  ...
  ,
  next(last, sym, typestk, pop(blkstk, length.blks), newcode, localtypes)
 ...
 else if wordname.sym = "createthreadZ"_1 ∧ inmodule(sym, "builtin") then
  let psym = 
  ...
  next(last
   ...
   , push(pop(typestk, nopara.sym), i64)
   , blkstk
   , curblk + Wcall.recordsym(alltypes, sym) + f64converti64s + const32.tableindex.sym2
   + f64converti32s
  ...
  let paratypes = 
   for acc = empty:seq.wtype, @e ∈ paratypes.sym do acc + wtype64(alltypes, @e) /do acc
  assert paratypes = top(typestk, length.paratypes)
  report
   "type missmatch $(sym)
    $(for acc = "", @e ∈ top(typestk, length.paratypes) do acc + %.@e /do acc) /
    $(for acc = "", @e ∈ paratypes do acc + %.@e /do acc)
    /br $(for acc = "", @e ∈ code do acc + %.@e /do acc)"
  ...
  next(last
   , sym
   , push(pop(typestk, length.paratypes), wtype64(alltypes, resulttype.sym))
   , blkstk
   , curblk + this
   , localtypes)
/do
 assert not.isempty.typestk report "generatefuncbody:did not expect empty stack $(code) $(toseq.typestk)"
 let zk1 = for acc = empty:seq.localinfo, e ∈ toseq.localtypes do acc + localinfo.e /do acc
 let zk = for acc = empty:seq.wtype, e ∈ sort.zk1 do acc + type.e /do acc,
...
let funccall = 
 ...
 /do code + struct + i32load + tobyte.2 + LEBu.16 + Wcallindirect.functypeidx
,
funcbody([i64]
 , if rt = i64 then
  funccall
