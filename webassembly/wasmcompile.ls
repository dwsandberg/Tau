...
let a =
 for acc = empty:set.seq.word, f ∈ knownfuncs
 do acc + %.sym.f,
 acc
let b =
 for acc = empty:set.seq.word, p ∈ encodingdata:coverage
 do acc + towfunc.p,
 acc
...
let exp =
 for acc = empty:seq.seq.byte, @e ∈ roots
 do acc + exportfunc(funcidx.@e, 1^jsname.@e),
 acc
...
createwasm(imp, exp + exportmemory.1#"memory", dataseg, startfuncidx, outname, info)
    ...
    if isempty.ele ∧ module.sym = internalmod ∧ name.sym ∉ "set" then
     ...
     let bodycode =
      if isempty.ele then const64.0
       ...
       for code2 = empty:seq.byte, j ∈ arithseq(nopara.sym, 1, 0)
       do code2 + Wlocal.j,
       code2 + code.1#ele,
...
do
 ...
 ∨ sym = symbol(internalmod, "set", typeptr, typeptr, typeptr) then
  ...
  else if sym = Littrue then next(last, sym, push(typestk, i32), blkstk, curblk + const32.1, localtypes)
  else if sym = Litfalse then next(last, sym, push(typestk, i32), blkstk, curblk + const32.0, localtypes)
 ...
 else if wordname.sym = 1#"allocatespace" then next(last, sym, typestk, blkstk, curblk + Wcall.allocatesym + i64extendi32u, localtypes)
  ...
  let blockcode =
   ...
   do
    let z =
     if isExit.sym.a then code.a + brX(n.blks - i - (if isloop then 0 else 1))
     else if iscontinue.sym.a then code.a + setloopvar + br + LEBu(n.blks - 1 - i)
     else
  ...
  let psym =
   basesym(
    if isFref.lastfref then lastfref
    else
  ...
  assert paratypes = top(typestk, n.paratypes) report
   "type missmatch^(sym)^(for acc = "", @e ∈ top(typestk, n.paratypes)
   do acc + %.@e,
...
let zk1 =
 for acc = empty:seq.localinfo, e ∈ toseq.localtypes
 do acc + localinfo.e,
 acc
let zk =
