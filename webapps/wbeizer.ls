...
/em grp1 is a group that contains circles that represent the control points./br
/em grp1 also has a attribute data-segments that contains the number of biezer curves in path./br
/em lines is a path that conects the control points./br
/em curve is a path that draw the biezer curves.
...
if c sub 2 = "." sub 1 then [subseq(c, 1, 3), c << 3]
else [[c sub 1], c << 1]
...
function att(s:seq.word, value:seq.word) seq.word
s + "=" + dq + "/nsp" + value + dq + "/sp"
