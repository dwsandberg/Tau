...
The file webcmd.html contains a small html fragment  for this example. The lines of the body are:
<* block <p id = status> loading<p>
/br <input id = cmdline value ="docsource+built core.libsrc " >
...
*>
 ...
 The first p element will be for a status line ,
 the input element will be to enter a command, and 
 the table element will be a history of the commands ran with links to the results.  
 The final p element will be used to form the name of the next cmd output results. These compoents of the HTML file will be referenced by id from the file webcmd.ls which contains the Tau code. 
 
 
The line in the build file specifies the pieces needed for this example:
<* block  wasm webcore.libsrc tausupport core symbols compilerfront tools
...
<* block 
  /em webcmd is called when the page is loaded.  The name of the callback(webcmd) matches the name of the page(webcmd). 
/br /em AddHistory is called after reading the history file when the page is loaded.
/br  /em runcmd    is called when the run button is pressed.  The link between runcmd is specified
 with onclick ="runcmd ()" in webcmd.html
...
{called after web browser is finish loading webcmd.html.  The name of this function must match
the name of the webcmd.html file. }
...
if isempty.errors.historyfiles then
 ...
 setElementValue("cmdno", 3#data)
  + setElementValue("history", 2#data)
...
let newhistory =
 ...
 /row^(cmdAsEntered.cmdinfo) /sp /cell^(href.outfiles) /row"
  + history
let newcmdno = [toword(toint.1#cmdno + 1)]
...
Function loadcmd(a:real) real
  setElementValue("cmdline",getElementValue("ex."+print(1,a)))
...
do
 if cmd = 1#e then
  let cl = [cmd] + addDefaultName(cmdline0, 2#e)
...
file(
extractValue(cmdline, "output o" )
, PEGdebug(1#files, extractValue(cmdline, "steps" ), extractFlag(cmdline, "notable" ))
)
]
...
file(
extractValue(cmdline, "output o" )
, usegraph(1#files, extractValue(cmdline, "include" ), extractValue(cmdline, "exclude" ))
)
]
...
file(
extractValue(cmdline, "output o" )
, buildhelp(1#files, 2#files, extractFlag(cmdline, "shellscript" ), extractFlag(cmdline, "doc" ))
)
]
