...
let names = n
let cf = 
 compilerFront:callconfig(if isempty.pass then"pass2"else pass, input)
let prg = prg.cf
...
* One use is to figure out what functions are used between modules. The usegraph of the core functions indicates there are
dependences between the modules texio, file and bits.To see the dependences use  /< block front+built stdlib.libsrc mods
=textio file bits format  /> A graph will be display with the dependences between the modules. The nodes in the graph are
the procedure names. Since a name does not uniquely identify a function hovering over the beginning of the name will pop
up a more complete discription beginning with the name of the function. 

* The dependence on the module bits will not be displayed. If an earilier pass of the compiler is specified like this  /< block
front  +built stdlib.libsrc mods=textio file bits format pass=text  /> then it will be displayed. 

* The dependence with in the module textio can be seen with  /< block front  +built stdlib.libsrc mods=textio pass=text
flags=within  />

* To see all the functions that call functions named  /em breakparagraph in the library use  /< block front  +built stdlib
.libsrc n=breakparagraph pass=text out=calledby  />

* This will list the function definitions in a package  /< block front  +built stdlib.libsrc mods=textio out=symdef  />
The format is the function followed by a post order transversal of the call tree. 
...
* The front command takes several parameters that control which functions are considered.
 ...
 /br ○  /strong pass=The option pass determines how much processing is done before looking at the symbols.  /< block •  /strong
library Only report on functions imported from libraries.
...
indirectly are included in the graph
 /br • calls The option n is ignored in building a call graph. Then only the symbols that • are called(directly or indirectly
)from symbols in n are included in the graph.
