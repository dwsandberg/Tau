...
if next ∈ dq then lexaction1(next, findindex(terminals, "$wordlist"_1), next)
 ...
 lexaction1("}"_1, findindex(terminals, "comment"_1), "}"_1)
else
 let token = 
  if next ∈ "< > >1 >2 ≤ ≠ ≥" then ">"_1
...
let mostfrequentwords = 
 dq
 + "\, ().:+_seq = a int if-then else Function let word 0 i T] [2 use function mytype empty
  inst"
let wordstoinclude = 
 mostfrequentwords + terminals + "= < > ⊻ >2 >1 ≤ ≠ ≥ >> << in+-∈ * / mod \ ∪ ∩_^“ ”"
 + prepreplacements("", "", "le ≤ ge ≥ ne ≠ and ∧ or ∨ xor ⊻ cup ∪ cap ∩ in ∈ nin ∉", 1)
let actionlist = 
...
+ toword.findindex(terminals, "W"_1)
+ "/br /br function Itoken:T int {generated by genlex module in tools}"
+ toword.findindex(terminals, "I"_1)
+ "
 /br
 /br function lextable:T seq.token.T
 /br {generated by genlex module in tools}
 /br ["
...
if i > length.pairs then old + new
 ...
 , subseq(new, 1,-j - 1) + [pairs_(i + 1)]
 + subseq(new,-j, length.new)
 , pairs
 , i + 2
 ) 