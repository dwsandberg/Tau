...
for acc = empty:seq.word, count = 0, s ∈ prg do
 ...
 let b = 
  if aborted.p then "/p ERROR:$(sym.s) /br $(message.p) /br fullcode $(code.s)" else result.p
 ,
...
/do
 if count = 0 then
  ...
  "Base Type Check Failed $(count) Times $(acc)"
 ...
 if length.codeonly = 0 then
  ...
  for stk = empty:stack.mytype, localtypes = localdict, skip = false, s ∈ codeonly do
   if skip then
    ...
    assert top.stk = typeboolean report "if problem $(for a = "", e ∈ top(stk, 1) do a + %.e /do a)",
    next(pop.stk, localtypes, false)
    ...
    let parakinds = 
     for acc = empty:seq.mytype, @e ∈ paratypes.s do acc + coretype(@e, typedict) /do acc
...
do
 let t2 = b_idx,
 next(
  t2 = t ∨ t = typebyte ∧ t2 = typeint ∨ t2 = typebyte ∧ t = typeint
  , idx + 1)
/do acc

Function checkresults(prg:seq.symdef) seq.word
for defines = asset.knownsym, uses = empty:set.symbol, sd ∈ prg do
 next(defines + sym.sd, uses ∪ asset.code.sd)
/do
 ...
 for acc10 = "/p /p checkresults /p", h ∈ toseq.undefined do
  ...
  ∨ isBuiltin.h ∧ wordname.h ∈ "createthreadZ" ∧ paratypes.h = [typeptr] then
   acc10
  else
   acc10 + %.h + "/br"
 /do "CheckResult:$(if isempty.acc10 then "OK" else acc10 + "/p end checkresults /p")"
