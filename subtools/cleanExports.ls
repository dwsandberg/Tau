...
+ if name.module.sym.e ≠ modname.e ∧ first.%.module.sym.e ∉ "builtin internal"then
 ...
 if subseq(org.e, length.org.e - length.b + 1, length.org.e) = b then org.e else org.e + b
else org.e
...
for tt = empty:seq.seq.word, ex ∈ toseq.t do
 tt + %.ex
/for(
 for tt2 = "", l ∈ alphasort.tt do tt2 + " /p" + l << (findindex(l, "Export"_1) - 1)/for(tt2))
   ...
   for acc = empty:seq.symbol, m1 ∈ libmods.m do
    if name.modname.m1 ≠ newname then acc
     ...
     for acc2 = acc, sym ∈ exports.m1 do
      ...
      if isempty.t ∨ paragraphno.t_1 = 0 ∨ name.module.sym ≠ newname then acc2 + sym else acc2
     /for(acc2)
   /for(acc)
