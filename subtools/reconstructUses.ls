...
{OPTION PROFILE}
let exporttypes = 
 for acc = empty:seq.mytype, sym ∈ exports do
  acc + types.sym + resulttype.sym
 /do acc
let usestypes = 
 for acc = exporttypes, sd ∈ toseq.prg do
  if module.sym.sd = modname then acc + types.sym.sd + resulttype.sym.sd else acc
 /do removeseq.acc \ asset.[typereal, typeint, typeT]
,
...
for acc = empty:seq.symbol, sym ∈ syms do acc + fixLengthSym.sym /do acc
...
if isempty.exports then
 ...
 let uses5 = 
  for uses = exports, symlist = exports, sd ∈ toseq.prg.m do
   if module.sym.sd = modname.md then
    ...
    next(uses, symlist)
  /do asset(uses + addtypes(symlist, modname.md, prg.m))
 ...
 /do asset.chooseUses(uses, unhandled, modname, olduses, exported) - thismodule
...
let in = 
 ...
 /do for new = unhandled, t ∈ toseq.acc do new + inModule(exported, deepcopySym.t) /do new
,
...
let tmp = 
 if not.isempty.t ∨ isSimple.module.sym then
  ...
  /do acc2
,
for acc = empty:set.modref, e ∈ tmp do acc + in.e /do acc
...
for acc = empty:set.mytype, t ∈ s do acc + removeseq.t /do acc
