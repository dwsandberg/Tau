...
let d = for acc = empty:seq.addrsym, a ∈ addresssymbols do acc + a /do    acc ,
for acc = %.length.d , p ∈ profiledata_1 do 
  acc +  "/br"+%.caller.p +%.callee.p +%.counts.p
  /do acc

let d = for acc = empty:seq.addrsym, a ∈ addresssymbols do acc + a /do   asset.acc  /for
let profileData2 = for acc = empty:seq.parc, e ∈ profiledata do acc + e /do acc  /for
 do for acc ="", p /in profileData2 do acc+"
 /br"+%.decode (d, caller.p)+%.decode (d, callee.p)+%.counts.p+%.space.p+%.clocks.p+%.unused
 .p /do acc  
 
let b = 
 for acc0 = empty:seq.labeledarc.symbol, max = 0, arc ∈ profileData2 do
  let m = measure(arc, measure) do
 ...
 /do (
  for acc = empty:seq.labeledarc.symbol, arc2 ∈ acc0 do
   let m = toint.first.label.arc2 do
...
do acc + arc.arc /do newgraph.acc /for
{for txt ="", a /in toseq.arcs.tmp do txt+" /br"+%.head.a+%.tail.a /for (txt)} do
{for txt ="", a /in b do txt+" /br"+%.head.a+%.tail.a+label.a /for (txt)} 
drawgraph(tmp, asset.b) 