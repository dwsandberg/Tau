...
Function abstracttypename(m:mytype) word name.(typerep.m) sub 1

Function isSimple(t:mytype) boolean n.typerep.t = 1

Function abstracttype(t:mytype) mytype
if n.typerep.t = 1 then t else mytype.[(typerep.t) sub 1, (typerep.typeT) sub 1]

Function abstractModref(typ:mytype) modref
let t = (typerep.typ) sub 1,
modref(library.t, modname.t, if n.typerep.typ > 1 then typeT else noparameter)

Function tomodref(m:mytype) modref
let t = (typerep.m) sub 1,
...
if name.a ∈ "char word packed2 packed3 packed4 packed5 packed6" then name.a = name.b
else name.a = name.b ∧ modname.a = modname.b ∧ wild(library.a, library.b) = EQ

Function >1(a:typedef, b:typedef) ordering
if name.a ∈ "char word packed2 packed3 packed4 packed5 packed6" then name.a >1 name.b
else name.a >1 name.b ∧ modname.a >1 modname.b ∧ wild(library.a, library.b)
...
mytype.[typedef(s sub 1, s sub 2, s sub 3)]
...
Function addabstract(a:mytype, t:mytype) mytype mytype([(typerep.a) sub 1] + typerep.t)

Function moduleref(modname:seq.word, para:mytype) modref
modref(modname sub 1, modname sub 2, para)
...
mytype.[
 typedef("$base" sub 1, "internal" sub 1, "internallib" sub 1)
 , typedef(toword.i, "internal" sub 1, "internallib" sub 1)
]
...
Function <<(base:mytype) mytype
{OPTION COMPILETIME NOINLINE}
mytype(typerep.typeref."<< << *" + typerep.base)
...
do
 if isempty.m then next(knownmods, s, defines, unresolveduses, unresolvedexports, mref)
 else if m sub 1 ∈ "Module module" then
  let newmod = modref(lib, m sub 2, if n.m > 2 then typeT else noparameter)
  ...
  let newdefines = defines + newtype(mref, m sub 2)
  ...
  next(
   ...
   , defines
   , unresolveduses
   , unresolvedexports + subseq(m, 4, findindex(m, "{" sub 1) - 1)
   , mref
  )
 else if m sub 1 ∈ "precedence" then
  let discard = addprec(m, false),
...
let mref2 =
 if name
 ∈ "process boolean ptr seq encoding word bits byte char packed2 packed3 packed4 packed5 packed6 encodingpair encodingstate typename index" then modref("*" sub 1, name.mref, para.mref)
 else mref,
mytype([typedef(name, name.mref2, library.mref2)] + typerep.para.mref2)
...
do
 ...
 if isempty.b then next(exports, x + t2) else next(exports + b sub 1, x)
...
do
 let b = resolveuse(dict, knownmods, t3, p),
 if isempty.b then next(uses, x2 + t3) else next(uses + b sub 1, x2),
...
let a = findelement2(knownmodules, modref("?" sub 1, ref sub 1, noparameter)),
if n.a ≠ 1 then empty:seq.modref
else if n.ref = 1 then
 assert not.isAbstract.a sub 1 report "incorrect format for referencing parametrized module: 'use:(ref) ' in module: :(modname.p)",
 toseq.a
else
 assert isAbstract.a sub 1 report "Unexpected type on:' use:(ref) ' in module: :(modname.p)"
 let b = resolvetype(knowntypes, ref << 2),
 if isempty.b then empty:seq.modref
 else [modref(library.a sub 1, name.a sub 1, b sub 1)]
...
do
 ...
 if isempty.a then r else r ∪ exports.a sub 1,
r ∪ defines.this 
