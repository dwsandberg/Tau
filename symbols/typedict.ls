...
let typesused = for acc = empty:seq.mytype, sym ∈ toseq.syms do acc + typesused.sym /do acc
let typesyms = 
...
report
 "recursive type problem:
  /br
  $(for acc10 = "", h ∈ unknown.bx5 do acc10 + print2.h + "/br" /do acc10)
  "
,
for acc = emptytypedict, d ∈ toseq.known.bx5 do add(acc, type.d, flatflds.d) /do acc

function print2(h:typeentry) seq.word
for acc = "type $(type.h) is", z ∈ flatflds.h do acc + %.z + "," /do acc >> 1
...
if isempty.flatflds then
 ...
 /do if unchanged then flatflds else expandflat(type, acc, types)

function replaceT(with:mytype, typs:seq.mytype) seq.mytype
for acc = empty:seq.mytype, t ∈ typs do acc + replaceT(with, t) /do acc

Function asseqseqmytype(dict:typedict) seq.seq.mytype
for acc = empty:seq.seq.mytype, tr ∈ toseq.totypedict.dict do acc + totypeseq.tr /do acc

Function %(dict:typedict) seq.word
for txt = "", tr ∈ toseq.totypedict.dict do
 for acc2 = txt, t ∈ totypeseq.tr do
  acc2 + %.t
 /do acc2 + "/br"
/do txt
...
let need = 
 for acc = empty:set.mytype, have = empty:set.mytype, te ∈ toseq.totypedict.subdict do
  next(
   ...
   /do acc2
   , have + type.te)
 /do acc \ (asset.[typeint, typeboolean, typeptr, typereal, typeword] ∪ have)
,
...
/do if isempty.new then subdict else closedict(all, typedict(totypedict.subdict ∪ new))
