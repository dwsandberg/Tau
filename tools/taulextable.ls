...
let alist = actionlist 
  ...
  "&br &br function Itoken int // generated by genlex module in tools //"+ toword.findindex("I"_1, tokenlist)+
  "&br &br function commenttoken int // generated by genlex module in tools //"+ toword.findindex("comment"_1, tokenlist)+
  "&br &br function  stringtoken int // generated by genlex module in tools //"+ toword.findindex("$wordlist"_1, tokenlist) 
+"&br  &br function lextable seq.lexaction FORCEINLINE.// generated by genlex module in tools //["
+ @(seperator.",&br", identity,"", @(item(23,4,209), identity, constantseq(209, defaultaction), alist))+"]"
...
Function findhash seq.word 
 // look for a semiperfect hash function // 
  ...
  let b = @(+, w,"", actionlist)
   ...
   let x=23 let y=4 let m=209
   let actualhashvalues=sort.@(+,xx( 23,4,209),empty:seq.seq.word,t)
    "Hash function is of form   ( x * ( firstchar + 2 * secondchar)+ y * thirdchar ) mod m +1 "+
    "&br actual hash values for x="+toword.x+"y="+toword.y+"m="+toword.m  +
     @(seperator.", &br", identity,"&br", actualhashvalues)+  "&br &br  x, y, m, list of collisions"
...
// check to see if b_i b_j collide. If they do not continue checking with next value of i. //
      if i &ne j &and  hashfunc (x,y,m,b_i) = hashfunc (x,y,m,b_j)  then 
