...
Function totext(l:lexaction1)seq.word 
 let w = if decodeword(w.l)_1 = decodeword("&"_1)_1 
...
Function getlextable seq.word 
 // generate the lextable for the Tau compiler. // 
  ...
  {"&br &br function Wtoken int // generated by genlex module in tools //"+ toword.findindex("W"_1, tokenlist)+"&br &br function Itoken int // generated by genlex module in tools //"+ toword.findindex("I"_1, tokenlist)+"&br &br function commenttoken int // generated by genlex module in tools //"+ toword.findindex("comment"_1, tokenlist)+"&br &br function stringtoken int // generated by genlex module in tools //"+ toword.findindex("$wordlist"_1, tokenlist)+"&br &br function lextable seq.lexaction FORCEINLINE.// generated by genlex module in tools // ["+ @(seperator.", &br", identity,"", @(item(23, 4, 209), identity, constantseq(209, defaultaction), alist))+"]"}
...
Function findhash seq.word 
 // look for a semiperfect hash function // 
  ...
  {"Hash function is of form(x *(firstchar + 2 * secondchar)+ y * thirdchar)mod m + 1"+"&br actual hash values for x ="+ toword.x +"y ="+ toword.y +"m ="+ toword.m + @(seperator.", &br", identity,"&br", actualhashvalues)+"&br &br x, y, m, list of collisions"+ @(+, searchhash(// subseq(findprimes(3, 400), 60, 4000)// arithseq(50, 1, 2), t, 0,""),"", arithseq(10, 1, 200))}
...
/function reconstruct(t:triple)word if c.t > 0 then encodeword([ a.t, b.t, c.t])else if b.t > 0 then encodeword([ a.t, b.t])else encodeword([ a.t])
...
Function searchhash(p:seq.int, b:seq.triple, i:int, result:seq.word, m:int)seq.word 
 let base = length.p 
  ...
  if not(l ="fail")
  then searchhash(p, b, i + 1, result +"&br"+ @(+, toword,"", [ x, y, m])+ if l ="success"then""else l, m)
