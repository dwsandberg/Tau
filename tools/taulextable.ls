...
function tokenlistX seq.word \\ tokenlistX is from parser generator \\
".=():>]-for * comment, [_ ; is T if # then else let assert report ∧ ∨ $wordlist A E G F W P L I FP NM D B"
...
let wordstoinclude = mostfrequentwords + tokenlistX + "= < > ? ≤ ≠ ≥ >> << in +-∈  * / mod ∪ ∩_^'"
...
function tolexaction(next:word)lexaction1
 ...
 if next ∈ ('"\\ ' +"'") then lexaction1(next, findindex("$wordlist"_1, tokenlistX), next)
 else if next = merge("&" + "le")then
 lexaction1(next, findindex(">"_1, tokenlistX),"≤"_1)
 ...
 lexaction1(next, findindex("-"_1, tokenlistX),"∈"_1)
  ...
  let token = if next ∈ ". ,():"then next
    ...
    let idx= findindex(token ,tokenlistX)
    let action= if idx > length.tokenlistX then     findindex( "W"_1,tokenlistX) else idx
...
let t = for @e ∈ actionlist, acc = empty:seq.seq.word ,,, acc + totext.@e
 " &br  &br function Wtoken:T int \\ generated by genlex module in tools \\" + toword.findindex("W"_1, tokenlistX)
+ " &br  &br function Itoken:T int \\ generated by genlex module in tools \\"
+ toword.findindex("I"_1, tokenlistX)
+ "
&br 
