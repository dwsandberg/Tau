...
function plist(t:seq.word,i:int,parano:int,names:seq.word) seq.word
   if i=1  then 
     if  length.names > 0 
      then "("+ (if names_parano=":"_1 then "" else [names_parano]+":")+t_i+
      plist(t,i+1,parano+1,names)
     else t
   else if t_i=".a"_1 &or t_i=". a"_1 then subseq(t,i,i+1)+plist(t,i+2,parano,names) 
   else 
   if parano &le length.names then "," +(if names_parano=":"_1 then "" else [names_parano]+":")+t_i+plist(t,i+1,parano+1,names)
...
// let z ="Parsedfunc 11 check 2 seq.boolean int seq.word l i"
 let nopara= toint(z_4)
   ...
   subseq(z,5,headlength+2-nopara)+">>>"+subseq(z,headlength+2-nopara+1 ,  headlength+2)+
   "&br &keyword Function"   +  z_3+
...
Function ref(modname:word)seq.word {"&{ noformat <a href = &quot"+ merge("#"_1, modname)+"&quot >"+ 
modname +"</a> &}"}
...
if key in "  Library library use" then "&{ block &keyword "+p+"&}"
 else if key="module"_1 then "&{ block &{ noformat <hr id = &quot"+ p_2 +"&quot > &} &keyword" +p +"&}"
 else if key="skip"_1 then "&{ block"+ subseq(p,2,length.p)+" &}"
 ...
 else if key in "parsedfunc Parsedfunc" then 
 let z=p
   let nopara= toint(z_4)
   let headlength=toint(z_2)
   let head=  (if key="parsedfunc"_1 then "&keyword function" else "&keyword Function" ) +  z_3+
   plist(subseq(z,5,headlength+2-nopara),1,1,subseq(z,headlength+2-nopara+1 ,  headlength+2))
   let tr=x(z,headlength+4,empty:stack.tree.word)
   // assert not(label.tr="builtin"_1) report "XXXX"+label.tr_1 + z_3 //
    let tr1= if label.tr="builtin"_1 &and label.tr_1= z_3 then 
         tree("builtin"_1,[tree("usemangle"_1)]) else tr
...
function x(s:seq.word,i:int,stk:stack.tree.word) tree.word
 ...
 let w=s_i
  ...
  else if w in "LIT PARAM" then
     // let a=gatherarg(s,i+1) //
     // x(s,i+length.a,push(stk,tree(merge(a)))) //
      assert length.s < i+2 &or not(s_(i+2) = "46"_1) report "?"
      x(s,i+2,push(stk,tree(s_(i+1))))
     ...
     let noargs1=length.term1-2-2
      ...
      let args=top(stk,noargs) 
       let t=   tree("process"_1,  [ tree(getname.codedown.label(args_(noargs-1)),subseq(args,1,noargs-4))])
        x(s,i+2,push(pop(stk,noargs),t))
    else if w in "FORCEINLINE PROFILE" then
        x(s,i+1,push(pop(stk,1),tree(w,[top.stk])))
 else
 let a=codedown(w) 
