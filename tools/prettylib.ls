...
function prettyparagraph(p:seq.word)seq.word 
 let key = p_1 
  ...
  then let z = p 
   ...
   let tr1 = if label.tr ="builtin"_1 
    then if label(tr_1)= z_3 
     then tree("builtin"_1, [ tree("usemangle"_1)])
     else if label(tr_1)="1"_1 
     then tree("builtin"_1, [ tree("NOOP"_1)])
     else if label(tr_1)="STATE"_1 ∧ label(tr_1_1)= z_3 
     then tree("builtin"_1, [ tree("STATE"_1, [ tree("usemangle"_1)])])
     else // assert false report print.tr // tr 
    else tr 
   let zz = prettytree2(defaultcontrol, tr1, head)
   // assert not(z_3 ="blockit"_1)&or label.tr ="export"_1 report print.tr_2_2 + z // zz 
  else""
...
function x(s:seq.word, i:int, stk:stack.tree.word)tree.word 
 if i > length.s 
  ...
  then x(s, i + 1, push(stk, tree.merge."sizeoftype:T"))
...
function getname(a:seq.seq.word)word 
 let b = a_1_1 
  if length(a_2)= 1 
  then b 
  else let c = towords.decodeword.b 
