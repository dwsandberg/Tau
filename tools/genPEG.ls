...
maketable(z3, "dq:(dq), // /, //action /action, //br /br", false)
...
[
 ...
 , "/error function recoveryEnding(rinfo:resultType /nogrammar, mytable:seq.tableEntry /end)seq.word /br for acc =:(dq.""), frame ∈ reverse.toseq.stk.rinfo do /br if action.Sstate.frame ∈[T, T', NT]then acc+recover.mytable sub index.Sstate.frame else acc acc /else function place(r:resultType)int i.top.stk.r /end"
 ...
 , "/trace function tracestart(trace:seq.word)seq.word:(dq)/br Key to column labels S--step no; D--Depth of Stack F--on fail reset I to F; I--Location in input; Lower case are values on top of stack /p S /td D /td f /td success /td fail /td result /td Result /td F /td State /td I /td Remaining input /td /tr $semicolon(trace):(dq)/end"
 , "/trace function trace(stepno:int, trace0:seq.word, stk:stack.frame, result:seq.attributeType, faili:int, state:state, i:int, input:seq.seqElementType /error, recovery:seq.word /end)seq.word let stkdata = %.n.toseq.stk+if isempty.stk then:(dq)/td /td /td /td:(dq)else:(dq)/td $semicolon(faili.top.stk)/td $semicolon(Sstate.top.stk)/td $semicolon(Fstate.top.stk)/td $semicolon(last.result.top.stk):(dq), trace0+:(dq)$semicolon(stepno)/td $semicolon(stkdata)/td $semicolon(%(:(dq)^:(dq), result)>> 1)/td $semicolon(faili)/td $semicolon(state)/td $semicolon(i)/td $semicolon(subseq(input, i, min(i+20, n.input-1)))/td /error $semicolon(recovery)/td /end /tr:(dq)/end"
...
/eol
Non-terminals:B C FP FP' S Str3 Type /eol
Terminals:(),.:: any function genPEG{}/eol
S ← function genPEG(FP FP')Type{Str3 B}/ function genPEG(FP FP')Type /eol
