...
if 1#zz ∉ "[" then templatedecs
 ...
 if 1#"notable" ∈ rest >> (n.zz + 1) then applySubstitutions(substitutionsTable, notable(gin, tbl, wordmap))
 else
   ...
   let Recover =
    if not.isWords then ""
     ...
     do
      ...
      if n.tmp > 1 ∧ 2^tmp ∉ "^" ∧ tmp ∈ ["1#^(dq.[2^tmp])"] then acc + 2^tmp
      else acc + "^" + "(^(tmp))",
     ",^(dq.acc)",
   table2
...
if n.r < 3 then r
 ...
 do
  if l1 ∈ "." ∧ l2 ∈ "$" then next(acc + (%(noNT - toint.e) + "^" + "R"), none, none)
...
[
 ...
 , "use PEGrules"
 , "/isWords /else type tblEle is action:state, match:seqElementType, Sstate:state, Fstate:state /end"
 ...
 /br if action.Sstate.frame /in [T, T', NT] then acc+recover.(index.Sstate.frame)#mytable else acc acc /else function place (r:resultType) int i.top.stk.r /end"
 , "type frame is Sstate:state, Fstate:state, i:int, result:seq.attributeType, faili:int, failresult:seq.attributeType"
 , "type resultType is stk:stack.frame /error, input:seq.word, place:int /end /trace, trace:seq.word /end"
 , "Function status (a:resultType) word if Sstate.top.stk.a /ne Match then 1#^(dq."Failed") else if place.a = {length of input} faili.top.stk.a then 1#^(dq."Match") else 1#^(dq."MatchPrefix")"
 ...
 , "/trace function trace (stepno:int, trace0:seq.word, stk:stack.frame, result:seq.attributeType, faili:int, state:state, i:int, input:seq.seqElementType /error, recovery:seq.word /end) seq.word let stkdata = %.n.toseq.stk+if isempty.stk then^(dq) /cell /cell /cell /cell^(dq) else^(dq) /cell $carrot (faili.top.stk) /cell $carrot (Sstate.top.stk) /cell $carrot (Fstate.top.stk) /cell $carrot (1^result.top.stk)^(dq), trace0+^(dq) /row $carrot (stepno) /cell $carrot (stkdata) /cell $carrot (% (^(dq)^^(dq), result) >> 1) /cell $carrot (faili) /cell $carrot (state) /cell $carrot (i) /cell $carrot (subseq (input, i, min (i+20, n.input-1))) /cell $carrot (recovery)^(dq) /end"
 ...
 /br for /trace stepno = 1, trace0 =^(dq)^(dq), /end /error rinfo = resultType (empty:stack.frame, myinput, 0 /trace,^(dq)^(dq) /end), /end stk = empty:stack.frame, state = startstate, i = 1, inputi = 1#myinput, result = [initAttr], faili = 1, failresult = [initAttr]
 /br while (toint.state > toint.Match
 /br /trace /and stepno /le endStep) do let trace = if stepno < startStep then trace0 else /error let tmp = if isempty.stk.rinfo then^(dq."") else^(dq) $carrot (place.rinfo) /cell^(dq)+recoveryEnding (rinfo, table) /end trace (stepno, trace0, stk, result, faili, state, i, myinput /error, tmp /end) /else) do /end
 /br let actionState = action.state
 /br if actionState = Fail then {goto Fstate.top.stk, i = faili.top, pop.stk, discard result}
 /br let top = top.stk
 /br if toint.action.Fstate.top /ge toint.S' then let newi = i.top next (/trace stepno+1, trace, /end /error rinfo, /end pop.stk, nextState.Fstate.top, newi, idxNB (myinput, newi), result.top, faili.top, failresult.top) else
 ...
 /br let top = top.stk,
 /br /error let newrinfo = if i > place.rinfo then resultType (stk, myinput, i /trace, trace /end) else rinfo /end next (/trace stepno+1, trace, /end /error newrinfo, /end stk, nextState.state, i, inputi, result.top, i, result.top)
 /br else if actionState = All then let top = top.stk let att = [toAttribute (1^result, subseq (myinput, i.top, i-1))], next (/trace stepno+1, trace, /end /error rinfo, /end pop.stk, Sstate.top, i, inputi, result.top+att, faili.top, failresult.top)
 /br else if actionState = Lambda then
 /br /error let newrinfo = if i > place.rinfo then resultType (stk, myinput, i /trace, trace /end) else rinfo /end
 ...
 /br else if actionState = Reduce* then /error let newrinfo = if i > place.rinfo then resultType (stk, myinput, i /trace, trace /end) else rinfo /end
 ...
 /br else {match non Terminal} let te = idxNB (packedTable, index.state) assert action.action.te /in [NT, NT*] report^(dq) PROBLEM PEG $carrot (state)^(dq) /br let newstk = push (stk, frame (Sstate.te, Fstate.te, i, result, faili, failresult)) let tmp = [toAttribute (1^result, empty:seq.seqElementType)] next (/trace stepno+1, trace, /end /error rinfo, /end newstk, nextState.action.te, i, inputi, tmp, i, tmp)
