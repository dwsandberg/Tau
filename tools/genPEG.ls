...
let j3 =
 if rest sub j2 ∈ "{" then j2 + findindex(rest << j2, "}" sub 1) + 1 else j2
let zz = rest << (j3 - 1),
...
[
 ...
 , "/trace function tracestart(trace:seq.word)seq.word:(dq)/br Key to column labels S--step no; D--Depth of Stack F--on fail reset I to F; I--Location in input; Lower case are values on top of stack <* table /row S /cell D /cell f /cell success /cell fail /cell result /cell Result /cell F /cell State /cell I /cell Remaining input $carrot(trace)*>:(dq)/end"
 ...
 /trace ∧ stepno ≤ endStep)do let trace = if stepno < startStep then trace0 else /error let tmp = if isempty.stk.rinfo then:(dq."")else:(dq)$carrot(place.rinfo)/cell $carrot(recoveryEnding(rinfo, table)):(dq)/end trace(stepno, trace0, stk, result, faili, state, i, myinput /error, tmp /end)/else)do /end /br
