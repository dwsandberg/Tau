...
function result(c:control, wrap:seq.word, seperator:seq.word, a:seq.prettyresult)prettyresult
 let totalwidth = @(+, displaywidth, 0, a)
 + (length.a - 1) * @(+, displaywidth(c), 0, seperator + space)
 let s = if totalwidth > 6000 then
 if @(max, displaywidth, 0, a) < 600 then divideseq(seperator, a, 10)else divideseq(seperator, a, 1)
 else @(seperator(seperator), text,"", a)
  prettyresult(totalwidth + @(+, displaywidth(c), 0, wrap + seperator), 0, 0, if length.wrap > 1 then subseq(wrap, 1, length.wrap - 1) + s + last.wrap
  else s)

Function prettyparagraph(p:seq.word)seq.word
 if length.p < 2 then p
 else
  let key = p_1
   if key in "Library library use"then" &{ block  &keyword" + p + " &}"
   else if key = "module"_1 then
   '  &{ block  &{ noformat <hr id ="' + p_2 + '">  &}  &keyword ' + p
    + " &}"
   else if key = "skip"_1 then
   if p_2 in "/function /Function"then
    " &{ block" + prettynoparse.subseq(p, 2, length.p) + " &}"
    else
     " &{ block" + addbreak(control.charwidths," &br", subseq(p, 2, length.p), 1, 0) + " &}"
   else if key in "export"then
   assert false report"is this still used" + p
    ...
    let t = text.x
    let t2 = if last.z = "builtinZinternal1Zinternal1"_1 ∧ z_(length.z - 1) = z_(headlength + 3)then
    subseq(t, 1, length.t - 1) + "usemangle"
    else t
     if @(+, displaywidth(control.charwidths), 0, head) + displaywidth.x > 6000 then
     " &{ block" + head + " &br" + t2 + " &}"
     else" &{ block" + head + t2 + " &}"

function x(ctl:control, s:seq.word, i:int, stk:stack.prettyresult)prettyresult
 if i > length.s then top.stk
 else
  // assert i < 128 report"at top"+ [ s_i]+ subseq(s, i, length.s)+ toword.prec.top.stk + toword.rprec.top.stk //
  let w = s_i
   if w in "LIT PARAM"then
   x(ctl, s, i + 2, push(stk, result(ctl, [ s_(i + 1)])))
   else if w in "WORDS"then
   ...
   let size = toint.s_(i + 1)
    ...
    let arg = top.stk
    let r = if displaywidth.arg + txtwidth > 6000 then
    prettyresult(10000, 2, 2," &br" + txt + " &br" + text.arg)
   ...
   let noargs = toint.s_(i + 1)
    let args = top(stk, noargs)
     ...
     let t1 = if noargs1 = 0 then result(ctl, [ term1_1_1])
     else result(ctl, [ term1_1_1] + "()",",", subseq(args, 1, noargs1))
     let t2 = if noargs2 = 0 then result(ctl, [ term2_1_1])
     else
      result(ctl, [ term2_1_1] + "()",",", subseq(args, noargs1 + 1, noargs1 + noargs2))
   ...
   let args = top(stk, 3)
    ...
    let r = if width > 6000 then
    resultblock(ctl
     ," &{ block  &keyword if" + text.args_1 + " &br  &keyword then" + text.args_2
     + if subseq(text.args_3, 1, 4) = " &{ block  &keyword if"then
     " &br  &keyword else" + subseq(text.args_3, 3, length.text.args_3)
     else" &br  &keyword else" + text.args_3 + " &}")
    else
     prettyresult(width + @(+, displaywidth(ctl), 0,"if then else"), 0, 10," &keyword if" + text.args_1 + " &keyword then" + text.args_2
     + " &keyword else"
     + text.args_3)
   ...
   let args = top(stk, 2)
    ...
    + if subseq(text.args_2, 1, 4) in [" &{ block  &keyword let"," &{ block  &keyword assert"]then
    " &br" + subseq(text.args_2, 3, length.text.args_2)
   ...
   let args = top(stk, 3)
    let reportclause = if displaywidth.args_3 + displaywidth.args_1 > 6000 then" &br report" + text.args_3
    else"report" + text.args_3
    let r = resulthigh(ctl," &{ block  &keyword assert" + text.args_1 + reportclause + " &{ block" + text.args_2
    + " &}  &}")
     x(ctl, s, i + 2, push(pop(stk, 3), r))
   else if w = "PRECORD"_1 then
   let noargs = toint.s_(i + 1)
    let args = top(stk, noargs)
    let t = functioncall(ctl,"process"_1, [ functioncall(ctl, getname.codedown.(text.args_(noargs - 1))_1, subseq(args, 1, noargs - 4))])
     x(ctl, s, i + 2, push(pop(stk, noargs), t))
   else if w = "siQ7AeoftypeQ3ATZQ24typesiQ7Aezlocal"_1 then
   x(ctl, s, i + 1, push(stk, result(ctl, [ merge."sizeoftype:T"])))
   else if w = "FREF"_1 then
   x(ctl, s, i + 2, push(stk, result(ctl, [ s_(i + 1)])))
   else
    let a = codedown.w
    ...
    let r = if noargs = 2 then
    if name = "makereal"_1 then
     ...
     else if name in "_^"then binaryop(ctl, [ name], 1, args_1, args_2)
     else if name in "* / mod ∪ ∩"then binaryop(ctl, [ name], 3, args_1, args_2)
     else if name in "-"then binaryop(ctl, [ space, name, space], 4, args_1, args_2)
     else if name in "in + - ∈ ∋"then binaryop(ctl, [ name], 4, args_1, args_2)
     else if name in "= < > ? ≤ ≠ ≥ >> <<"then binaryop(ctl, [ name], 5, args_1, args_2)
     else if name in "∧"then binaryop(ctl, [ name], 6, args_1, args_2)
...
function binaryop(ctl:control, op:seq.word, prec:int, a:prettyresult, b:prettyresult)prettyresult
 ...
 if prec ≤ prec.b then binaryop(ctl, op, prec, a, result(ctl,"()","", [ b]))
 else if prec < rprec.a then binaryop(ctl, op, prec, result(ctl,"()","", [ a]), b)
 else if displaywidth.a > 6000 then
 prettyresult(displaywidth.a + displaywidth.b + @(+, displaywidth(ctl), 0, op), prec, max(prec, rprec.b), text.a + " &br" + op + text.b)
 else
  prettyresult(displaywidth.a + displaywidth.b + @(+, displaywidth(ctl), 0, op), prec, max(prec, rprec.b), text.a + op + text.b)

function addbreak(ctl:control, insert:seq.word, s:seq.word, i:int, width:int)seq.word
 if i > length.s then s
 else if width < 5000 then addbreak(ctl, insert, s, i + 1, width + displaywidth(ctl, s_i))
 else
  addbreak(ctl, insert, subseq(s, 0, i - 1) + insert + subseq(s, i, length.s), i, 0)
 ...
 let b = a_1_1
  if length.a_2 = 1 then b
   ...
   let c = towords.decodeword.b
    if length.c > 1 ∧ last.c = "T"_1 then
    merge(subseq(c, 1, length.c - 1)
     + print.mytype.subseq(a_2, 1, length.a_2 - 1))
    else b
...
function functioncall(ctl:control, name:word, args:seq.prettyresult)prettyresult
 let noargs = length.args
  if noargs = 1 then
  if name = "-"_1 then
   if 2 < prec.args_1 then
    let r = result(ctl,"-()","", [ args_1])
      prettyresult(displaywidth.r, 2, 2, text.r)
    else
     prettyresult(displaywidth.args_1 + displaywidth(ctl,"-"_1), 2, 2,"-" + text.args_1)
   else
    ...
    if prec.args_1 > 2 ∨ rprec.args_1 > 2 then
    prettyresult(displaywidth.args_1 + @(+, displaywidth(ctl), 0, [ name] + "()"), 0, 0, [ name] + "(" + text.args_1 + ")")
    else
     prettyresult(displaywidth.args_1 + @(+, displaywidth(ctl), 0, [ name] + "."), 2, 2, [ name] + "." + text.args_1)
  else if noargs = 0 then result(ctl, [ name])
  else result(ctl, [ name] + "()",",", args)
...
Function escapequote(t:word)word if t = '"'_1 then merge."& quot"else t
