       ...
       assert key in"parsedfunc Parsedfunc"report"unknown key"+ key 
        let z = p 
         ...
         let t = text.x 
          ...
          let t2 = 
            if subseq(t, 1, 1)="builtin"
            then 
             if length.t > 2 ∧ t_3 = z_3 
             then"builtin.usemangle"
        ...
        then 
         ...
         then 
          ...
          let wholepart = if length.number - decimals = 0 then [ char.48]else subseq(number, 1, length.number - decimals)
           result(ctl 
           , [ encodeword((if isneg then [ hyphenchar]else empty:seq.char)+ wholepart)]+ decimalpart)
         else if name in"_^"
...
function breakup(seperator:seq.word, b:seq.prettyresult, len:int, i:int)seq.word @(seperator(seperator), text,"", subseq(b, i, i + len - 1))
...
function displaywidth(c:control, w:word)int @(+,_(widths.c), 0, @(+, toint, empty:seq.int, decodeword.w))
