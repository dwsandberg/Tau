...
function resolveamb( ruleprec:seq.ruleprec,unaryop:int,addop:int,           a:seq.action) seq.action
   ...
   //    assert false report "XXX"+@(+,print,"",a) //
      let c1actions=@(+,*.-1,empty:seq.int,cactions)
       if lookahead.a_1 in "* " &and unaryop in c1actions then
       ...
       let c2actions= if  asset.c1actions &cap asset.[unaryop,addop] = asset.[addop,unaryop] then
...
Function lr1parser(grammarandact:seq.seq.seq.word, ruleprec:seq.seq.word, alphabet:seq.word)seq.word
 ...
 // assert false report  print(grammar2,actions) //
  ...
  let actions2=@(+,resolveamb(RP,ruleno(grammar2,"E - E") ,ruleno(grammar2,"E E - E" )),empty:seq.seq.action,dups(actions))
