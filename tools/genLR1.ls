...
function ?(a:action, b:action)ordering lookahead.a ? lookahead.b  &and stateno.a ? stateno.b   
...
let a=toaction.a1 let b=toaction.b1
stateno.a ? stateno.b &and  codedaction.a ? codedaction.b &and lookahead.a ? lookahead.b   
...
function print(grammar:seq.seq.word, actions:seq.action)seq.word 
 let d= sort.for a &in actions , b=empty:seq.action2,,, b + action2(a)
 let c= for a &in d , b=empty:seq.action,,, b + toaction(a)
...
function print (grammar:seq.seq.word,a:seq.action,i:int,laststate:int,lastaction:int,result:seq.word) seq.word
 ...
 let this=a_i
  let p1 = if laststate ≠ stateno.this then
   ...
   +" &br state:" + toword.stateno.this else""
...
/function print(a:seq.arc.word) seq.word  
for arc &in a ,b="",,, b+tail.arc+head.arc
  
   
function follow(grammar:seq.seq.word)graph.word
let allarcs=for rule ∈ grammar, acc = empty:seq.arc.word ,,, acc + 
  for  e &in rule ,arcs= empty:seq.arc.word ,i,, if i > 2 then  arcs+arc(rule_(i-1),rule_i) else arcs
  follow(newgraph.allarcs, grammar)

function follow(g:graph.word, grammar:seq.seq.word)graph.word
 let g2 = for rule ∈ grammar, newgraph = g ,,, 
  let a = for succ ∈ toseq.successors(g, rule_1), acc = empty:seq.arc.word ,,, acc + arc(last.rule,succ)
   newgraph + for  pred ∈ toseq.predecessors(g, rule_1), acc = a ,,, acc +  arc(pred,rule_2 )
...
function print(s:state)seq.word
 '  &br ' + for dotrule ∈ toseq.toset.s, acc ="",,, list(acc,"|", print.dotrule);
...
function close(g:seq.seq.word, s:set.dottedrule)set.dottedrule
 let newset = for dotrule ∈ toseq.s, acc = s ,,, acc ∪ close(g, dotrule)
...
function close(grammar:seq.seq.word, p:dottedrule)set.dottedrule
 if place.p > length.rule.p then empty:set.dottedrule
  ...
  asset.for rule ∈ grammar, acc = empty:seq.dottedrule ,,, 
...
function advance(g:seq.seq.word, state:set.dottedrule, next:word)set.dottedrule
 close(g, asset.for p ∈ toseq.state, acc = empty:seq.dottedrule ,,, 
...
function resolveamb(ruleprec:seq.seq.word,grammar:seq.seq.word,dup:seq.action) seq.action
    ...
    if length.dup = 2 &and codedaction.dup_1 < 0 &and  codedaction.dup_2 > 0 then
...
Function lr1parser(grammarandact:seq.seq.seq.word, ruleprec:seq.seq.word, 
terminals:seq.word,attributename:seq.word)seq.word
 let grammar2 = for @e ∈ grammarandact, acc = empty:seq.seq.word ,,, acc + first.@e
 let nontermials = for rule &in grammar2, acc=empty:set.word,,, acc+ first.rule
 assert  isempty(asset.terminals &cap nontermials ) report "terminals and nontermials sets must be distinct"
 ...
 let  symbolsused = for rule ∈ grammar2, acc =empty:set.word,,, acc &cup asset.rule
 ...
 let k=(for  item &in ruleprec ,problems="",,,  problems+
   if length.item =1 &and item_1 &nin alphabet then "lookahead "+item+"is not in alphabet "
   else if length.item > 1 &and item &nin grammar2 then "rule is not in grammar:"+item
   else "")
 ...
 let m = encoding:seq.encodingpair.state
  if stateno > length.m then result
   ...
   let reduceActions= for rule &in reductions ,acc=empty:seq.action ,,, 
      acc+
      for lookahead &in  toseq(successors(follow.graminfo, first.rule) &cap alphabet) , acc2=empty:seq.action ,,,
...
,"=",">","E E ∧ E","∧","E E ∨ E","∨","E let A E ","A  W = E", "E assert E report D E",";","D E",' E comment E ']
...
, [ ' A  W = E ', ' let e = R_4 let name = tokentext.R_2 assert isempty.lookup(dict.R, name, empty:seq.mytype)report errormessage("duplicate symbol:"+ name, input, place)let newdict = dict.R + Local(name,(types.e)_1)bindinfo(newdict, code.e + Define.name, types.e, tokentext.R_2)'] 
, [ ' E let A E ', ' let name = tokentext.R_1 let f = lookup(dict.R, name, empty:seq.mytype)assert not.isempty.f report"internal error/could not find local symbol to delete from dict with name"+ name bindinfo(dict.R_1-f_1, code.R_1 + code.R_2, \\ +"SET"+ name, \\ types.R_2,"")'] 
