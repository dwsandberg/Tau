...
function finished(s:state)seq.seq.word for @e ∈ toseq.toset.s, acc = empty:seq.seq.word ; acc + finished.@e
...
function print(a:graph.word)seq.word for @e ∈ toseq.nodes.a, acc ="nodes"; acc + print2(a, @e)
...
Function test11 seq.word extractgrammer.' function action(ruleno:int, input:seq.word, place:int, R:reduction.bindinfo)bindinfo if ruleno = // G F # // 1 then R_1 else if ruleno = // F W NM(FP)T E // 2 then createfunc(R, input, place, tokentext.R_2, types.R_4, R_6, R_7)else if ruleno = // F W N(FP)T E // 3 then createfunc(R, input, place, tokentext.R_2, types.R_4, R_6, R_7)else if ruleno = // F W NM T E // 4 then createfunc(R, input, place, tokentext.R_2, empty:seq.mytype, R_3, R_4)else if ruleno = // F W NM is W P // 5 then assert(tokentext.R_4)_1 ∈"record sequence"report errormessage("Expected record or sequence after is in type definition got:"+ tokentext.R_4, input, place)bindinfo(dict.R, empty:seq.symbol, types.R_5, tokentext.R_4 + tokentext.R_2)else if ruleno = // F T // 6 then R_1 else if ruleno = // FP P // 7 then bindinfo(types.R_1 @ addparameter(dict.R, cardinality.dict.R, input, place, @e), empty:seq.symbol, types.R_1,"")else if ruleno = // P T // 8 then bindinfo(dict.R, empty:seq.symbol, [ abstracttype(":"_1, gettype.R_1)],"")else if ruleno = // P P, T // 9 then bindinfo(dict.R, empty:seq.symbol, types.R_1 + [ abstracttype(":"_1, gettype.R_3)],"")else if ruleno = // P W:T // 10 then bindinfo(dict.R, empty:seq.symbol, [ abstracttype((tokentext.R_1)_1, gettype.R_3)],"")else if ruleno = // P P, W:T // 11 then bindinfo(dict.R, empty:seq.symbol, types.R_1 + [ abstracttype((tokentext.R_3)_1, gettype.R_5)],"")else if ruleno = // P comment W:T // 12 then bindinfo(dict.R, empty:seq.symbol, [ abstracttype((tokentext.R_2)_1, gettype.R_4)],"")else if ruleno = // P P, comment W:T // 13 then bindinfo(dict.R, empty:seq.symbol, types.R_1 + [ abstracttype((tokentext.R_4)_1, gettype.R_6)],"")else if ruleno = // E NM // 14 then let id = tokentext.R_1 let f = lookupbysig(dict.R, id, empty:seq.mytype, input, place)bindinfo(dict.R, [ f], [ resulttype.f],"")else if ruleno = // E NM(L)// 15 then unaryop(R, input, place, tokentext.R_1, R_3)else if ruleno = // E(E)// 16 then R_2 else if ruleno = // E { E } // 17 then R_2 else if ruleno = // E if E then E else E // 18 then let thenpart = R_4 assert(types.R_2)_1 = mytype."boolean"report errormessage("cond of if must be boolean", input, place)assert types.R_4 = types.R_6 report errormessage("then and else types are different", input, place)let newcode = code.R_2 + [ Lit.2, Lit.3, Br]+ code.R_4 + Exit + code.R_6 + [ Exit, Block((types.R_4)_1, 3)]bindinfo(dict.R, newcode, types.thenpart,"")else if ruleno = // E E_E // 19 then opaction(R, input, place)else if ruleno = // E-E // 20 then unaryop(R, input, place, tokentext.R_1, R_2)else if ruleno = // E W.E // 21 then unaryop(R, input, place, tokentext.R_1, R_3)else if ruleno = // E E * E // 22 then opaction(R, input, place)else if ruleno = // E E-E // 23 then opaction(R, input, place)else if ruleno = // E E = E // 24 then opaction(R, input, place)else if ruleno = // E E > E // 25 then opaction(R, input, place)else if ruleno = // E E ∧ E // 26 then opaction(R, input, place)else if ruleno = // E E ∨ E // 27 then opaction(R, input, place)else if ruleno = // L E // 28 then R_1 else if ruleno = // L L, E // 29 then bindinfo(dict.R, code.R_1 + code.R_3, types.R_1 + types.R_3,"")else if ruleno = // E [ L]// 30 then let types = types.R_2 assert types @ ∧(true,(types_1 = @e))report errormessage("types do not match in build", input, place)bindinfo(dict.R, [ Stdseq, Lit.length.types]+ code.R_2 + newsymbol("kindrecord", mytype."T builtin", [ typeint, typeint]+ types, typeptr), [ typeseq + types_1],"")else if ruleno = // A let W = E // 31 then let e = R_4 let name = tokentext.R_2 assert isempty.lookup(dict.R, name, empty:seq.mytype)report errormessage("duplicate symbol:"+ name, input, place)let newdict = dict.R + Local(name,(types.e)_1)bindinfo(newdict, code.e + Define.name, types.e, tokentext.R_2)else if ruleno = // E A E // 32 then let name = tokentext.R_1 let f = lookup(dict.R, name, empty:seq.mytype)assert not.isempty.f report"internal error/could not find local symbol to delete from dict with name"+ name bindinfo(dict.R_1-f_1, code.R_1 + code.R_2, // +"SET"+ name, // types.R_2,"")else if ruleno = // E assert E report E E // 33 then assert(types.R_2)_1 = mytype."boolean"report errormessage("condition in assert must be boolean in:", input, place)assert(types.R_4)_1 = mytype."word seq"report errormessage("report in assert must be seq of word in:", input, place)let newcode = code.R_2 + [ Lit.2, Lit.3, Br]+ code.R_5 + Exit + code.R_4 + symbol("assert(word seq)", typerep.(types.R_5)_1 +"builtin", typerep.(types.R_5)_1)+ Exit + Block((types.R_5)_1, 3)bindinfo(dict.R, newcode, types.R_5,"")else if ruleno = // E I // 34 then bindlit.R else if ruleno = // E I.I // 35 then bindinfo(dict.R, [ Words(tokentext.R_1 +"."+ tokentext.R_3), symbol("makereal(word seq)","UTF8","real")], [ mytype."real"],"")else if ruleno = // T W // 36 then isdefined(R, input, place, mytype.tokentext.R_1)else if ruleno = // T W.T // 37 then isdefined(R, input, place, abstracttype((tokentext.R_1)_1,(types.R_3)_1))else if ruleno = // E $wordlist // 38 then let s = tokentext.R_1 bindinfo(dict.R, [ Words.subseq(s, 2, length.s-1)], [ mytype."word seq"],"")else if ruleno = // E comment E // 39 then R_2 else if ruleno = // N_// 40 then R_1 else if ruleno = // N-// 41 then R_1 else if ruleno = // N = // 42 then R_1 else if ruleno = // N > // 43 then R_1 else if ruleno = // N * // 44 then R_1 else if ruleno = // N ∧ // 45 then R_1 else if ruleno = // N ∨ // 46 then R_1 else if ruleno = // NM W // 47 then R_1 else if ruleno = // NM W:T // 48 then bindinfo(dict.R, empty:seq.symbol, empty:seq.mytype, tokentext.R_1 +":"+ print.(types.R_3)_1)else if ruleno = // D E @ NM(E, // 49 then applypart1(R_1, R_3, R_5, input, place)else if ruleno = // D E @ N(E, // 50 then applypart1(R_1, R_3, R_5, input, place)else assert ruleno = // E D L)// 51 report"invalid rule number"+ toword.ruleno applypart2(R_1, R_2, input, place)'
