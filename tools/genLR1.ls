...
Function lr1parser(grammarandact:seq.seq.seq.word, ruleprec:seq.seq.word)seq.word 
 let grammar2 = @(+, first, empty:seq.seq.word, grammarandact)
  ...
  {(if length.amb > 0 then"ambiguous actions:"+ amb else"")+"&br noactions"+ toword.length.actions +"&br nosymbols:"+ toword.length.alphabet +"alphabet:"+ alphabet +"&br norules"+ toword.length.grammarandact +"&br nostate"+ toword.length.mapping.estate +"&br follow"+ @(+, print,"", toseq.arcs.follow.graminfo)+ let a = @(addaction.alphabet, identity, dseq.0, actions)
   {"&br &br function tokenlist seq.word &quot"+ alphabet +"&quot"+"&br &br function startstate int"+ toword.initialstateno +"&br &br function actiontable seq.int ["+ @(seperator.",", toword,"", a)+"]"+"&br &br"+ generatereduce(grammarandact, alphabet)+"&br &br function printstate(stateno:int)seq.word &br ["+ @(seperator.",", print,"", mapping.estate)+"]_stateno"} }
...
Function gentau2 seq.word 
 // used to generater tau parser for Pass1 of the tau compiler. // lr1parser(taurules2, tauruleprec)
...
function taurules2 seq.seq.seq.word 
 ...
 ["P P, T", 
 "bindinfo(dict, &quot &quot, types.$_1 + [ mytype(code.$_3 + &quot:&quot)])"], 
 ...
 ["E I","$_1"], 
 ["E I.I", 
 "let d = decode.(code.$_3)_2 bindinfo(dict, &quot LIT &quot + [ encodeword(decode.(code.$_1)_2 + d)]+ &quot LIT &quot + countdigits(d, 1, 0)+ &quot makerealZrealZintZint &quot, [ mytype.&quot real &quot])"], 
 ["T W","$_1"], 
