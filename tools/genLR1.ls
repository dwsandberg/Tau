...
function resolveamb(ruleprec:seq.seq.word, grammar:seq.seq.word, dup:seq.action)seq.action
 if length.dup = 1 then dup
  ...
  let reducepos1 = for r = 0, i = 1, p = ruleprec while r = 0 do next(if length.p > 1 ∧ p = rule1 then i else 0, i + 1)/for(r)
  let reducepos2 = for r = 0, i = 1, p = ruleprec while r = 0 do next(if length.p > 1 ∧ p = rule2 then i else 0, i + 1)/for(r)
   if reducepos1 = 0 ∧ reducepos2 = 0 then dup
   else
    resolveamb(ruleprec, grammar, if between(reducepos1, 1, reducepos2)then [ dup_1] + dup << 2 else dup << 1)
 else if length.dup = 2 ∧ codedaction.dup_1 < 0 ∧ codedaction.dup_2 > 0 then
 let rule1 = grammar_(-codedaction.dup_1)
 let shiftpos = for r = 0, i = 1, p = ruleprec while r = 0 do next(if length.p = 1 ∧ p_1 = lookahead.dup_2 then i else 0, i + 1)/for(r)
 let reducepos = for r = 0, i = 1, p = ruleprec while r = 0 do next(if length.p > 1 ∧ p = rule1 then i else 0, i + 1)/for(r)
  ...
  assert isempty(symbolsused - asset.alphabet)report"Symbols not included in alphabet" + toseq(symbolsused - asset.alphabet)
   ...
   /for(problems)
    ...
    let actions = closestate(graminfo, 1, empty:seq.action, asset.terminals)
     ...
     let actions2 = for acc = empty:seq.seq.action, @e = dups.actions do acc + resolveamb(ruleprec, grammar2, @e)/for(acc)
     let actions3 = for acc = empty:seq.action, @e = actions2 do acc + @e /for(acc)
     let amb = for acc ="", @e = actions2 do
      acc
     ...
     /for(acc)
      ...
      + if length.amb = 0 then
      let x = for table = sparseseq.0, @e = actions3 do
...
let m = encoding:seq.encodingpair.state
 if stateno > length.m then result
  ...
  let reduceActions = for acc = empty:seq.action, rule = reductions do
   acc
   + for acc2 = empty:seq.action, lookahead = toseq(successors(follow.graminfo, first.rule) ∩ alphabet)do
    acc2 + reduce(stateno, lookahead, ruleno(grammar.graminfo, rule))
   /for(acc2)
  /for(acc)
  let newresult = for acc = result + reduceActions, lookahead = toseq.asset.shifts.state do
...
+ if i = last then ' report"invalid rule number"+ toword.ruleno  /br ' else"then"
 ...
 + for acc ="", w = s_2 do
  acc
  + if w ∈ "let assert"then" /br" + w else [ w]
 /for(acc)
...
function tauruleprec seq.seq.word { list of rules and lookaheads. The position of the lookahead is noted. Rule reductions after are discard and rule the first rule listed before the position is used to reduce. }
{ [""]}
["(","E NM", ' E comment E ',"E E_E","_","E W.E","E E * E","E-E","*","E E-E"
,"-","E E > E","E E = E","=",">","E E ∧ E","∧","E E ∨ E","∨"]
+ ["/for","/if","E if E then E else E","E assert E report D E","A W = E","E let A E","D E"]

function taurules2 seq.seq.seq.word [ [ ' G F # ', ' R_1 ']
, [ ' F W NM(FP)T E ', ' createfunc(R, input, place, tokentext.R_2, types.R_4, R_6, R_7)']
, [ ' F W_(FP)T E ', ' createfunc(R, input, place, tokentext.R_2, types.R_4, R_6, R_7)']
...
, [ ' FP P ', ' bindinfo(for acc = dict.R, @e = types.R_1 do addparameter(acc, cardinality.dict.R, input, place, @e)/for(acc), empty:seq.symbol, types.R_1,"")']
...
, [ ' E if E then E else E /if ', ' ifexp(R, R_2, R_4, R_6, input, place)']
...
, [ ' E [ L]', ' let types = types.R_2 assert for acc = true, @e = types do acc ∧ types_1 = @e /for(acc)report errormessage("types do not match in build", input, place)bindinfo(dict.R, code.R_2 + Sequence(types_1, length.types), [ addabstract("seq"_1, types_1)],"")']
, [ ' A W = E ', ' let name = tokentext.R_1 assert isempty.lookup(dict.R, name, empty:seq.mytype)report errormessage("duplicate symbol:"+ name, input, place)let newdict = dict.R + Local(name,(types.R_3)_1)bindinfo(newdict, code.R_3 + Define.name, types.R_3, name)']
, [ ' E let A E ', ' bindinfo(dict.R_1, code.R_2 + code.R_3, types.R_3,"")']
, [ ' E assert E report D E ', ' assert(types.R_2)_1 = mytype."boolean"report errormessage("condition in assert must be boolean in:", input, place)assert(types.R_4)_1 = mytype."word seq"report errormessage("report in assert must be seq of word in:", input, place)let assertsym = symbol("assert:T(word seq)", typerep.(types.R_5)_1 +"builtin", typerep.(types.R_5)_1)bindinfo(dict.R, ifthenelse(code.R_2, code.R_5, code.R_4 + assertsym,(types.R_5)_1), types.R_5,"")']
, [ ' E I ', ' bindlit.R ']
...
, [ ' F1 W = E ', ' let name = tokentext.R_1 assert isempty.lookup(dict.R, name, empty:seq.mytype)report errormessage("duplicate symbol:"+ name, input, place)bindinfo(dict.R_1, code.R_3, types.R_3, name)']
, [ ' F1 F1, W = E ', ' let name = tokentext.R_3 assert isempty.lookup(dict.R, name, empty:seq.mytype)report errormessage("duplicate symbol:"+ name, input, place)bindinfo(dict.R, code.R_1 + code.R_5, types.R_1 + types.R_5, tokentext.R_1 + tokentext.R_3)']
, [ ' F2 F1 ', ' forlocaldeclare(R_1, input, place)']
, [ ' E for F2 do E /for(E)', ' forbody(dict.R_1, R_2, R_4, R_1, R_7, input, place)']
, [ ' E for F2 while E do E /for(E)', ' forbody(dict.R_1, R_2, R_6, R_4, R_9, input, place)']
, [ ' D E ',"R_1"]
]
...
, [ ' E(E)', ' R_2 ']
, [ ' E if E then E else E ', ' let x1 ="%%%keyword if"+ removeclose.text.R_2 +"%%%keyword then"let x = attribute(x1 + removeclose.text.R_4)let t = if width.R_2 + width.R_4 + width.R_6 < 30 then [ x, key.R_5, R_6]else if width.R_2 + width.R_4 < 30 then [ x, elseblock.R_6]else [ attribute(x1 + removeclose.text.block.R_4), elseblock.R_6]pretty(t + attribute."\ if")']
, [ ' E if E then E else E /if ', ' let x1 ="%%%keyword if"+ removeclose.text.R_2 +"%%%keyword then"let x = attribute(x1 + removeclose.text.R_4)let t = if width.R_2 + width.R_4 + width.R_6 < 30 then [ x, key.R_5, R_6]else if width.R_2 + width.R_4 < 30 then [ x, elseblock.R_6]else [ attribute(x1 + removeclose.text.block.R_4), elseblock.R_6]pretty(t + attribute."\ if")']
...
, [ ' F2 F1 ', ' R_1 ']
, [ ' E for F2 do E /for(E)', ' if width.R_2 + width.R_4 < 30 then pretty.[ key.R_1, list.R_2, attribute("%%%keyword do"+ removeclose.text.R_4 +"%%%keyword /for"), R_6, R_7, R_8]else pretty.[ key.R_1, list.R_2, attribute("%%%keyword do"+ removeclose.text.block.R_4 +"%%%br %%%keyword /for"), R_6, R_7, R_8]']
, [ ' E for F2 while(E)E /for(E)', ' pretty.[ key.R_1, list.R_2, attribute("%%%keyword while("+ text.R_5 +")"+ removeclose.text.R_7 +"%%%keyword /for"), R_9, R_10, R_11]']
, [ ' D E ',"R_1"]
]

Function test11 seq.word extractgrammer.' Function action(ruleno:int, input:seq.word, place:int, R:reduction.attribute2)attribute2 if ruleno = \\ G F # \\ 1 then R_1 else if ruleno = \\ F W NM(FP)T E \\ 2 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W_(FP)T E \\ 3 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W-(FP)T E \\ 4 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W =(FP)T E \\ 5 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W >(FP)T E \\ 6 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W *(FP)T E \\ 7 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W ∧(FP)T E \\ 8 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W ∨(FP)T E \\ 9 then pretty.[ key.R_1, R_2, R_3, R_4, R_5, R_6, if width.R_4 + width.R_7 > 30 then block.R_7 else R_7]else if ruleno = \\ F W NM T E \\ 10 then pretty.[ key.R_1, R_2, R_3, R_4]else if ruleno = \\ F W NM is P \\ 11 then pretty.[ key.R_1, R_2, R_3, list.R_4]else if ruleno = \\ F T \\ 12 then \\ use \\ pretty.[ R_1]else if ruleno = \\ FP P \\ 13 then list.R_1 else if ruleno = \\ P T \\ 14 then R_1 else if ruleno = \\ P P, T \\ 15 then R_1 + R_3 else if ruleno = \\ P W:T \\ 16 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ P P, W:T \\ 17 then R_1 + pretty.[ R_3, R_4, R_5]else if ruleno = \\ P comment W:T \\ 18 then pretty.[ R_1, R_2, R_3, R_4]else if ruleno = \\ P P, comment W:T \\ 19 then R_1 + pretty.[ R_3, R_4, R_5, R_6]else if ruleno = \\ E NM \\ 20 then R_1 else if ruleno = \\ E NM(L)\\ 21 then if length.R_3 = 1 ∧ length.text.R_1 = 1 then wrap(3, R_1,".", R_3)else pretty.[ R_1, R_2, list.R_3, R_4]else if ruleno = \\ E(E)\\ 22 then R_2 else if ruleno = \\ E if E then E else D \\ 23 then let x1 ="%%%keyword if"+ removeclose.text.R_2 +"%%%keyword then"let x = attribute(x1 + removeclose.text.R_4)let t = if width.R_2 + width.R_4 + width.R_6 < 30 then [ x, key.R_5, R_6]else if width.R_2 + width.R_4 < 30 then [ x, elseblock.R_6]else [ attribute(x1 + removeclose.text.block.R_4), elseblock.R_6]pretty(t + attribute.";")else if ruleno = \\ E E_E \\ 24 then wrap(1, R_1, text.R_2, R_3)else if ruleno = \\ E-E \\ 25 then unaryminus.R_2 else if ruleno = \\ E W.E \\ 26 then wrap(3, R_1, text.R_2, R_3)else if ruleno = \\ E E * E \\ 27 then wrap(4, R_1, text.R_2, R_3)else if ruleno = \\ E E-E \\ 28 then wrap(5, R_1, text.R_2, R_3)else if ruleno = \\ E E = E \\ 29 then wrap(6, R_1, text.R_2, R_3)else if ruleno = \\ E E > E \\ 30 then wrap(7, R_1, text.R_2, R_3)else if ruleno = \\ E E ∧ E \\ 31 then wrap(8, R_1, text.R_2, R_3)else if ruleno = \\ E E ∨ E \\ 32 then wrap(9, R_1, text.R_2, R_3)else if ruleno = \\ L E \\ 33 then R_1 else if ruleno = \\ L L, E \\ 34 then R_1 + R_3 else if ruleno = \\ E [ L]\\ 35 then pretty.[ R_1, list.R_2, R_3]else if ruleno = \\ A W = E \\ 36 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ E let A E \\ 37 then \\ checkpara(pretty.[ R_1, R_2], block("%%%br let assert", R_3))\\ attribute("%%%keyword let"+ subseq(text.R_2, 1, 2)+ protect(text.R_2 << 2, text.block("%%%br let assert", R_3)))else if ruleno = \\ E assert E report D E \\ 38 then pretty.[ key.R_1, R_2, attribute("%%%keyword report"+ protect(text.R_4, text.block("%%%br let assert", R_5)))]else if ruleno = \\ E I \\ 39 then R_1 else if ruleno = \\ E I.I \\ 40 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ T W \\ 41 then R_1 else if ruleno = \\ T W.T \\ 42 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ E $wordlist \\ 43 then attribute2.[ prettyresult(0, length.text.R_1,"%%%{ literal"+ escapeformat.text.R_1 +"%%%}")]else if ruleno = \\ E comment E \\ 44 then let t ="%%%{ comment \\"+ escapeformat.text.R_1 << 1 >> 1 +"\\ %%%}"let t2 = if width.R_1 + width.R_2 > 30 ∧(text.R_2)_1 ≠"%%%br"_1 then t +"%%%br"else t pretty.[ attribute2.[ prettyresult(0, length.text.R_1, t2)], R_2]else if ruleno = \\ NM W \\ 45 then R_1 else if ruleno = \\ NM W:T \\ 46 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ D E \\ 51 then R_1 else if ruleno = \\ D E ; \\ 52 then R_1 else if ruleno = \\ F1 W = E \\ 53 then pretty.[ R_1, R_2, R_3]else if ruleno = \\ F1 F1, W = E \\ 54 then pretty.[ R_1, R_2, R_3, R_4, R_5]else if ruleno = \\ F2 F1 \\ 55 then R_1 else if ruleno = \\ E for F2 do E /for(E)\\ 56 then if width.R_2 + width.R_4 < 30 then pretty.[ key.R_1, list.R_2, attribute("%%%keyword do"+ removeclose.text.R_4 +"%%%keyword end"), R_6, R_7, R_8]else pretty.[ key.R_1, list.R_2, attribute("%%%keyword do"+ removeclose.text.block.R_4 +"%%%br %%%keyword end"), R_6, R_7, R_8]else assert ruleno = \\ E for F2 while(E)E /for(E)\\ 57 report"invalid rule number"+ toword.ruleno pretty.[ key.R_1, list.R_2, attribute("%%%keyword while("+ text.R_5 +")"+ removeclose.text.R_7 +"%%%keyword end"), R_9, R_10, R_11]'
