...
Function zerowidth(a:seq.T, b:nodeinfo.T)nodeinfo.T if n.b in a then b else nodeinfo(n.b, x.b, y.b, 0, seperation.b)
...
Function posindegree(g:graph.T, layers:seq.seq.T, layer:int, node:T)nodeinfo.T 
   let x = findindex(node, layers_layer)
    if x > length(layers_layer)
     then posindegree(g, layers, layer + 1, node)
     else 
...
Function defaultpos(g:graph.T, layers:seq.seq.T)set.nodeinfo.T @(+, pos(layers, 1), empty:set.nodeinfo.T, @(+, identity, empty:seq.T, layers))

Function posindegree(g:graph.T, layers:seq.seq.T)set.nodeinfo.T @(+, posindegree(g, layers, 1), empty:set.nodeinfo.T, @(+, identity, empty:seq.T, layers))
...
Function ?(a:arcinfo.T, b:arcinfo.T)ordering head(a.a)? head(a.b)∧ y.a ? y.b ∧ tail(a.a)? tail(a.b)∧ backarc.a ? backarc.b
...
function adjust(vertnodesize:int, minx:int, layerwidths:seq.int, p:nodeinfo.T)nodeinfo.T 
   assert true report"layer width"+ @(+, toword,"", layerwidths)
    let interlayerspace = 500 
     let y = if y.p = 1 then 0 else @(+, identity, interlayerspace *(y.p - 1), subseq(layerwidths, 1, y.p - 1))
