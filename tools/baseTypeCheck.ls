...
if i > 0 then addlocals(typemap(localno, i#para) ∪ localtypes, para, localno - 1, i - 1)
else localtypes
...
if isconst.sym.s2 ∨ name.sym.s2 ∈ "type]" ∨ isAbstract.module.sym.s2 then ""
 ...
 if n.codeonly = 0 then ""
  ...
  do
   if skip then next(stk, localtypes, false)
   ...
   else if isstart.s then next(push(stk, if isseq.resulttype.s then typeptr else resulttype.s), localtypes, false)
   else if isbr.s then
    assert top.stk = typeboolean report
     "if problem^(for a = "", e ∈ top(stk, 1) do a + %.e,
     a)",
    next(pop.stk, localtypes, false)
    ...
    let parakinds =
     for acc = empty:seq.mytype, @e ∈ paratypes.s
     do acc + coretype(@e, typedict),
     acc
...
do
 ...
 ∨ isBuiltin.h ∧ wordname.h ∈ "createthreadZ" ∧ paratypes.h = [typeptr] then acc10
 else acc10 + %.h + "/br",
"CheckResult:^(if isempty.acc10 then "OK"
else
 acc10
  + "/p end checkresults
 /p"
