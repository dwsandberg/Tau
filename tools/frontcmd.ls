...
{ENTRYPOINT The /strong front is a multiple purpose command. It outputs data from various stages
 of the compiler. 
...
{ENTRYPOINT The /strong transform cmd takes a list of input source files. For each module in the
 ...
 clause are removed.
 /br • /strong html. An html file is produced with an index of modules. This option is useful
 ...
 ... 
 /p The /keyword transform command treats then function genEnum as magic and will generate code in a
 module for enumeration types instead of creating the code by hand. /< a href ="./install4.
 html" > Example /< /a>}
let m =
...
{ENTRYPOINT The /strong unusedsymbols cmd analyzes code for unused functions. It forms the
 function call graph for the program. 
