...
{COMMAND The /strong front is a multiple purpose command. It outputs data from various stages of the compiler. 
/p One use is to figure out what functions are used between modules. The usegraph of the core functions indicates there are dependences between the modules texio, file and bits. To see the dependences use <* block front /sp+built core.libsrc mods: textio file bits format *> A graph will be display with the dependences between the modules. The nodes in the graph are the procedure names. Since a name does not uniquely identify a function hovering over the beginning of the name will pop up a more complete discription beginning with the name of the function. 
/p The dependence on the module bits will not be displayed. If an earilier pass of the compiler is specified like this <* block front  +built taubc.libsrc mods = textio file bits format pass = text *> then it will be displayed. 
/p The dependence with in the module textio can be seen with <* block front  +built taubc.libsrc mods = textio pass = text within = *>
/p To see all the functions that call functions named /em breakparagraph in the library use <* block front  +built taubc.libsrc n = breakparagraph pass = text out = calledby *>
/p This will list the function definitions in a package <* block front  +built taubc.libsrc mods = textio out = symdef *> The format is the function followed by a post order transversal of the call tree. 
...
/br ○ /strong pass The option pass determines how much processing is done before looking at the symbols. <* block • /strong library Only report on functions imported from libraries. 
...
/br ○ /strong % The option % determines the format of the output. <* block • /strong sym Just the symbol names
/br • /strong symdef The symbol definitions. The format is the symbol followed by a post order transversal of the call tree.
/br • /strong symdefgraph For each symbol definition, the definition is presented as a call tree graph.
/br • /strong calledby The option n is ignored in building a call graph. Then only the symbols that • call symbols in n directly or indirectly are included in the graph
/br • /strong calls The option n is ignored in building a call graph. Then only the symbols that • are called (directly or indirectly) from symbols in n are included in the graph.
...
do
 ...
 ∨ name.module.ss ∈ mods ∧ (ignorenames ∨ name.ss ∈ names) ∧ name.ss ∉ ~n ∧ name.module.ss ∉ ~mods then next(selected + sd, if name.ss ∈ names then root + ss else root)
...
let txt =
 if out = "sym" then
  ...
  let syms =
   for acc = empty:set.symbol, sd5 ∈ selected
   do acc + sym.sd5,
   acc
  let g =
  ...
  if out = "text" then
   for txt = "txt", a ∈ toseq.arcs.g
   do txt + "/br" + %.tail.a + %.head.a,
   txt
...
{COMMAND The /strong transform cmd takes a list of input source files. For each module in the input a pretty printed file is created in the directory <Tau>/tmp Addition parameters allows for different variants. <* block transform+tests profileExample.ls helloworld.ls
/br transform+tests profileExample.ls reorguse:
/br transform  +built profileExample.libsrc	 taubc.libinfo bind:
/br transform  +built profileExample.libsrc	 taubc.libinfo bind: reorguse: *>
/p This first variant does not require the source to be semantically correct but the syntax must be correct. It does not change the order of the paragraphs. 
...
{COMMAND /strong unusedsymbols cmd analyzes code looking for unused functions. 
/p It forms the function call graph for the program. 
/br It then looks for any any sources in the call graph that are not the entry point of the program and lists them. Any functions that are generated from type definitions are also removed. 
/p Here is an example <* block tau tools unusedsymbols /sp+built tools.libsrc taubc.libinfo *>
...
/br-/strong excessExports list symbols exported from a module but only used internally to that module.
/br-/strong ignore Symbols with these names are not listed in output. Default value is genEnum genPEG}
