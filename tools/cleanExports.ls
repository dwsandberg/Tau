...
if nopara.s = 1 then [merge.%.1#paratypes.s] + (if name.s ∈ "type" then "a" else "b")
else if name.s = 1#%.resulttype.s then [merge.%.resulttype.s] + "c"
else "zzzz d"
...
for tt2 = "", l ∈ alphasort.tt
do tt2 + "/p" + pretty(l << (findindex(l, 1#"Export") - 1)),
tt2
   ...
   do
    if name.modname.m1 ≠ newname then acc
     ...
     do
      ...
      if isempty.t ∨ paragraphno.1#t = 0 ∨ name.module.sym ≠ newname then acc2 + sym else acc2,
     acc2,
   acc,
...
if isempty.acc ∨ 1#acc ∉ "(" then if subseq(s, 2, 3) = "type:" then getExportName.s << 2 else ""
else acc << 1 >> 1 