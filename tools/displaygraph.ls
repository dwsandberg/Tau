...
function restoredirection(orgarc:set.arcinfo.T, org:graph.T, modified:graph.T, a:arc.T)seq.arcinfo.T 
   let other = if head.a in nodes.org ∧ not(tail.a in nodes.org)then expandback(org, modified, tail.a)else tail.a 
   let for = findelement(arcinfo.arc(other, head.a), orgarc)
   let back = findelement(arcinfo.arc(head.a, other), orgarc)
   let both =(if not(isempty.for)then [ setarc(for_1, a, false)]else empty:seq.arcinfo.T)+(if not(isempty.back)then [ setarc(back_1, a, true)]else empty:seq.arcinfo.T)
    if isempty.both then [ arcinfo.a]else both

Function restorearcs(orgarc:set.arcinfo.T, org:graph.T, modified:graph.T)seq.arcinfo.T @(+, restoredirection(orgarc, org, modified), empty:seq.arcinfo.T, toseq(arcs.modified))
