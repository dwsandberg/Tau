...
function toarcinfo(measure:seq.word, max:int, map:nodemap, a:parc)arcinfo.seq.word 
 let label = toword((if measure ="time"
   then clocks.a 
   else if measure ="count"then counts.a else space.a)* 100 / max)
...
Function profileresults(measure:seq.word)seq.word 
 // Returns label graph of profile results. Measure is time, count, or space. // 
  ...
  let m = if measure ="time"
   ...
   else assert measure ="space"report"unknown profile measure"
...
function removesmallcount(limit:int, g:labeledgraph.parc, node:parc)labeledgraph.parc 
 if cardinality.arcstosuccessors(g, node)= 0 
  then if @(+, counts, 0, toseq.backarcstopredecessors(g, node))< limit 
   then assert false report [ head.node]
    deletenode(g, node)
   else g 
  else g
...
function shorten(a:seq.seq.seq.word, i:int)seq.word 
 let j = max(differ(a_(i - 1), a_i, 1), differ(a_i, a_(i + 1), 1))
  if j = 1 
  then a_i_1 
  else let z = @(+, formattype, empty:seq.seq.word, subseq(a_i, 1, j))
...
function +(g:labeledgraph.parc, p:profileresult)labeledgraph.parc 
 @(+, toarc.p, g, arithseq(length.counts.p, 1, 1))
