...
do
 ...
 do acc3 + ("/p" + (header.e) sub 2 + name.fn.f + escapeformat + header.e << 2 + escapeformat),
 acc3
for info = empty:seq.helpinfo, e ∈ sort>alpha.acc
do info + helpinfo(e sub 2, e sub 3, e << 3),
if doc then
 ...
 do
  txt
   + "helpinfo (:(dq.[cmd.e]) sub 1,:(dq.[exe.e]) sub 1,:(dq.escapedq.txt.e)),
  /br"
 let newdata = txt >> 2 + "]",
 ...
 do
  if txt2 = "" ∧ subseq(p, 1, 2) = "# File" then txt2
   ...
   else if p sub 1 ∈ "function Function" then pretty.p + "/p"
   else p + "/p",
 txt2
...
do
 ...
 /br}",
"# functions are used instead of alias because alias adds extra arguments
/br:(acc)"
...
if level = "full" then
 ...
 let a = cmddesc.getHeaders(["Function:(cmd.d):(txt.d)"], false) sub 1,
 if level = "summary" then
  ...
  do
   if l ∈ ["/br", ""] then next(acc, opt, nest)
    ...
    let newopt = if nest = 0 then [l sub 3] else opt
    let br = if isempty.acc ∨ acc sub n.acc ∈ "block *>" then "" else "/br",
    let add = if isempty.option ∨ newopt = option then br + "/strong" + l << (2 - nest) else "",
    next(acc + add, newopt, nest),
  header.d + acc
 else "/strong:(cmd.d):(break(a, "/br", true) sub 2 << 3)"
...
[helpinfo (" PEGdebug" sub 1," tools" sub 1," (input:seq.file, steps:seq.word, notable:boolean) seq.word {COMMAND /strong PEGdebug displays trace of running a PEG. /br-/strong input Expected first paragraph of input to be input and second paragraph to be the grammar./br-/strong steps from to. Only display steps between from and to./br-/strong notable. Do not display parse table or grammar.}"),
helpinfo (" buildhelp" sub 1," tools" sub 1," (input:seq.file, target:seq.file, shellscript:boolean, doc:boolean) seq.word {COMMAND /strong buildhelp update helpdata in module. /br /strong input files from which to extract the help data./br /strong target file to update help data in /br /strong shellscript produces shell script to define commands./br /strong doc format for command documentation source file.}"),
...
helpinfo (" makebitcode" sub 1," taubc" sub 1," (input:seq.file, exports:seq.word, info:boolean, profile:boolean, showllvm:seq.word, entryUses:seq.word, output:seq.word) seq.file {OPTION COMMAND /strong makebitcode compiler /br Options:/br /strong entryUses addition use clause added to module when building entry point. /br /strong profile generates information for profiling /br /strong showllvm show text form of llvm code generated for the names of procedure given./br /info show text form of code info /br /exports list of packes to show. }"),
