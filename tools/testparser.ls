...
Function testparser seq.word 
 // runs the parser on a sample string. // parse."1 + 2 + 3"+"OK"
...
function dict(result:int)int 
 // dict is not used in this example. In more complicated example the result fld of the stkele would be a record // 
  0
...
function consumeinput(b:stepresult, next:word)stepresult 
 let stk = stk.b 
  ...
  if actioncode > 0 
  then stepresult(push(stk, stkele(actioncode, 0)), place.b + 1, if track then track.b +"&br next"+ next + printstate.actioncode else track.b, 0,"")
  ...
  consumeinput(stepresult(x, place.b, if track 
   then track.b +"&br reduce by"+ toword(-actioncode)+ printstate.stateno.top.x 
   else track.b, tokenstate.b, string.b), next)
...
function reduce(stk:stack.stkele, ruleno:int, place:int, input:seq.word)stack.stkele 
 // generated function // 
  ...
  let newtree = if ruleno = // G F # // 1 
   ...
   then 3 
   else assert ruleno = // E E + E // 6 report"invalid rule number"+ toword.ruleno 
