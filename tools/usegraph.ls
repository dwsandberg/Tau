...
{OPTION PROFILE COMMAND /strong usegraph graphs /em use releationship between modules in source code. 
/br
/br options: 
...
/br /strong order finds ordering of modules so for any arc in graph the tail occurs befor the head. <* block • /strong nograph Does not include graph in output. *>
 ...
 if n.s = 4 then moduleref("X" + module, bt)
  ...
  do
   ...
   if e ∈ "." then next(newtype, left >> 1)
    ...
    let abstracttype = if isempty.found then typeref.[e, "unknown" sub 1, "X" sub 1] else found sub 1,
...
for allmodules = empty:seq.passsymbols, alltypes = empty:set.symbol, getuses ∈ [false, true]
do
 for
 ...
 for alltypes1 = empty:set.symbol, m ∈ result1
 do alltypes1 ∪ defines.m,
