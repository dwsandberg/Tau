...
{OPTION PROFILE}
let exporttypes =
 for acc = empty:seq.mytype, sym ∈ exports
 do acc + types.sym + resulttype.sym,
 acc
let usestypes =
...
if isSimple.modr then exports.mr
 ...
 for acc = empty:seq.symbol, sym ∈ exports.mr
 do acc + replaceTsymbol(para.modr, sym),
 acc
...
let in =
 ...
 for new = unhandled, t ∈ toseq.acc
 do new + inModule(exported, deepcopySym.t),
 new
...
∨ subseq(modtext, i + 1, i + 1) ∈ ["Export", "type"] then i
