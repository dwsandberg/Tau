...
let tab = maketable(gin, "dq:(dq), // /, //action /action, //br /br", true)
...
else "Result:/p:(result.r)"
)/p:(trace.r)/table:(if notable then "" else "/p:(tab)/p:(%(3, gin))")"
...
"Key to column labels S--step no; D--Depth of Stack F--on fail reset I to F; I--Location in input; Lower case are values on top of stack /p S /td D /td f /td success /td fail /td result /td Result /td F /td State /td I /td Remaining input /td /tr:(trace)"
...
do
 ...
 if actionState = Fail then
  ...
  let top = top.stk
  let newrinfo =
   if i > place.rinfo then recoverInfo(stk, myinput, i, faili, trace) else rinfo,
