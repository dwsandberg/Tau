...
Function printBitCodes(file:seq.word)seq.word 
 // This takes a bitcode file generated by the compiler and produces a fragment of a function for recreating the bit code file. llvm tools will also decode a bitcode file // 
  ...
  {"function rebuild seq.bits &br let NumEle = 1 &br let TYPES = ["+ printtypes.types +"]let discard = @(+, typerecord, empty:seq.encoding.llvmtype, subseq(TYPES, 2, length.TYPES))&br let DEFLIST ="+ @(seperator."+", identity,"", subseq(constanddefs, 1, firstconst - 1))+"&br let discard2 ="+ @(seperator."+", identity,"", subseq(constanddefs, firstconst, length.constanddefs))+"&br let bodies = ["+ printfuncs(length.constanddefs, functiondefs, functionblocks, 1,"")+"]&br llvm(DEFLIST, bodies, subseq(TYPES, 2, length.TYPES))&br &br function Cdef(l:int, name:seq.word, rec:seq.int)seq.seq.int let discard = C.merge(name)[ rec]&br &br function Cdef(l:int, type:int, rec:seq.int)seq.seq.int let discard = Cprt(type, rec)empty:seq.seq.int"}
