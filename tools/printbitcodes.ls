...
Function printBitCodes(file:seq.word) seq.word
 ...
 "let TYPES=["+printtypes.types+"]"
 +"&br let DEFLIST=["+label("&br",symlist,@(+,      print.MODULEBLOCK,empty:seq.seq.word,deflist),1,"")+"]"
 +"&br let CONSTANTS=["+
    @(seperator.",",identity,"",constlist)+"]"
 +"&br  let bodies= ["
   + printfuncs(length.deflist+length.constlist,types,deflist,symlist,1,findblock(FUNCTIONBLOCK,blocks.z),1,"")
    +"]"
 ...
 function printfuncs( offset:int,types:seq.seq.int,deflist: seq.seq.int,symlist:seq.seq.word,i:int,defs:seq.block,j:int,result:seq.word) seq.word
  if i > length.deflist then result + if j < length.defs then "&br+extra bodies" else ""
  else if deflist_i_1=MODULECODEFUNCTION &and deflist_i_4=0 then 
      let nopara= length.(types_((deflist_i_2)+2))-3
      let a =  (if length.result = 0 then "" else ",")+"&br //" + symlist_i + "nopara:"+toword.nopara + "//"+
             if j > length.defs    then " missing body " else 
           //    print2(defs_j) //
                 yy(recs.getinfo(defs_j),1,offset+nopara+1,"",offset)
        printfuncs(offset,types,deflist,symlist,i+1,defs,j+1,result+a)
   else 
      printfuncs(offset,types,deflist,symlist,i+1,defs,j,result)
 ...
 function label(prefix:seq.word, labels:seq.seq.word,items:seq.seq.word,i:int,result:seq.word) seq.word
   if i > length.items then result
   else 
     let a = if i > length.labels then prefix+ items_i 
   else  
     { if i > 1 then "," else "" } +prefix+"//"+ labels_i +"//" + items_i
...
function printconstant(s:seq.seq.int,lasttype:int,number:int,i:int,result:seq.seq.word) seq.seq.word
 if i > length.s then result else
  let a = s_i
  if a_1=CONSTSETTYPE then
     printconstant(s,a_2,number,i+1,result)
   else   
    let b="&br //"+toword(number)+"// C("+toword.lasttype+","+print(CONSTANTSBLOCK,a)+")" 
    printconstant(s,lasttype,number+1,i+1,result+b)
 
 function print(blockid:int,a:seq.int) seq.word 
 if blockid=VALUESYMTABBLOCK then
        [toword.(a_2), encodeword(subseq(a,3,length.a))]
      else 
  @(xx(blockid),identity,"",a)+"]" 
 
 function xx(blockid:int,a:seq.word,v:int)  seq.word
   a+ 
     if length.a = 0 then 
       { if blockid in [TYPEBLOCK,MODULEBLOCK,CONSTANTSBLOCK] then "[" else 
     "&br[" } +lookup(blockid,v)
    else ","+toword.v

function print( a:seq.bit) seq.word @(+,asbyte.a,"",arithseq(length.a / 8,  8,1))

 function  asbyte( a:seq.bit, i:int) seq.word
     [toword.toint.@(  formval(a),identity,bits.0,  arithseq(8,-1,i+7) ) ]  

 function   formval(a:seq.bit,val:bits,i:int)  bits       val << 1 &or bits.toint(a_i) 
...
function getvbr(a:seq.bit, idx:int,size:int) pp getvbr(a,size,bits.0,0,idx,0)
 ...
 if i = size - 1 
  ...
  else getvbr(a, size,  bits(b) << nobits &or val, nobits+1, idx, i + 1)
...
function getinfo(b:seq.bit,noargs:int,r:seq.int,idx:int,recs:seq.seq.int,blocks:seq.block,abbrvlen:int,blockid:int) content
    if  length.r > 0 then // working on record //
       if noargs = 0 then 
         getinfo(b,0,empty:seq.int,idx,recs+r,blocks,abbrvlen,blockid)
       else 
...
function yy( s:seq.seq.int,i:int,slot:int,result:seq.word,offset:int) seq.word
  ...
  let tp=d_1
   ...
   then [ false, false, false]+ constantseq(length.d - 3, true)
        ...
        if length.d < 8 then  [false,false,true,false,true,false,true,false,true]  else
            [false]+big(length.d / 2)
  ...
  let slotinc=    { if tp in [ INSTLOAD, INSTALLOCA, INSTCALL, INSTGEP, INSTCAST, INSTCMP2, INSTBINOP, INSTPHI]
   then 1 else 0 }
  yy(s,i+1,slot+slotinc, result+ { if length.result=0 then "&br " else "+ &br " } +lookup(FUNCTIONBLOCK,d_1)+"(" +toword(slot-offset)
  +yy(relocmask,d,length.d,offset,slot)+")",offset)
...
function yy(relocmask:seq.boolean,s:seq.int,i:int,offset:int,slot:int) seq.word
     { if i < 3 then "" else yy(relocmask,s,i-1,offset,slot) } + ","  + if  relocmask_i then 
       let arg = s_i 
...
Function stats(file:seq.word)seq.word 
 ...
 let constants=findblock(CONSTANTSBLOCK,blocks.z)_1
 @(+,      print.CONSTANTSBLOCK,"",codefreq(0,recs.getinfo.constants))
+ @(+,      print.FUNCTIONBLOCK,"",codefreq(0,insts))
