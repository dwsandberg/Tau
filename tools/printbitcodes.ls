      ...
      assert blockid.(blocks.z)_1 = TYPEBLOCK report"Expected TYPE Table as first block"
       assert blockid.symbols = VALUESYMTABBLOCK report"EXPECTED SYMBOL Table as last block"
         let types = recs.getinfo.(blocks.z)_1 
          ...
          assert length.functionblocks = length.functiondefs 
          report"number of function declarations does not match number of bodies"
           "function rebuild seq.bits &br let NumEle = 1 &br let TYPES = ["+ printtypes.types +"]let discard = @(+, typerecord, empty:seq.encoding.llvmtype, subseq(TYPES, 2, length.TYPES))&br let 
 DEFLIST ="
           ...
           +"]&br llvm(DEFLIST, bodies, subseq(TYPES, 2, length.TYPES))&br &br function Cdef(l:int, name:seq 
.word, rec:seq.int)seq.seq.int let discard = C.merge(name)[ rec]&br &br function Cdef(l:int, type:int, rec 
:seq.int)seq.seq.int let discard = Cprt(type, rec)empty:seq.seq.int"
