...
{ENTRYPOINT /strong usegraph creates a graph with each node being a module and the arcs to the
 module referenced in the /keyword use clauses. 
 ...
 /br > usegraph /sp+built core.libsrc exclude = seq standard /< a href ="./install2.html"
 > Result /< /a>
 /br > usegraph /sp+built core.libsrc include = UTF8 words standard textio exclude = seq
 standard /< a href ="./install3.html" > Result /< /a>
 /br > usegraph /sp+core UTF8.ls textio words standard encoding xxhash exclude = seq standard *>
 /p The last two examples give the same result. The first excludes modules from consideration and
 the second only includes source files of modules that should be included. }
...
{OPTION PROFILE ENTRYPOINT /strong doclibrary create summary documentation for library source code
 . 
 /br The option /strong mods list the modules to be document if the option is not empty. 
    ...
    next(
     ...
     + 
      ...
      ""
       ...
       ^(for acc2 = "", @e ∈ usedin do acc2 + tail.@e, alphasort.acc2)"
     , subseq(p, 2, n.p)
     , ""
     , ""
    )
