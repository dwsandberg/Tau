...
let d = for acc ="", @e = gettext."tools/doc.txt"do acc + addselect.@e end(acc)
...
Function callgraphbetween(libname:seq.word, modulelist:seq.word)seq.word
 ...
 let a = for acc = empty:seq.mytype, @e = modulelist do acc + mytype.@e end(acc)
 let arcs = for acc = empty:seq.arc.word, @e = z do acc + modarc(a, @e)end(acc)
  display.for acc = empty:seq.arcinfo.seq.word, @e = toseq.arcs.newgraph.arcs do acc + toarcinfo.@e end(acc)
...
Function callgraphwithin(libname:seq.word, modulelist:seq.word)seq.word
 ...
 let nodestoinclude = for acc = empty:set.word, @e = toseq.nodes.g do acc ∪ filterx(modulelist, @e)end(acc)
 let g2 = for acc = g, @e = toseq.nodestoinclude do deletenode(acc, @e)end(acc)
  display.for acc = empty:seq.arcinfo.seq.word, @e = toseq.arcs.g2 do acc + toarcinfo.@e end(acc)
...
function readable(fsig:word)seq.word
 let p = codedown.fsig
  if length.p = 1 then p_1
   ...
   let plist = for acc = empty:seq.mytype, @e = subseq(p, 3, length.p)do acc + mytype.@e end(acc)
    p_1 + ":" + print.mytype.p_2 + "("
    + for acc ="", @e = plist do list(acc,",", print.@e)end(acc)
    + ")"

Function usegraph(g:graph.word, include:seq.word, exclude:seq.word)seq.word
 let g1 = exclude + for acc ="", @e = exclude do acc + addabstractpara.@e end(acc)
 let g2 = for acc = g, @e = g1 do deletenode(acc, @e)end(acc)
 let g3 = if include = ""then g2
 else
  let a = include + for acc ="", @e = include do acc + addabstractpara.@e end(acc)
  let b = for acc = empty:set.arc.word, @e = a do acc ∪ arcstosuccessors(g2, @e)end(acc)
   newgraph.toseq.b
 let d = for acc = empty:seq.arcinfo.seq.word, @e = toseq.arcs.g3 do acc + toarcinfo.@e end(acc)
  display.d
...
Function doclibrary(libname:seq.word)seq.word
 ...
 let r = for acc ="", @e = liba do acc + findrestrict.@e end(acc)
 ...
 let modname = lib_i_2
   ...
   docmodule(usegraph, exports, todoc, lib, i + 1,"", funcs, types)
    ...
    let name = [ modname] + if length.lib_i > 2 then".T"else""
     ...
     + name
     + "is used in modules: "
     + alphasort.for acc ="", @e = toseq.arcstopredecessors(usegraph, merge.name)do acc + tail.@e end(acc)
...
Function uncalledfunctions(libname:seq.word)seq.word
 ...
 let g = newgraph.formcallgraph(compile("pass1", libname), 2)
 let sources = for acc ="", @e = toseq.nodes.g do acc + sources(g, empty:set.word, @e)end(acc)
  for acc ="", @e = alphasort.sources do list(acc," &br", readable.@e)end(acc)
...
function callarcs(s:seq.word, i:int, result:seq.word)seq.word
 if i + 1 > length.s then result
  ...
  let this = s_i
   ...
   callarcs(s, findindex2("'"_1, s, i + 1) + 1, result)
   else if this="BLOCK"_1 &and  s_(i+1) &ne "("_1 then
    ...
    let next = s_(i + 1)
     ...
     let j = findindex2(")"_1, s, i + 1)
      ...
      let chs = decodeword.this
       ...
       report"call arcs problem" + this + toword.i + "full text" + s
