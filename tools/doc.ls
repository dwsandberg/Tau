...
function filterx(include:seq.word, w:word)set.word 
   let p = codedown.w 
    if length.p < 2 then empty:set.word else if last(p_2)in include then empty:set.word else asset.[ w]
...
* If a paragraph in the library module if of the form:* only document <module1 name> <module2 name>... then only those modules 
 named will be documented.

* If a paragraph in the library is of the form:* usegraph exclude <list of modules> include <list of modules> then a use graph will be 
 construction including and excluding the modules listed. Both the exclude and include are optional, but for a large library should 
 be used to restrict the size of the graph. An example of a use graph is included at the end of this module.

function findrestrict(s:seq.word)seq.word if subseq(s, 1, 3)="skip * only document"then subseq(s, 4, length.s)else""
...
function docmodule(usegraph:graph.word, exports:seq.word, todoc:seq.word, lib:seq.seq.word, i:int, currentmod:seq.word, funcs:seq.word, types:seq.word)seq.word 
   if i > length.lib 
   then if length.types > 0 ∨ length.funcs > 0 then"&br defines types: "+ types + funcs else""
    ...
    let modname = lib_i_2 
     if not(modname in todoc ∨ length.todoc = 0)
      then docmodule(usegraph, exports, todoc, lib, i + 1,"", funcs, types)
       ...
       let leftover = if length.types > 0 ∨ length.funcs > 0 then"&br defines types: "+ types + funcs else""
       let name = [ modname]+(if length(lib_i)> 2 then".T"else"")
        leftover +"&{ select x &section &keyword module"+ name +"&}"+(if modname in exports then"Module"+ name +"is exported from library. "else"")
        +"&br Module"
        + name 
        +"is used in modules: "
     ...
     if key ="module"_1 
      ...
      then 
       let m = 
        if length(lib_i)= 2 
        then lib_i_2 
        else if kind ="mod"_1 then merge([ lib_i_2]+".T")else merge.subseq(lib_i, 2, length(lib_i))
        if currentmod = m 
   ...
   then result 
    ...
    let name = src_i 
     if name in"IDXUC CALLIDX STATE PROCESS2 FREF EQL if VERYSIMPLE"
      ...
      else if name = func then formcallgraph(func, src, i + 1, result)else formcallgraph(func, src, i + 1, result + arc(func, name))
