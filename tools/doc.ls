...
Tau is word based rather than character based. The follow PEG grammar descibes
how a seqence of bytes is converted into words.   
We will use /em sp /em dq /em cr and /em lf  to represent the ASCII space, double qoute, carrage return, and line feed.
<* table
/row * Words /cell ← whitespace  / StandAlone / Word
/row StandAlone /cell ← .sp /. /:sp/: /+/-/ =/^/_/,/ (/)/ dq / { /} / [ /]
/row WhiteSpace /cell ← sp / cr  / lf 
/row * Word /cell ← ! WhiteSpace ! StandAlone any 
*>
Whitespace words are discared. StandAlone words do not need white space before or after
them.  Note that a period followed by a space is different than a period not followed by a space. 
Colons are treated the same. 

A source file for Tau is a UTF-8 encoded file.  View  blank lines in the file as breaking the
file into a sequence of paragraphs.   Each paragraph can then be broken into words as above. 
The paragraphs are then grouped into /em modules. Here is a PEG grammar describing the structure
...
Export ∧(boolean,boolean) boolean {The second operatand   is evaluated if and only if the first operatand does not determine the result}
...
Export %(n:int) seq.word  { By convention function that begin with a %  have a return type of seq.word, supply a human readable representation.}

Export %(w:word) seq.word 

<* section Expressions *>

 We would like an expression such as   a + 3 * b ^ 2 ^ -2  to evaluation to what a mathematician
 woule expect where /sp ^ /sp is the exponent operator.  Adding 
 parenthesis specifies the order of evaluation:   a + (3 * (b ^ ( 2 ^ (-2))). 
 The binary operators /sp+/sp and * are left associative and /sp ^ /sp is right associative.
 
The following is a PEG grammar for simple expressions. The order of evaluation is
obsccured by this grammar since left recursion must be elimiated and 
The left associative of + and * are taken care of in further processing, but the
 grammar takes care of the right associative nature of ^.  
<* table 
E ← Sum 
...
Tau has many more binary operators than the above grammar handles.  Here is then
full list.  Operators on the same line are of the same precedence. 
/br _^
/br unary minus
/br * / mod ∪ ∩ \
/br  +-∈ ∉
...
Note that the operators ∪ ∩ \ which are commonly used for set union, intersection and differences all have that same precedence.

For easy entry,  the operators ≤ ≥ ≠ ∧ ∨ ⊻ ∈ ∉ ∩ ∪ have ASCII equivalents 
...
A function call with a single argument can be expressed one of two ways: f1.a or f1 /nosp(a).  This allows expressions to
be written with fewer parentheses.  Note how the order of evaluation differs in the following expression:
...
function quadratic(a:int,b:int,c:int,x:int) int
       a+ b * x + c * x ^ 2  
              
Function quadratic(a:real,b:real,c:real,x:real) real
       a+ b * x + c * x ^ 2  
...
If two function differ in name, number of parameters, or  types of the parmeters
they are consider distinct functions.  Capatilizing the first letter of Function implies the function
is export from the module.  If the first letter is lower case the function is not exported 
unless an Export clause for the function is include like

Export quadratic(a:int,b:int,c:int,x:int) int

Here is the syntax for function definitions. It includes a non-termial,  Declare, which we
define later.

 <* table
S /cell ← function Name (FPL) Type Declare' E 
...
A expression can always be viewed as a series of function calls.  For the first
function defined above the values of the parameters can be viewed as 
the functions with no parameters with the function name being the parameter name
and the function result type being the parameter type.
...
Any expression has excatly one type that can be determine from the
subexpression using the return type of the function called. 

When calling a function the types of action arguments MUST match the type given in 
the definition of the function.  

When defining a function the type of the expression must match
the return type of the function. 

<* section Literals *>
 
Below is a grammar than shows how characters are combined to form three types of 
literals with the names /em int, /em bits /and /em real.  Examples of int are
0 1 1000 1234.  Exampes of bits are 0x1 0x0000 0X0a 0X0B.  Examples of real are
0.0 1.0 1000.0 and 1234.456. 
...
String in Tau are defined as a sequence of words(seq.word).  An example literal for
a sequence of words is " this is a sample string of 8 words"
A simple grammar for
strings would be
<* block 
/br String ← "  Str2 "
/br * Str2 ← ! "   any
*>
But Tau uses allows a string to include an  expression of type /em  seq.word. 
 The example literal above could also be represented as
 "this is a ^("sample string") of 8 words"
...
There is two function named /em dq in the stardard package.
The first that takes no arguments and returns
the sequence of words with one word consisting of single  doublequote
character. The second function takes a sequence of words as an argument
and returns the args with a doublequote added to the begining and end. 
...
These function allow  a double quote within a string use.  For example:
  ...
  is the same as 
  writing 
  <* block  /ldq  hello world without quotes;"/sp+/sp dq /sp+/sp /ldq hello world"  +/sp dq /sp+/sp"/nosp   with quotes"   *>
...
let b= 444
for  sum=0,product=1 , e /in "1 2 3 4 mark 5 6" while e /nin "mark"
do 
next(sum+toint.e,product * {comment within expression}  toint.e)
assert product=24 /and b = 444 report "Problem occured. Expect 24 but got ^(product)" 
sum+product

Here is the grammar for constructs used in this section: 
<* table
...
A /keyword let declaration in a function allows an name to be given to the value of an expression so the name references the value in the following expression. A let statement DOES NOT define a variable that can change values.

Comments are enclossed in curly brackets:{} and can be nested. 
...
Comments can occur as a declaration or as a unary operator within expressions.   

The /keyword assert construct evaluates the first expression which must be of type boolean.  If the first expression is false
the second expression is evaluated and must be of type seq.word. The 
value of the second exporess is thrown and evaluation of the function   stops.
If the first expression evaluates to true execution continues as if the
assert was omitted from the function.

In the /keyword for construct in exampleDeclaration sum and product are accumlators. 
Each accummulator is given an initial value. For each value e in the sequence, 
new values are calculated for each accumlator by the pseudo   
function next which also advances to the next value in the sequence if they are any.  
If a while expression is present it's type must be boolean, and is evaluated
just after e is assign a value.  If it is false the evaluation of the
for construct is stopped.  
...
In the exampleDeclaration the identifiers sum product and e will take on the values 
 <* table 
...
If only one accumulation is declared then the next function can be dropped,
for example:

function sum(s:seq.int) int  for   accumulator=0 ,e ∈ s    do accumulator+e    , accumulator  


The optional comma in declarations can usually be left out, but there are exceptions. 
In the following function the comma cannot be left out or a  error will occur as 
...
does not distinguish between opening and closing quote marks, / /nosp ldq  inserts a double quote
suppressing the space after it.  Adding  / /nosp sp will force a space after a word and / /nosp nosp will suppress the
...
Function testdoc(input:seq.file,o:seq.word) seq.file {ENTRYPOINT}
...
Now a set of entries will be a dictionary.   From the mathematical view of the set each entry is an representation of an integer.  The integer 3 could be represented by
 myentry(3, /ldq X") , myentry (3, /ldq A B C") or and infinite number of other possibilities.  But only one representation will be used in any listset.myentry.
...
Making the function tail recursive is not the only way to reduce the stack size. The follow version uses O(ln n) instead of O(n)where n is the length of the sequence.
 ...
 if n.l < 2  then l   else reverse4.subseq(l, n.l / 2 + 1, n.l)+ reverse4.subseq(l, 1, n.l / 2)

Perhaps the best way to reverse a sequence is to use

  Function reverse5(s:seq.int)seq.int    
  for acc=empty:seq.int,e ∈ s   do [e]+acc , acc  
...
/cell   Indented block 
/row  /escape /nosp format   <hr/> /escape /nosp format
/cell  /escapeformat <hr> /escapeformat  /cell 
/row  < /nosp *    table <caption> <rows of table>      * /nosp >
/cell /cell Defines a table 
/row  /keyword /row../keyword /cell../keyword /cell..  /cell /cell Defines a row  with three columns. 
