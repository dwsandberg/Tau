...
let d= for acc ="", @e = prettyfile(false,"",gettext."tools/doc.txt") do acc + addselect.@e /for(acc)
...
function addselect(s:seq.word)seq.word 
 ...
 else if  first.s = first."/section"then
 " /< /section" + s << 1 + " />"
 else if first.s /in "Module module" then " /p  /keyword"+s
 else" /p" + s
...
Function doclibrary(libname:seq.word)seq.word
 ...
 let todoc = for acc ="", s = liba do 
   if subseq(s, 1, 3) = "* only document"then acc+subseq(s, 4, length.s) 
   else acc
  /for(if isempty.acc then exports else acc)
 ...
 txt+'  /< noformat <a href ="' + merge.["#"_1, modname] + '"> ' + modname
 + "</a>  />" /for(txt)

 
  + if length.todoc > 0 then""
...
* If a paragraph in the library module is of the form:* only document <module1 name> <module2 name>... then only those modules named will be documented.
