...
let bodypart =
 if n.pairs > 4 ∧ (highValue - lowValue < 40 ∨ 100 * n.pairs / (highValue - lowValue + 1) > 95) then
  ...
  let tmp = 1 - lowValue,
  let tmp1 = if tmp = 0 then "i" else if tmp > 0 then "(i+:(tmp))" else "(i-:(-tmp))",
  "if between (i,:(lowValue),:(highValue)) then let r = [:(dq.codes5) sub:(tmp1)], if r ≠:(dq."?") then r else:(dq(type + "."))+toword.i else"
 else
  for txt = "", e ∈ toseq.pairs
  do txt + "if i =:(value.e) then:(dq.[name.e]) else",
  txt
let decodefunc = "Function:(decodename) (code::(type)) seq.word:(list >> 1)] let i =:(if type = "int" then "" else "toint.") code,:(bodypart):(dq(type + "."))+toword.i",
