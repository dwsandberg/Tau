...
do
 ...
 do
  ...
  else acc3 + ([p sub 2] + name.fn.f + escapeFormat(tmp << 3)),
 acc3
...
if doc then
 ...
 for
  ...
  {This function was created using buildhelp:(%%.input)target::(%%.target)}/br["
  , e ∈ info
 do
  txt
  + "helpinfo(/br:(dq.[cmd.e])sub 1 /br,:(dq.[exe.e])sub 1,:(dq.escapedq.txt.e)), /br"
 let newdata = txt >> 2 + "]",
 ...
 if last.txt2 ∈ "/p" then txt2 >> 1 else txt2
...
if % = "shellscript" then script.helpdata
 ...
 if isempty.% then "For more information on command use tauhelp cmd: <command> %: full /br:(acc)"
 else acc
...
do
 ...
 acc + "/br function:(cmd.p)/sp{/br tau:(exe.p):(cmd.p):(defaultDir)$@ /br}",
"# functions are used instead of alias because alias adds extra arguments /br:(acc)"
...
[
 ...
 , helpinfo(
  "wasm" sub 1
  , "webassembly" sub 1
  , "(input:seq.file, Library:seq.word, exports:seq.word, output:seq.word, info:boolean, profile:boolean)seq.file{COMMAND compile to webAssembly /p Library /br exports /br output /br profile /br}"
 )
] 