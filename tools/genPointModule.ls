...
let basetype = "real"
let typename = {"point2"}[typenames sub 1]
let matrixname = {"matrix2"}[typenames sub 2],
%(
 "/p"
 , [
  "Module:(typename)/p This package was generated with: genPointModule typesnames: :(typenames)elements: :(elements)"
  , "precedence > for >1 >2"
  , "use real"
  , "use standard"
  , types(basetype, typename, elements)
  , pointFunctions(basetype, typename, elements)
  , matrixFunctions(elements, basetype, typename, matrixname)
  , "function square(a::(basetype))real a * a"
 ]
)
 >> 1
...
do next(para0 + ":(x)::(basetype),", exports0 + "Export:(x)(:(typename)):(basetype)/p"),
...
let opdefs =
 [
  ...
  , "-(a::(typename), b::(typename)):(typename)term: e.a-e.b, final::(typename)"
  , "=(a::(typename), b::(typename))boolean term: e.a = e.b ∧"
  , ">1(a::(typename), b::(typename))ordering term: e.a >1 e.b ∧"
  ...
  , "point(a::(typename))seq.word term: %(3, e.a)+final: "
...
do
 ...
 let head = "Function:(subseq(ops, 1, findindex(ops, "term" sub 1) - 1))",
 for body0 = "", e ∈ elements
 do
  for acc = body0, w ∈ term do if w ∈ "e" then acc + e else acc + w,
  ":(acc)",
 ":(code):(head):(final)(:(body0 >> 1))/p",
code
 + if elements = "x y z" then "Function cross(a::(typename), b::(typename)):(typename):(typename)(y.a * z.b-z.a * y.b, z.a * x.b-x.a * z.b, x.a * y.b-y.a * x.b)/p"
else ""
...
let rotations =
 if n.elements0 = 3 then elements0 else if n.elements0 = 2 then "z" else ""
  ...
  do
   let calc =
    if kind ∈ "*s" then "a *:(col).:(row).b"
     ...
     if row = col then if row = elements sub n.elements then "1.0" else ":(row).a"
     else "0.0"
    else if kind ∈ "translate" then
     if row = col then "1.0"
     else if col = elements sub n.elements then ":(row).a"
     else "0.0"
    else if kind ∈ rotations then
     if row ∈ [elements sub n.elements, kind] ∨ col ∈ [elements sub n.elements, kind] then if row = col then "1.0" else "0.0"
   ...
   if kind ∈ "*p" ∧ col ≠ elements sub n.elements then acc2
   else acc2 + calc + if kind ∈ "=" then "∧" else ",",
  if kind ∈ "=" then acc1 + acc2 >> 1 + "∧"
  else if kind ∈ "*p" then if row = elements sub n.elements then acc1 else acc1 + ":(acc2 >> 1)/br,"
  else acc1 + "row(:(acc2 >> 1))/br,",
 all
 + "Function"
 + head << 1
 + "/br"
 + if kind ∈ "*p" then ":(typename)(:(acc1 >> 2))/p"
 else if kind ∈ "=" then ":(acc1 >> 1)/p"
 else ":(matrixname)(:(acc1 >> 2))/p",
