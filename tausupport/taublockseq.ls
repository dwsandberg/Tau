...
if length.s ≤ blksz then
 ...
 let d = for acc = set(set(newseq, 0), length.s), @e ∈ s do set2(acc, @e) /do acc,
 ...
 let discard = 
  for acc = set(set(blkseq, blockseqtype), length.s), @e ∈ arithseq(noblks, blksz, 1) do
   ...
   let d = for acc2 = set(set(newseq, 0), length.k), e ∈ k do set2(acc2, e) /do acc2
...
if length.s ≤ blksz then
 ...
 let d = for acc = set(set(newseq, 1), length.s), @e ∈ s do set2(acc, @e) /do acc,
 bitcast:seq.T(newseq)
else
 let noblks = (length.s + blksz - 1) / blksz
 let blockseqtype = getseqtype.toseq.blockseq(1, empty:seq.T)
 ...
 let discard = 
  for acc = set(set(blkseq, blockseqtype), length.s), @e ∈ arithseq(noblks, blksz, 1) do
   ...
   let d = for acc2 = set(set(newseq, 1), length.s2), e ∈ s2 do set2(acc2, e) /do acc2,
   set(acc, newseq)
  /do acc
 ,
 bitcast:seq.T(blkseq) 