...
if n.s ≤ blksz then
 ...
 if getseqtype.s = 0 then
  let discard = blockcopy(newseq, s, n.s + 2),
  bitcast:seq.T(newseq)
 else
  for acc = set(set(newseq, 0), n.s), e ∈ s do set(acc, e),
  bitcast:seq.T(newseq)
else
 ...
 do
  ...
  let d =
   for acc2 = set(set(newseq, 0), n.k), e ∈ k do set(acc2, e),
   acc2,
  set(acc, newseq),
 bitcast:seq.T(blkseq)

function blockcopy(p:ptr, b:seq.T, n:int) ptr
for acc = p, i = 0 while i < n do next(set(acc, fld:int(toptr.b, i)), i + 1),
acc

function blockcopy(p:ptr, b:T, n:int) ptr
{if n = 3 then set(set(set(p, fld:int(toptr.b, 0)), fld:int(toptr.b, 1)), fld:int(toptr.b, 2))else}
for acc = p, i = 0 while i < n do next(set(acc, fld:int(toptr.b, i)), i + 1),
acc
...
if n.s ≤ blksz then
 ...
 for acc = set(set(newseq, 1), n.s), e ∈ s do blockcopy(acc, e, ds),
 bitcast:seq.T(newseq)
else
 let noblks = (n.s + blksz - 1) / blksz
 let blockseqtype = getseqtype.toseq.blockseq(1, empty:seq.T)
 ...
 do
  ...
  let d =
   for acc2 = set(set(newseq, 1), n.s2), e ∈ s2 do blockcopy(acc, e, ds),
   acc2,
  set(acc, newseq),
 bitcast:seq.T(blkseq)
