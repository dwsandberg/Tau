...
if length.s ≤ blksz then
 ...
 let d = 
  for acc = set(set(newseq, 0), length.s), @e ∈ s do set(acc, @e) /for (acc)
 ...
 let discard = 
  ...
  do
   ...
   let d = 
    for acc2 = set(set(newseq, 0), blksz)
    , e ∈ subseq(s, @e, @e + blksz - 1)
    do set(acc2, e) /for (acc2)
...
if length.s ≤ blksz then
 ...
 let d = 
  for acc = set(set(newseq, 1), length.s), @e ∈ s do set(acc, @e) /for (acc)
 bitcast:seq.T(newseq)
else
 let noblks = (length.s + blksz - 1) / blksz
 let blockseqtype = getseqtype.toseq.blockseq(1, empty:seq.T)
 ...
 let discard = 
  ...
  do
   ...
   let d = 
    for acc2 = set(set(newseq, 1), length.s2), e ∈ s2 do set(acc2, e) /for (acc2)
   set(acc, newseq)
  /for (acc)
 bitcast:seq.T(blkseq) 