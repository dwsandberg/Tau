...
Function >1(a:labeledarc.T, b:labeledarc.T) ordering tail.a >1 tail.b ∧ head.a >1 head.b

Function >1(a:arcpath.T, b:arcpath.T) ordering
head.arc.a >1 head.arc.b
∧ {from.a >1 from.b /and} tail.arc.a >1 tail.arc.b
...
if length.grp = 1 then grp
 ...
 for acc = empty:seq.arcpath.T, new = base, q ∈ grp do
  next(acc + arcpath(arc.q, d.q >> 3 + print(3, new), 0)
  , new + inc
  )
 /for (acc)
...
"<script> function shiftstart (arcs) {let bb = document.getElementById (arcs [0]).getBBox
 ...
 /br let element = document.getElementById (idval) ; let d = $(dq."
 M")+(bb.x+bb.width)+$(dq.",")+(bb.y+bb.height)+element.getAttribute (
 $(dq."d")).substring (5) ; element.setAttribute ($(dq."d"), d) ;}}) ;} </script>
 /br <style>.arcs {fill:none ; stroke:black ; stroke-width:.07 ;}.nodes {font-size
 :.03em; stroke-width:.1 ;} svg g:hover text {opacity:1;} svg g:hover rect {opacity:
 1;} </style>"
+ encodeword.[char.10]
...
let arcpaths0 = 
 for ap = empty:set.arcpath.T, a ∈ paths.layout do
  let from = 
   if length.a = 2 then x.lookup(nodeinfo.layout, nodeinfo(a_1, 0, 0))_1
   else 0
...
let arcpaths = 
 if not.haslabels then arcpaths0
  ...
  for acc = empty:seq.arcpath.T
  , grp = empty:seq.arcpath.T
  , p ∈ toseq.arcpaths0
  do
...
do
 ...
 if i ≤ cardinality.nodes.xxx ∧ (nodes.xxx)_i = n.n then
  ...
  let svg = 
   "<text id = $(dq.[toword.id]) class = $(dq."nodes") x =
    $(dq.print(3, nodex)) y = $(dq.print(3, nodey)) > $(node2text.n.n) </text>"
   + encodeword.[char.10]
   + for arctxt = "", j = id + 1, s ∈ succ do
    ...
    let path = 
     if isempty.paths then
      "L $(print(3, toreal.y.xy * scalex))
       $(print(3, toreal.x.xy * scaley))"
     else d.paths_1
    next(arctxt
    + "<path id = $(dq.[toword.j]) class = $(dq."arcs") d = $(dq."M 0 0 $(path
     )") ></path>"
    + encodeword.[char.10]
    + if haslabels then
     ...
     if isempty.lab then ""
      ...
      "<text class = $(dq."nodes") > <textPath href = $(dq.[merge ("
       #" + toword.j)]) startOffset = $(dq."100%") text-anchor = $(dq."
       end") > <tspan dy = $(dq."-0.1") > $(label.lab_1) </tspan> </textPath> </text>"
      + encodeword.[char.10]
    else ""
    , j + 1
    )
  ...
  , max(maxx, nodex)
  , max(maxy, nodey)
  , if isempty.hovertext then hover
  else hover + hovertext(n.n, nodex, nodey, hovertext)
  )
 else next(txt, i, id, draw, max(maxx, nodex), max(maxy, nodey), hover)
/for (
 let hovertxt = for svg2 = "", e ∈ sort.hover do svg2 + assvg.e /for (svg2)
 "
  /br /fmt none $(drawscript:T) <svg id = $(dq."svg10") dqns =
  $(dq."http://www.w3.org/2000/svg") width = $(dq."100%") viewBox = $(dq ("5.
  0" + space + "-1" + print(2, maxx + 5.0)
 + print(2, maxy + 1.3))) onload = $(dq."[
  $(draw >> 1)].forEach (shiftstart)") > $(txt + hovertxt)+</svg> /end")
...
"<g> <rect opacity = $(dq."0.0") x = $(dq.print(2, nodex.h)) y =
 $(dq.print(2, nodey.h - 0.5)) height = $(dq."0.5") width = $(dq."
 1") ></rect> <rect pointer-events = $(dq."none") fill = $(dq."white") opacity = $(dq."0.
 0") x = $(dq.print(2, nodex.h)) y =
 $(dq.print(2, nodey.h - 0.5)) height = $(dq."1") width = $(dq."
 100") ></rect> <text pointer-events = $(dq."none") class = $(dq."nodes") x =
 $(dq.print(2, nodex.h)) y = $(dq.print(2, nodey.h)) opacity = $(dq."0.
 0") > $(text.h) </text> </g>"
+ encodeword.[char.10] 