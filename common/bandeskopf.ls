...
for acc = empty:seq.arc.T, @e ∈ arithseq(n.layers - 2, 1, 2)
do acc + marklayer(g, dummy, layers, @e),
acc
...
do acc + findvertarcsUL(g, layers, @e),
acc

function findvertarcsUR(g:graph.T, layers:seq.seq.T) seq.arc.T
for acc = empty:seq.arc.T, @e ∈ arithseq(n.layers - 1, 1, 2)
do acc + findvertarcsUR(g, layers, @e),
acc
...
let lastassignedx =
 ...
 else for acc = x, @e ∈ toseq.assigned do max(acc, findx(RtoL, q, @e)), acc
...
do acc + layerarcsR(arcstoroots, layer, @e),
acc
...
do acc + layerarcs(arcstoroots, layer, @e),
acc
...
function alignUL(
 ...
 , layers:seq.seq.T
 , marked:set.arc.T
 , layerX:set.nodeinfo.T
) set.nodeinfo.T
let vertarcs = findvertarcsUL(deletearcs(g, marked), layers)
let g3 = newgraph.vertarcs
let arcstoroots = for acc = empty:seq.arc.T, @e ∈ toseq.nodes.g3 do acc + isroot(g3, @e), asset.acc
let layerarcs = for acc = toseq.arcstoroots, @e ∈ layers do acc + layerarcsR(arcstoroots, @e), asset.acc
...
let g3 = newgraph.vertarcs
let arcstoroots = for acc = empty:seq.arc.T, @e ∈ toseq.nodes.g3 do acc + isroot(g3, @e), asset.acc
let layerarcs = for acc = toseq.arcstoroots, e2 ∈ layers do acc + layerarcs(arcstoroots, e2), asset.acc
let a = newgraph.toseq.layerarcs
let b = sinksfirst.a + singlenodelayers.layers,
assignx(true, layerX, b, empty:set.nodeinfo.T, vertarcs, 1)

function singlenodelayers(a:seq.T) seq.T if n.a = 1 then a else empty:seq.T

function singlenodelayers(a:seq.seq.T) seq.T
for acc = empty:seq.T, @e ∈ a do acc + singlenodelayers.@e,
acc
...
let m = for acc = 0, @e ∈ toseq.UR do min(acc, x.@e), acc
for acc = empty:seq.nodeinfo.T, @e ∈ arithseq(n.UL, 1, 1) do acc + merge(UL, UR, m, @e),
asset.acc
...
let paths =
 for acc = empty:seq.seq.T, a ∈ toseq.arcs.g.lg
 do
...
let a = for acc = empty:seq.T, @e ∈ layers do acc + @e, acc
for acc = empty:set.nodeinfo.T, @e ∈ a do acc + posindegree(g, layers, 1, @e),
acc 