...
let ok=  for  acc=empty:seq.byte,     i /in [0,1,2,3,4,5,6,7] do
        let val=   toint( tobits.i << (61)) 
        acc+LEBs.val+LEBu.val
  /for( for ok="",next=1, j /in [0,1,2,3,4,5,6,7] do
     let t= decodeLEBs(acc,next)
     let tu=decodeLEBu(acc,next.t)
      next( if value.t=toint( tobits.j << (61)) 
          /and value.tu=toint( tobits.j << (61))
         then ok else ok+"/br"+toword.j
      +print.tobits.value.t
      +  print.( tobits.j << (61))
      ,next.tu)
     /for(ok))
...
/and  val1=value.decodeLEBs(LEBs.val1,1) /and isempty.ok then
 "PASS"
...
let value1 = if toint.value < 0  /and toint.signbit /ne 0 then bits.-1 << (64 - 7) ∨ value >> 7 else value >> 7
...
function decodeLEB2(a:seq.byte,i:int,signbit:bits) decoderesult
  ...
  /for ( 
        let value=if (lastbyte /and signbit) = 0x0 /or j=10 then acc
...
for  acc=empty:seq.int,result=0x0,shift=0, b /in a do
  let byte = tobits.b
  let c = byte ∧ 0x7F
   let newresult = result ∨ c << shift
   if c = byte then
...
for  acc=empty:seq.int,result=0x0,shift=0, b /in a do
  ...
  let c = byte ∧ 0x7F
   let newresult = result ∨ c << shift
   if c = byte then
     let val=if(byte ∧ 0x40) = 0x0 then  newresult
