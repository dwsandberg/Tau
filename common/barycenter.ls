...
let a = for acc = 0, @e ∈ pred do acc + findindex(layer1, @e) /do acc,
baryinfo(toreal.a / toreal.length.pred, node)

function baryinfo(g:graph.T, layer1:seq.T, layer2:seq.T) seq.T
let a = 
 for acc = empty:seq.baryinfo.T, @e ∈ layer2 do acc + averagepred(g, layer1, @e) /do acc
,
for acc = empty:seq.T, @e ∈ sort.a do acc + node.@e /do acc
