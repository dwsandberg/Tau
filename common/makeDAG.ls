...
if i > length.l then
 let r = 
  for acc = asset.empty:seq.arc.T, @e ∈ reversed do acc + backarc.@e /for (acc)
...
if cardinality (successors(g, n) \ b) = 0 then asset.[n]
else empty:set.T

Function sources2(g:graph.T, b:set.T, n:T) set.T
if cardinality (predecessors(g, n) \ b) = 0 then asset.[n]
else empty:set.T
...
let d = 
 for acc = empty:set.T, @e ∈ toseq (nodes.g \ b) do acc ∪ sinks2(g, b, @e) /for (acc)
