...
do
 if first.aa /in "-" then next(acc,defs2,cmds,defined) else
 if first.aa ∈ ":"then next(acc, defs2, cmds + aa,defined)
  else  
   let k=1
   let cmd0=aa_k
   let cidx=findindex(cmd0,cmds)
   assert subseq(cmds, cidx - 1, cidx) = ":" + cmd0 report "unknown cmd"+cmd0+aa
  let cmd = cmds_if subseq(cmds, cidx+3,cidx+3) =":" then  cidx+2  else cidx  
  let cmdlib=subseq(cmds, cidx + 1, cidx + 1) + ".lib"
   let idx = findindex("="_1, aa+"?=" ) 
   let rest =  "cmd=" + cmd  + if idx > length.aa then""else aa << (idx - 2)
   let filenames=getfilenames("built",  subseq(aa, 2, 4) + cmdlib
           +subseq(aa,5,length.aa-length.rest+3)  )
  let tail=first.filenames
  next(for arcs = acc, fn ∈ filenames << 1 do
      arcs + arc(tail,fn)
 /for(if cmd /in "makelib orgmakelib" then 
    arcs+arc(tail,changeext(tail, "libsrc") )  else arcs )
 ,   if cmd /in "makelib orgmakelib" then 
   defs2+cmdpara(changeext(tail, "libsrc")
 ,[tail],"cmd=noop") 
 else defs2 /if 
     + cmdpara(tail, filenames << 1, rest) 
  , cmds
  , if cmd /in "makelib orgmakelib" then defined+tail+changeext(tail, "libsrc") else defined+tail
  )
...
let scriptstart= for acc2="set $(sp)-e  
/br if $(sp)[[ $(sp) $1 $(sp)== $(sp+dq."-n"+sp) ]] ; $(sp)then 
/br norun=true
/br fi  
/br source bin/tauconfig.sh",n /in toseq(nodes.g \ defined \ asset.[filename."orgstdlib.lib"  ])do 
      acc2+"/br checksrc"+  fullname.n  
 /for( acc2)
let g2 = 
 for g2 = g, n ∈ sources.g do g2 + arc(filename("#.#"), n)/for(g2)
 {let out=outgraph.deletenode(g2, filename("orgstdlib.lib"))[file(filename(o), out)]) 
}
let out = 
 createorder(deletenode(g2, filename."orgstdlib.lib")
, defs2
, scriptstart
) +"/br mkbuild"
[file(filename.o, out)])
...
let txt = 
 for txt = out, p ∈ if isempty.sinks then toseq.nodes.g else sinks do 
   txt  + tosetvars(defs, p)
 /for(txt)
if isempty.sinks then txt
else createorder(subgraph(g, nodes.g \ asset.sinks), defs, txt + " /br #________________")

function ?(a:filename, b:filename)ordering alphaword.name.a ? alphaword.name.b 
∧ alphaword.ext.a ? alphaword.ext.b ∧ toalphaseq.dirpath.a ? toalphaseq.dirpath.b
...
let cmdpara = lookup(defs2, cmdpara(fn, empty:seq.filename, empty:seq.word))
 ...
 if cmd = "noop" then "" else
  ...
  , "void init_$([u])(); $(ccode)init_$([u])();"
   ...
   "/p parts=$(dq([node] + parts)) 
    /br dependlibs=$(dq.depends) /br ccode=$(dq.ccode)
   /br outofdate ||( libexe $(xxx)libsrc $(parts << 1) $(data.cmdpara_1 << 3) o=$(out)
   /br libexe $(baselib)$(baselib)$([fullname.filename.out])  ;runlib $([name.fn]))"
  )
  else
...
if lib /in "stdlib orgstdlib" then [lib] else
{let cmdpara= for cmdpara=empty:set.cmdpara , ext /in "libsrc lib" while isempty.cmdpara do
 lookup(defs, cmdpara(filename("$([lib])."+ext), empty:seq.filename, empty:seq.word))
/for(cmdpara)}
 let cmdpara=lookup(defs, cmdpara(filename("$([lib])."+"lib"), empty:seq.filename, empty:seq.word))
