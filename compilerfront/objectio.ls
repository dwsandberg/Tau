...
{Note: " bitcast:seq.T (result.process.inrec.in)" will not work because a deepcopy is done when the
 process returns a value. The deepcopy will be of a type ptr which will not work. The bitcast must
 be done before the deepcody is done, which inprocess does.}
