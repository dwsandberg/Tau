...
for acc = empty:seq.seq.int, def ∈ defs do
 if isseq.first.def then
  let idx = for idx = 1, d ∈ defs while first.d ≠ parameter.first.def do idx + 1 /do idx,
  acc + [-idx]
 else
  for coded = empty:seq.int, t0 ∈ def << 1 do
   ...
   do
    idx + 1
   /do coded + if isseq.t0 then-idx else idx
  /do acc + if isempty.coded then [length.acc + 1] else coded
/do acc
...
let singlerow = 
 ...
 /do singlerow
,
close.if isempty.singlerow then
 a
else
 {remove types not represented by a record}
 for result = empty:seq.seq.mytype, row2 ∈ a do
  result + for acc = [first.row2], t ∈ row2 << 1 do acc + sub(t, singlerow) /do acc
 /do
...
/do
 ...
 if isempty.newdefs then
  ...
  close.for acc = x, new ∈ toseq(asset.used \ asset.defs) do acc + [new] /do acc
...
if t ∈ [typeint, typeword, typereal] then
 ...
 for acc = t, row ∈ singlerow do if t = first.row then last.row else acc /do acc
...
if elementtypeidx ∈ [word0, int0] then
 ...
 "ptr packed2 packed3 packed4 packed5 packed6"_packedsize
...
if patternidx < 0 ∨ length.pattern = 1 ∧ first.pattern < 0 then
 ...
 if fld:int(obj, 0) ∈ [0, 1] then
  ...
  let blocksize = length.fld:seq.int(obj, 2)
  let noblocks = (seqlen + blocksize - 1) / blocksize,
  outrecflds(buildblock
 ...
 outrecflds(buildrecord, pattern, [buildrecord, patternidx], finished0, inobj, allpatterns)

function outrecflds(buildtype:int
 , fldtypes:seq.int
 , start:seq.int
...
/do
 ...
 else if buildtype = buildrecord ∧ length.acc < 10 then
  [buildtblrecord] + acc << 1
 else if buildtype = buildseq ∧ (length.acc < 10 ∨ acc_2 = word0 ∧ length.acc < 300) then
  [buildtblseq] + acc << 1
 else
  acc

function dumptable seq.word
for txt = "/br tableentry", r ∈ encodingdata:tableentry do
 txt + %.r
/do txt
...
let allpatterns = 
 for idx = 0, pat ∈ inrecs while first.pat ≠ -1 do idx + 1 /do subseq(inrecs, 1, idx)
,
...
do
 if first.rec = buildword then
  ...
  next(stk
   , map
   + hash.encodeword.for acc = empty:seq.char, i ∈ rec << 1 do acc + char.i /do acc)
 else if first.rec = buildblock then
  let noblocks = rec_3
  ...
  let firstp = set(set(obj, blocktype.resulttype(length.elepattern, eletypeidx)), rec_4)
  let newstk = 
   for p = firstp, m = 0, val ∈ rec << 4 do
    if val > 0 then
     next(set(p, map_val), m)
    else
     next(set(p, undertop(stk,-val - 1)), if val < m then val else m)
   /do push(pop(stk,-m), obj)
  ,
  next(newstk, map)
 else
  let newstk = 
   if first.rec = buildseq ∨ first.rec = buildtblseq then
    let eletypeidx = rec_2
    let seqelepat = allpatterns_eletypeidx
    let elepattern = packedpattern(seqelepat, eletypeidx)
    let seqlen = rec_3
    ...
    let blksize = 100
    let myblksz = 
     if length.fldtypes + 3 ≤ blksize then
      blksize
     else
    ...
    do
     ...
     if typ ∈ [int0, real0] then
      ...
      next(set(newp, map_val), i + 1, m, newobjs)
     else
    ...
    assert rec_1 ∈ [buildtblrecord, buildrecord] report "PROBLEM object01"
    {buildrecord}
    let pattern = allpatterns_(rec_2)
    assert length.pattern < 8000 report "to big of a pattern"
  ...
  if first.rec ∈ [buildtblseq, buildtblrecord] then
   ...
   next(newstk, map)
/do if isempty.stk then bitcast:ptr(last.map) else top.stk
...
for all = empty:seq.byte, rec ∈ data do
 ...
 /do
  if pos then
   for acc = empty:seq.byte, i ∈ rec do
    acc + LEBu.i
   /do LEBu(length.acc * 2) + acc
  else
   for acc = empty:seq.byte, i ∈ rec do
    acc + LEBs.i
   /do LEBu(length.acc * 2 + 1) + acc
/do LEBu.length.all + all
