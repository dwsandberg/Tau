...
Function actions100(a:PEGtable) seq.seq.int actions.a
...
do
 if inst > 0 then
 push(stk, inst#constants.actions)
 else if inst < -n.otherinst then
 push(stk, (-n.otherinst - inst)#strings)
 else if inst = /e then
 push(stk, "")
 else
...
do
 if subseq(acc, n.acc - 1, n.acc) = "$." then
 acc >> 2 + merge("$." + w)
 else acc + w,
acc

Function maketable(gin:seq.pegrule, subs0:seq.word) PEGtable
let tbl = makeTbl(gin, false)
 ...
 acc0
  ...
  do
   ...
   do
    ...
    if isempty.str then
    PEGtable(tbl, consts, actions.acc + actions)
    else
     let k = findindex(consts, str),
      if k > n.consts then
      PEGtable(tbl, consts + str, actions.acc + [actions + [n.consts + 1]])
      else PEGtable(tbl, consts, actions.acc + [actions + k]),
  acc,
postProcess(acc0, subs0)

Function changeActions(gin:seq.pegrule, how:seq.word) seq.pegrule
for newg = empty:seq.pegrule, reduceno = 2, r ∈ gin
do
 for acc3 = empty:seq.pegpart, p ∈ parts.r
 do
  acc3
   + pegpart(
   part.p
   ,
    if how = "reduce" then
     for NT's = "", i ∈ arithseq(NTcount(p, gin) + 1, 1, 0) do NT's + "S.^(i)",
     NT's + %.leftside.r + "." + %.n.acc3
    else how
  ),
 next(newg + changeParts(r, acc3), reduceno + n.parts.r),
newg 