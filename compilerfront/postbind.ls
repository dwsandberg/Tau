...
if isempty.code ∨ n.code > 10 then false
 ...
 do
  if idx ≤ nopara then next(sym = Local.idx, idx + 1)
   ...
   next(
    if isconst.sym then true
    else if isBuiltin.sym then name.sym ∈ "fld getfld length getseqlength getseqtype"
    else if isInternal.sym then name.sym ∉ "idxNB"
...
do
 ...
 do
  if isrecordconstant.symz then
  ...
  else if isspecial.symz ∨ isconst.symz ∨ isBuiltin.symz ∨ isGlobal.symz ∨ inModFor.symz then next(typedictZ, resultZ, inlineZ)
  else
   let newdict2 = addtype(typedictZ, resulttype.symz)
   let b = getSymdef(source, symz)
   let sd =
    if not.isempty.b then 1#b
   ...
   do
    if isrecordconstant.sym3 then
    ...
    else if isconst.sym3 ∧ not.isFref.sym3 then next(newdict4, cache, nextvar, result1 + sym3)
    else if isspecial.sym3 then
     let sym = replaceTsymbol(modpara, sym3)
     let newsym =
      if isloopblock.sym then
       for acc = empty:seq.mytype, p ∈ paratypes.sym
       do acc + basetype(p, newdict4),
       Loopblock(acc, firstvar.sym, basetype(resulttype.sym, newdict4))
      else if isSequence.sym then Sequence(parameter.basetype(resulttype.sym, newdict4), nopara.sym)
      else if isstart.sym then Start.basetype(resulttype.sym, newdict4)
      else sym,
     ...
     if name.sym5 ∈ "pushflds" ∧ isBuiltin.sym5 then
      ...
      let newValue =
       if not.isempty.cacheValue then code.1#cacheValue
       else if isInternal.sym5 then
        let discard = symbolref.sym5,
        [sym5]
       else if name.sym5 ∈ "idxNB" then
        let orgtype = 1#paratypes.sym5,
        if name.symz ∈ "#sequenceIndex"
        ∧ name.module.symz ∈ "seq"
        ∧ abstracttypename.1#paratypes.symz ∈ "pseq" then
         {???? why must this case be included?}
         let discard = symbolref.sym5,
         [sym5]
      ...
      let newValue2 =
       if not.isempty.result2 ∧ 1^result2 = PreFref ∧ n.newValue ≠ 1 then {cannot inline Fref} [sym5]
       else newValue,
      next(
       newdict3
   ...
   next(
    ...
    , if verysimpleinline(symz, result1) ∧ not.isNOINLINE.sd then inlineZ + symdef(symz, result1, 0)
    else inlineZ
   )
...
do
 if isspecial.s then
  if isdefine.s then max(acc, value.s)
  else if isloopblock.s then max(acc, firstvar.s + nopara.s - 1)
  else acc
 else acc,
acc
...
if name.sym ∈ "finishStart" then
 ...
 [
  if name.sym ∈ "outofbounds" then symbol(moduleref."* tausupport", "outofbounds", seqof.typeword)
   ...
   tmp
  else if name.sym ∈ "set set2" then setSym.basetype(1^paratypes.sym, newdict3)
...
do
 ...
 do
  if isdefine.sym then
   ...
   next(nextvar, map, result + value.1#l, newdict, lastloop)
   ...
   next(
    ...
    else if name.sym ∈ "n length" then GetSeqLength
...
do
 if not.all ∧ name.module.f ∉ exports then acc
  ...
  do
   if isrecordconstant.sym2 then symbolref.sym2
   else if isAbstract.module.sym2 ∨ isconstantorspecial.sym2 ∨ isBuiltin.sym2 ∨ inModFor.sym2 then acc3
   else symbolref.sym2,
  acc3,
acc 