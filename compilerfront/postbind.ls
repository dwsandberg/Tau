...
if isempty.code ∨ last.code = Optionsym ∨ length.code > 10 then
 false
else
...
do
 next(
  if not.isabstract.module.sym.sd ∧ verysimpleinline(sym.sd, code.sd) then
   for acc = empty:seq.symbol, typedict3 = typedict.inline, symx ∈ code.sd do
    if islocal.symx ∨ isconst.symx then
     next(acc + symx, typedict3)
    else
