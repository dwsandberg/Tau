...
do
 let newacc =
  if p sub 1 ∈ "type" ∧ not.cmd then acc + headerType(p, modname, false, parano)
   ...
   let r0 = parse(p, "")
   assert status.r0 ∉ "Failed" report ":(red."Syntax Error in header")/br:(p)",
   let play =
    headerType(subseq(p, 1, place.r0 - 1), modname, subseq(result.r0, 1, 1) = "ENTRY", parano),
...
{OPTION NOINLINE}
if typ ∉ ["seq.word", "seq.byte", "seq.UTF8", "seq.file"] then "~ENTRY" else ""
...
Function result(a:resultType) seq.word last.result.top.stk.a
