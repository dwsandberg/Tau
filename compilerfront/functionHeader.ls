...
do
 let newacc =
  if 1#p ∈ "type" ∧ not.cmd then
  acc + headerType(p, modname, false, parano)
  ...
  if 1#p ∈ "Module module" then
  next(newacc, p << 1, mods + 2#p, parano + 1)
...
maketable."* Options /strong any Description Discard!Block <* block Values *> /action $.0 //br $.1 $.2
<* block $.4 *>
...
do
 if w ∈ "<*" then
 next(acc3 + w, options, w, nest + 1)
 else if w ∈ "*>" then
  if subseq(acc3, n.acc3 - 1, n.acc3) = "<* block" then
  next(acc3 >> 2, options, w, nest - 1)
  else next(acc3 + w, options, w, nest - 1)
 else if last ∈ "/br" ∧ nest = 0 then
  let k = findindex(params, w) + 1
  let kind = tokind.
   if (k + 2)#params ∈ "." then
   subseq(params, k + 1, k + 3)
...
do
 if 1#para ∈ options ∨ 1#para ∈ "xo input" then
 undoc
...
[
 ...
 , "/ Any Any F C"
  = 
   if $.1 = "Function" ∧ 1#"~ENTRY" ∉ $.3 ∧ 1#"COMMAND" ∈ $.4 then
   "ENTRY"
   else ""
...
""
 ...
 if 4^R = "Function" ∧ 1#"~ENTRY" ∉ 2^R ∧ 1#"COMMAND" ∈ 1^R then
 "ENTRY"
 else ""
...
if 1^R ∉ ["boolean", "seq.word", "seq.file"] then "~ENTRY" else ""
