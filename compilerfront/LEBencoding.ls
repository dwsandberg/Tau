...
do
 ...
 if c = byte then next(acc + toint.newresult, 0x0, 0)
...
do
 ...
 if c = byte then
  let val =
   if (byte ∧ 0x40) = 0x0 ∨ toint.newresult < 0 then newresult
   else newresult ∨ tobits.-1 << (shift + 7),
  next(acc + toint.val, 0x0, 0)
 else next(acc, newresult, shift + 7),
acc

Function LEBs(s:seq.int) seq.byte
for acc = empty:seq.byte, e ∈ s do acc + LEBs.e,
acc

Function LEBu(s:seq.int) seq.byte
for acc = empty:seq.byte, e ∈ s do acc + LEBu.e,
acc
