...
assert status.r ∉ "Failed" ∧ (1#input ∈ "function Function" ∨ i.top.stk.r = n.input + 1) report errormessage("Syntax error", input, i.top.stk.r),
result.r
...
assert not.isempty.a report errormessage("cannot resolve type^(text)", input.common, place),
1#a
...
if partno = 2 then createtype(common, 3^R, 2^R, place.rinfo)
else if partno = 3 then 1^R
else if partno = 4 then createfunc(common, place.rinfo, text.4^R, null, 3^R, 2^R)
else if partno = 5 then createfunc(common, place.rinfo, text.3^R, null, null, 2^R)
else if partno = 6 then createfunc(common, place.rinfo, text.5^R, 4^R, 3^R, 2^R)
else if partno = 7 then createfunc(common, place.rinfo, text.4^R, 3^R, null, 2^R)
else if partno = 8 then 2^R + 1^R
else if partno = 9 then 2^R + 1^R
else if partno = 10 then addpara(common, text.2^R, 1^R, place.rinfo)
else if partno = 11 then addpara(common, ":", 1^R, place.rinfo)
else if partno = 12 then toAttribute(2^R, text.2^R + "." + text.1^R)
else if partno = 13 then 1^R
else if partno = 14 then 1^R
else if partno = 15 then 2^R
else if partno = 16 then 2^R
else if partno = 17 then 2^R
else 1#R

function mytable seq.tableEntry
[
 {1} tableEntry(NT.T'.2, 1#"?", Match, Failure, "")
 , {2} tableEntry(T', 1#"type", NT.3, MatchAny.7, "type Id is FPL")
 , {3} tableEntry(NT.!T.43, 1#"Id", T.4, MatchAny.7, "Id is FPL")
 , {4} tableEntry(T, 1#"is", NT.5, MatchAny.7, "is FPL")
 , {5} tableEntry(NT.31, 1#"FPL", NT.6, MatchAny.7, "FPL")
 , {6} tableEntry(NT.T.51, 1#"Comment", Reduce.2, MatchAny.7, "")
 , {7} tableEntry(MatchAny, 1#"?", NT.8, Fail, "any Header")
 , {8} tableEntry(NT.9, 1#"Header", Reduce.3, Fail, "Header")
 , {9} tableEntry(NT.!T.43, 1#"Id", T'.10, Fail, "Id (FPL) Type")
 , {10} tableEntry(T', 1#"(", NT.11, NT.16, "(FPL) Type")
 , {11} tableEntry(NT.31, 1#"FPL", T.12, NT.15, "FPL) Type")
 , {12} tableEntry(T, 1#")", NT.13, NT.15, ") Type")
 , {13} tableEntry(NT.39, 1#"Type", NT.14, NT.15, "Type")
 , {14} tableEntry(NT.T.51, 1#"Comment", Reduce.4, NT.15, "")
 , {15} tableEntry(NT.!T.43, 1#"Id", NT.16, Fail, "Id Type")
 , {16} tableEntry(NT.39, 1#"Type", NT.17, NT.18, "Type")
 , {17} tableEntry(NT.T.51, 1#"Comment", Reduce.5, NT.18, "")
 , {18} tableEntry(NT.!T.43, 1#"Id", T'.19, Fail, "Id:Type (FPL) Type")
 , {19} tableEntry(T', 1#":", NT.20, T.27, ":Type (FPL) Type")
 , {20} tableEntry(NT.39, 1#"Type", T'.21, Fail, "Type (FPL) Type")
 , {21} tableEntry(T', 1#"(", NT.22, NT.29, "(FPL) Type")
 , {22} tableEntry(NT.31, 1#"FPL", T.23, NT.26, "FPL) Type")
 , {23} tableEntry(T, 1#")", NT.24, NT.26, ") Type")
 , {24} tableEntry(NT.39, 1#"Type", NT.25, NT.26, "Type")
 , {25} tableEntry(NT.T.51, 1#"Comment", Reduce.6, NT.26, "")
 , {26} tableEntry(NT.!T.43, 1#"Id", T.27, Fail, "Id:Type Type")
 , {27} tableEntry(T, 1#":", NT.28, Fail, ":Type Type")
 , {28} tableEntry(NT.39, 1#"Type", NT.29, Fail, "Type Type")
 , {29} tableEntry(NT.39, 1#"Type", NT.30, Fail, "Type")
 , {30} tableEntry(NT.T.51, 1#"Comment", Reduce.7, Fail, "")
 , {31} tableEntry(NT.MatchAny.35, 1#"FP", NT.32, Fail, "FP")
 , {32} tableEntry(NT.T.33, 1#"FPL'", Reduce.8, Fail, "")
 , {33} tableEntry(T, 1#",", NT.34, Success*, ", FP")
 , {34} tableEntry(NT.MatchAny.35, 1#"FP", Reduce*(9, T.33), Success*, "FP")
 , {35} tableEntry(MatchAny, 1#"?", T.36, NT.38, "any:Type")
 , {36} tableEntry(T, 1#":", NT.37, NT.38, ":Type")
 , {37} tableEntry(NT.39, 1#"Type", Reduce.10, NT.38, "Type")
 , {38} tableEntry(NT.39, 1#"Type", Reduce.11, Fail, "Type")
 , {39} tableEntry(NT.!T.43, 1#"Id", T.40, Fail, "Id.Type")
 , {40} tableEntry(T, 1#".", NT.41, NT.42, ".Type")
 , {41} tableEntry(NT.39, 1#"Type", Reduce.12, NT.42, "Type")
 , {42} tableEntry(NT.!T.43, 1#"Id", Reduce.13, Fail, "Id")
