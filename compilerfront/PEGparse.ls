...
function str(a:seq.pegrule) seq.word if isempty.a then "" else replacement.1#parts.1#a
...
adjust.
 if subseq(input, 1, 1) = "function" then
 postprocess.r
 else
  ...
  report
   ...
   /br^(subseq(input, 1, maxi - 1))
   /br Unparsed Input: ^(subseq(input, maxi, maxi + 8))",
  grammar.result.r
...
do
 if 1#part.p ∈ flaglist then
 next(flags + p, flags2 + 1#part.p, subs)
...
function %(a:attribute) seq.word str.a
...
toAttribute.
 if 1#a ∈ "attributeType" ∧ value = "seq.word" then
 [pegpart(a, "$.0^(value)"), pegpart("/wordsAttribute S2 Else /end", "$.0 $.1")]
 ...
 else if 1#a ∈ "commonName" then
 [pegpart(a, "$.0^(value)"), pegpart("/common S2 Else /end", "$.0 $.1")]
 else if 1#a ∈ "seqElementType attributeType resultType commonType wordmap" then
 [pegpart(a, "$.0^(value)")]
 else if 1#a ∈ "error trace fix nogrammar notable" then
 [pegpart(%.merge."/^(a)" + "S2 Else /end", "$.0 $.1")]
...
/br Non-terminals:B C Code Code' CodeFormat F1 FP FP' N2 Part Part' Part2 Part2' S Str Str1 Str3
Type
/br Terminals:() *+,.// //action:= K PEGgen [] any dq function {}
/br S ← [dq CodeFormat / function PEGgen (FP FP') Type {Str3 B} / function PEGgen (FP FP') Type /
F1
...
toAttribute(parts.5^R + parts.4^R + parts.1^R)
else if partno = 4 then
toAttribute(parts.3^R + parts.2^R)
else if partno = 5 then
1^R
else if partno = 6 then
attribute("", grammar.4^R + rule("*", str.3^R, parts.2^R + parts.1^R))
else if partno = 7 then
attribute("", grammar.4^R + rule("+", str.3^R, parts.2^R + parts.1^R))
else if partno = 8 then
attribute("", grammar.4^R + rule(str.3^R, parts.2^R + parts.1^R))
else if partno = 9 then
toAttribute(parts.2^R + parts.1^R)
else if partno = 10 then
1^R
else if partno = 11 then
toAttribute.[pegpart(str.2^R, str.1^R)]
...
toAttribute(2^R, str.2^R + "." + str.1^R)
...
toAttribute(parts.3^R + parts.FP(str.2^R, str.1^R))
