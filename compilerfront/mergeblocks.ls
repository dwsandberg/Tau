...
if x ∈ [Littrue, Litfalse] then
 ...
 /do
  if cardinality.used = length.parts - 2 ∧ isempty.tailR then
   for acc = empty:seq.symbol, p2 ∈ parts do acc + p2 /do acc + EndBlock
...
for acc = length.parts, e ∈ reverse.parts while not.isstartorloop.last.e do acc - 1 /do acc
...
/do parts >> length.acc + acc + endpart
