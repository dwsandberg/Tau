...
report errormessage("syntax error", recoverInfo(pop.stk.r, input, i.top.stk.r)),
code.result.r
...
let t = "<* literal^(message) *> /br^(subseq(pp, 1, idx))"
...
(if not.isempty.t ∧ 1^t ∈ "/keyword" then t >> 1 + addback else t + addback)
 ...
 else "Replaced^(dq.if place + 11 < n.input then unprocessed + "…" else unprocessed) with
 ^(ending) to parse."
 ...
 do
  ...
  let arg2code = subseq(code.R1, start, start + len - 1),
   if 1#op ∈ "∧ ∨" ∧ types = [typeboolean, typeboolean] ∧ not.outText.static then
    ...
    let f =
     if op = "≠" then
     [lookupbysig(static, dict, "=", types, rinfo), NotOp]
     else if op = "∉" then
     [lookupbysig(static, dict, "∈", types, rinfo), NotOp]
     else if op = "≥" then
     [lookupbysig(static, dict, "<", types, rinfo), NotOp]
...
if isempty.code.R0 then
 if forcecode then
 bindinfo(dict.R0, [Words(text.R0 + zz)], [t], "")
 else bindinfo(dict.R0, code.R0, types.R0, text.R0 + zz)
else if isempty.zz then
R0
else
 let f = lookupbysig(static, dict.R0, "+", [t, t], rinfo),
 bindinfo(dict.R0, code.R0 + Words.zz + f, [t], "")

function strExp(static:commonType, R0:bindinfo, exp:bindinfo, rinfo:recoverInfo) bindinfo
let t = seqof.typeword
let f = lookupbysig(static, dict.exp, "+", [t, t], rinfo)
let codeexp =
 if 1#types.exp = t then
 code.exp
 else
  let cvt = lookupbysig(static, dict.exp, "%", types.exp, rinfo)
  assert resulttype.cvt = seqof.typeword
  report errormessage("Expected function^(cvt) to have return type of seq.word", rinfo),
  code.exp + cvt,
if isempty.code.R0 ∧ isempty.text.R0 then
bindinfo(dict.R0, codeexp, [t], "")
else
...
assert 1#types.cond = typeboolean
report errormessage("cond of if must be boolean but is^(1#types.cond)", rinfo)
assert isempty.types.R0 ∨ types.R0 = types.thenpart
report errormessage("then and else types are different", rinfo),
...
for dict = d, no = 1, t ∈ types do next(dict + Local(no#names, t, no), no + 1),
...
report
 if n.name = 1 ∧ 1#name ∈ "if else then for" then
 errormessage("Syntax error", rinfo)
 else errormessage(
  "cannot find 1^(if n.name = 1 then name else [1#name, 1#":"] + name << 1) (
...
let discard =
 ...
 do
  ...
  assert not.isempty.xxx ∨ isAbstract.module.1#f
  report errormessage("using symbol^(1#f) requires unbound^(sym2)", rinfo),
  0,
 0,
1#f
...
report errormessage("duplciate symbol:^(name)", rinfo),
...
let types = types.R
assert for acc = true, @e ∈ types do acc ∧ 1#types = @e, acc
report errormessage("types do not match in build", rinfo),
...
report errormessage("duplicate symbol:^(name)", rinfo)
...
if kind ∈ "other" then
 ...
 bindinfo(
  dict.R
  , [if outText.static then symbol(internalmod, text.R, rt) else Lit.toint.1#text.R]
  , [rt]
  , ""
 )
...
assert returntype = 1#types.exp
report errormessage("function type of^(returntype) does not match expression type^(1#types.exp)", rinfo),
...
let seqtype =
 ...
 seqof.typeint
  ...
  report errormessage("duplicate symbol:^(elename)", rinfo)
  assert isseq.seqtype0
  report errormessage("final expression in for list must be a sequence but it is of type:^(seqtype0)", rinfo),
  seqtype0
...
assert 1#types.cond = typeboolean
report errormessage("condition in assert must be boolean in:", rinfo)
assert 1#types.exp = seqof.typeword
report errormessage("report in assert must be seq of word in:", rinfo),
if outText.static then
bindinfo(
 dict.cond
...
[
 ...
 , "/ Function Name Type Declare' E" = checktype(common, $.2, $.3, $.4, rinfo)
 , "String dq String' dq" = strExp(common, $.1, "", true, rinfo)
 , "* String' carrot (E)" = strExp(common, $.0, $.1, rinfo)
 , "/ carrot" = strExp(common, $.0, "^", false, rinfo)
 , "/ str2" = strExp(common, $.0, text.$.1, false, rinfo)
 ...
 , "/ String" = $.1
 , "/ Declare Declare' E"
  = bindinfo(dict.$.0, mergeText(common, mergeText(common, code.$.1, code.$.2), code.$.3), types.$.3, "")
...
/br Non-terminals:AccumList AccumList' And And' Atom Compare Compare' Declare Declare' E EL' FP FPL
FPL' ForDeclare IF Id N Name Or Or' Parser Power Power' Product Product' String String' Sum Sum' Type
Unary comma? str2
/br Terminals:#() *+,-./and /cap /cup /ge /in /le /ne /nin /or:< << = > >1 >2 >> Function [\]
^any assert carrot do dq else for function if let mod report slash then while {} ∈ ∉ ∧ ∨ ∩ ∪ ≠ ≤ ≥
⊻
/br Parser ← function Name (FPL) Type Declare' E / function Name Type Declare' E / Function Name (FPL
) Type Declare' E / Function Name Type Declare' E
...
if partno = 2 then
checktype(common, 3^R, 2^R, 1^R, rinfo)
...
strExp(common, 1^R, "", true, rinfo)
else if partno = 7 then
strExp(common, 2^R, 1^R, rinfo)
else if partno = 8 then
strExp(common, 1^R, "^", false, rinfo)
else if partno = 9 then
strExp(common, 2^R, text.1^R, false, rinfo)
...
binaryops(common, 2^R, 1^R, rinfo)
...
unaryop(common, dict.1^R, rinfo, "-", 1^R)
else if partno = 51 then
unaryop(common, dict.2^R, {place-1?} rinfo, text.2^R, 1^R)
...
seqExp(common, bindinfo(dict.2^R, code.2^R + code.1^R, types.2^R + types.1^R, ""), rinfo)
...
ifExp(common, 4^R, 3^R, 2^R, 1^R, rinfo)
...
bindlit2(common, 1^R, rinfo)
else if partno = 64 then
bindinfo(dict.2^R, empty:seq.symbol, empty:seq.mytype, text.2^R + text.1^R)
...
ifExp(common, 3^R, 2^R, 1^R, rinfo)
else if partno = 70 then
bindinfo(dict.1^R, empty:seq.symbol, empty:seq.mytype, text.2^R + "." + text.1^R)
else if partno = 71 then
1^R
else if partno = 72 then
letExp(common, 3^R, 2^R, rinfo)
else if partno = 73 then
assertExp(common, 3^R, 2^R, rinfo)
...
forbody(common, 3^R, bindinfo(dict.4^R, [Littrue], [typeboolean], ""), 2^R, rinfo)
else if partno = 76 then
forbody(common, 4^R, 3^R, 2^R, rinfo)
else if partno = 77 then
forSeqExp(common, 3^R, text.2^R, 1^R, rinfo)
else if partno = 78 then
forSeqExp(common, 3^R, text.2^R, 1^R, rinfo)
else if partno = 79 then
forSeqExp(common, 1^R, ".", 1^R, rinfo)
else if partno = 80 then
bindinfo(dict.4^R, code.2^R + code.1^R, types.2^R + types.1^R, text.3^R + text.1^R)
else if partno = 81 then
bindinfo(dict.3^R, code.3^R + code.1^R, types.3^R + types.1^R, text.3^R + text.2^R)
else if partno = 82 then
bindinfo(dict.1^R, mergeText(common, code.2^R, code.1^R), empty:seq.mytype, "")
else if partno = 83 then
declarePara(dict.2^R, text.2^R + text.1^R, types.2^R + types.1^R)
else if partno = 84 then
bindinfo(dict.2^R, empty:seq.symbol, types.2^R + types.1^R, text.2^R + text.1^R)
else if partno = 85 then
addpara(common, text.2^R, 1^R, rinfo)
else if partno = 86 then
addpara(common, 2^R, ":", 1^R, rinfo)
else 1#R

function mytable seq.tableEntry
[
 {1} tableEntry(NT.T'.2, 1#"?", Match, Failure, "")
 , {2} tableEntry(T', 1#"function", NT.3, T'.15, "function any (any) any any")
 , {3} tableEntry(NT.157, 1#"Name", T'.4, T'.15, "any (any) any any")
 , {4} tableEntry(T', 1#"(", NT.5, NT.12, "(any) any any")
 , {5} tableEntry(NT.227, 1#"FPL", T.6, T'.10, "any) any any")
 , {6} tableEntry(T, 1#")", NT.7, T'.10, ") any any")
 , {7} tableEntry(NT.176, 1#"Type", NT.8, T'.10, "any any")
 , {8} tableEntry(NT.226, 1#"Declare'", NT.9, T'.10, "any")
 , {9} tableEntry(NT.40, 1#"E", Reduce.2, T'.10, "any")
 , {10} tableEntry(T', 1#"function", NT.11, T'.15, "function any any any")
 , {11} tableEntry(NT.157, 1#"Name", NT.12, T'.15, "any any any")
 , {12} tableEntry(NT.176, 1#"Type", NT.13, T'.15, "any any")
 , {13} tableEntry(NT.226, 1#"Declare'", NT.14, T'.15, "any")
 , {14} tableEntry(NT.40, 1#"E", Reduce.3, T'.15, "any")
 , {15} tableEntry(T', 1#"Function", NT.16, Fail, "Function any (any) any any")
 , {16} tableEntry(NT.157, 1#"Name", T'.17, Fail, "any (any) any any")
 , {17} tableEntry(T', 1#"(", NT.18, NT.25, "(any) any any")
 , {18} tableEntry(NT.227, 1#"FPL", T.19, T.23, "any) any any")
 , {19} tableEntry(T, 1#")", NT.20, T.23, ") any any")
 , {20} tableEntry(NT.176, 1#"Type", NT.21, T.23, "any any")
 , {21} tableEntry(NT.226, 1#"Declare'", NT.22, T.23, "any")
 , {22} tableEntry(NT.40, 1#"E", Reduce.4, T.23, "any")
 , {23} tableEntry(T, 1#"Function", NT.24, Fail, "Function any any any")
 , {24} tableEntry(NT.157, 1#"Name", NT.25, Fail, "any any any")
 , {25} tableEntry(NT.176, 1#"Type", NT.26, Fail, "any any")
 , {26} tableEntry(NT.226, 1#"Declare'", NT.27, Fail, "any")
 , {27} tableEntry(NT.40, 1#"E", Reduce.5, Fail, "any")
 ...
 , {31} tableEntry(T', 1#"^", T.32, NT.36, "^(1#"^") (any)")
 , {32} tableEntry(T, 1#"(", NT.33, T'.35, "(any)")
 , {33} tableEntry(NT.40, 1#"E", T.34, T'.35, "any)")
 , {34} tableEntry(T, 1#")", Reduce*(7, T'.31), T'.35, ")")
 , {35} tableEntry(T', 1#"^", Reduce*(8, T'.31), NT.36, "^(1#"^")")
 , {36} tableEntry(NT.!T.37, 1#"str2", Reduce*(9, T'.31), Success*, "any")
 ...
 , {40} tableEntry(NT.43, 1#"Or", Reduce.11, Fail, "any")
 , {41} tableEntry(T, 1#",", NT.42, Success*, ", any")
 , {42} tableEntry(NT.40, 1#"E", Reduce*(12, T.41), Success*, "any")
 , {43} tableEntry(NT.51, 1#"And", NT.44, Fail, "any")
 , {44} tableEntry(NT.T'.45, 1#"Or'", Reduce.13, Fail, "")
 , {45} tableEntry(T', 1#"∨", NT.46, T'.47, "∨ any")
 , {46} tableEntry(NT.51, 1#"And", Reduce*(14, T'.45), T'.47, "any")
 , {47} tableEntry(T', 1#"/or", NT.48, T.49, "/or any")
 , {48} tableEntry(NT.51, 1#"And", Reduce*(15, T'.45), T.49, "any")
 , {49} tableEntry(T, 1#"⊻", NT.50, Success*, "⊻ any")
 , {50} tableEntry(NT.51, 1#"And", Reduce*(16, T'.45), Success*, "any")
 , {51} tableEntry(NT.57, 1#"Compare", NT.52, Fail, "any")
 , {52} tableEntry(NT.T'.53, 1#"And'", Reduce.17, Fail, "")
 , {53} tableEntry(T', 1#"∧", NT.54, T.55, "∧ any")
 , {54} tableEntry(NT.57, 1#"Compare", Reduce*(18, T'.53), T.55, "any")
 , {55} tableEntry(T, 1#"/and", NT.56, Success*, "/and any")
 , {56} tableEntry(NT.57, 1#"Compare", Reduce*(19, T'.53), Success*, "any")
 , {57} tableEntry(NT.81, 1#"Sum", NT.58, Fail, "any")
 , {58} tableEntry(NT.T'.59, 1#"Compare'", Reduce.20, Fail, "")
 , {59} tableEntry(T', 1#"=", NT.60, T'.61, "= any")
 , {60} tableEntry(NT.81, 1#"Sum", Reduce*(21, T'.59), T'.61, "any")
 , {61} tableEntry(T', 1#"≠", NT.62, T'.63, "≠ any")
 , {62} tableEntry(NT.81, 1#"Sum", Reduce*(22, T'.59), T'.63, "any")
 , {63} tableEntry(T', 1#">", NT.64, T'.65, "> any")
 , {64} tableEntry(NT.81, 1#"Sum", Reduce*(23, T'.59), T'.65, "any")
 , {65} tableEntry(T', 1#"<", NT.66, T'.67, "< any")
 , {66} tableEntry(NT.81, 1#"Sum", Reduce*(24, T'.59), T'.67, "any")
 , {67} tableEntry(T', 1#">1", NT.68, T'.69, ">1 any")
 , {68} tableEntry(NT.81, 1#"Sum", Reduce*(25, T'.59), T'.69, "any")
 , {69} tableEntry(T', 1#">2", NT.70, T'.71, ">2 any")
 , {70} tableEntry(NT.81, 1#"Sum", Reduce*(26, T'.59), T'.71, "any")
 , {71} tableEntry(T', 1#"≥", NT.72, T'.73, "≥ any")
 , {72} tableEntry(NT.81, 1#"Sum", Reduce*(27, T'.59), T'.73, "any")
 , {73} tableEntry(T', 1#"/ge", NT.74, T'.75, "/ge any")
 , {74} tableEntry(NT.81, 1#"Sum", Reduce*(28, T'.59), T'.75, "any")
 , {75} tableEntry(T', 1#"≤", NT.76, T'.77, "≤ any")
 , {76} tableEntry(NT.81, 1#"Sum", Reduce*(29, T'.59), T'.77, "any")
 , {77} tableEntry(T', 1#"/le", NT.78, T.79, "/le any")
 , {78} tableEntry(NT.81, 1#"Sum", Reduce*(30, T'.59), T.79, "any")
 , {79} tableEntry(T, 1#"/ne", NT.80, Success*, "/ne any")
 , {80} tableEntry(NT.81, 1#"Sum", Reduce*(31, T'.59), Success*, "any")
 , {81} tableEntry(NT.95, 1#"Product", NT.82, Fail, "any")
 , {82} tableEntry(NT.T'.83, 1#"Sum'", Reduce.32, Fail, "")
 , {83} tableEntry(T', 1#"-", NT.84, T'.85, "-any")
 , {84} tableEntry(NT.95, 1#"Product", Reduce*(33, T'.83), T'.85, "any")
 , {85} tableEntry(T', 1#"+", NT.86, T'.87, "+any")
 , {86} tableEntry(NT.95, 1#"Product", Reduce*(34, T'.83), T'.87, "any")
 , {87} tableEntry(T', 1#"∈", NT.88, T'.89, "∈ any")
 , {88} tableEntry(NT.95, 1#"Product", Reduce*(35, T'.83), T'.89, "any")
 , {89} tableEntry(T', 1#"/in", NT.90, T'.91, "/in any")
 , {90} tableEntry(NT.95, 1#"Product", Reduce*(36, T'.83), T'.91, "any")
 , {91} tableEntry(T', 1#"∉", NT.92, T.93, "∉ any")
 , {92} tableEntry(NT.95, 1#"Product", Reduce*(37, T'.83), T.93, "any")
 , {93} tableEntry(T, 1#"/nin", NT.94, Success*, "/nin any")
 , {94} tableEntry(NT.95, 1#"Product", Reduce*(38, T'.83), Success*, "any")
 , {95} tableEntry(NT.T'.117, 1#"Unary", NT.96, Fail, "any")
 , {96} tableEntry(NT.T'.97, 1#"Product'", Reduce.39, Fail, "")
 , {97} tableEntry(T', 1#"*", NT.98, T'.99, "* any")
 , {98} tableEntry(NT.T'.117, 1#"Unary", Reduce*(40, T'.97), T'.99, "any")
 , {99} tableEntry(T', 1#">>", NT.100, T'.101, ">> any")
 , {100} tableEntry(NT.T'.117, 1#"Unary", Reduce*(41, T'.97), T'.101, "any")
 , {101} tableEntry(T', 1#"<<", NT.102, T'.103, "<< any")
 , {102} tableEntry(NT.T'.117, 1#"Unary", Reduce*(42, T'.97), T'.103, "any")
 , {103} tableEntry(T', slash, NT.104, T'.105, "^(slash) any")
 , {104} tableEntry(NT.T'.117, 1#"Unary", Reduce*(43, T'.97), T'.105, "any")
 , {105} tableEntry(T', 1#"mod", NT.106, T'.107, "mod any")
 , {106} tableEntry(NT.T'.117, 1#"Unary", Reduce*(44, T'.97), T'.107, "any")
 , {107} tableEntry(T', 1#"∩", NT.108, T'.109, "∩ any")
 , {108} tableEntry(NT.T'.117, 1#"Unary", Reduce*(45, T'.97), T'.109, "any")
 , {109} tableEntry(T', 1#"∪", NT.110, T'.111, "∪ any")
 , {110} tableEntry(NT.T'.117, 1#"Unary", Reduce*(46, T'.97), T'.111, "any")
 , {111} tableEntry(T', 1#"/cap", NT.112, T'.113, "/cap any")
 , {112} tableEntry(NT.T'.117, 1#"Unary", Reduce*(47, T'.97), T'.113, "any")
 , {113} tableEntry(T', 1#"/cup", NT.114, T.115, "/cup any")
 , {114} tableEntry(NT.T'.117, 1#"Unary", Reduce*(48, T'.97), T.115, "any")
 , {115} tableEntry(T, 1#"\", NT.116, Success*, "\ any")
 , {116} tableEntry(NT.T'.117, 1#"Unary", Reduce*(49, T'.97), Success*, "any")
 , {117} tableEntry(T', 1#"-", NT.118, NT.119, "-any")
 , {118} tableEntry(NT.T'.117, 1#"Unary", Reduce.50, NT.119, "any")
 , {119} tableEntry(NT.!T.161, 1#"Id", T.120, T'.122, "any.any")
 , {120} tableEntry(T, 1#".", NT.121, T'.122, ".any")
 , {121} tableEntry(NT.T'.117, 1#"Unary", Reduce.51, T'.122, "any")
 , {122} tableEntry(T', 1#"{", NT.123, NT.126, "{} any")
 , {123} tableEntry(NT.T.235, 1#"N", T.124, NT.126, "} any")
 , {124} tableEntry(T, 1#"}", NT.125, NT.126, "} any")
 , {125} tableEntry(NT.T'.117, 1#"Unary", Reduce.52, NT.126, "any")
 , {126} tableEntry(NT.127, 1#"Power", Reduce.53, Fail, "any")
 , {127} tableEntry(NT.T'.133, 1#"Atom", NT.128, Fail, "any")
 , {128} tableEntry(NT.T'.129, 1#"Power'", Reduce.54, Fail, "")
 , {129} tableEntry(T', 1#"#", NT.130, T.131, "#any")
 , {130} tableEntry(NT.T'.117, 1#"Unary", Reduce*(55, T'.129), T.131, "any")
 , {131} tableEntry(T, 1#"^", NT.132, Success*, "^(1#"^") any")
 , {132} tableEntry(NT.T'.117, 1#"Unary", Reduce*(56, T'.129), Success*, "any")
 , {133} tableEntry(T', 1#"(", NT.134, T'.136, "(any)")
 , {134} tableEntry(NT.40, 1#"E", T.135, T'.136, "any)")
 , {135} tableEntry(T, 1#")", Reduce.57, T'.136, ")")
 , {136} tableEntry(T', 1#"[", NT.137, NT.140, "[any]")
 , {137} tableEntry(NT.40, 1#"E", NT.138, NT.140, "any]")
 , {138} tableEntry(NT.T.41, 1#"EL'", T.139, NT.140, "]")
 , {139} tableEntry(T, 1#"]", Reduce.58, NT.140, "]")
 , {140} tableEntry(NT.T.28, 1#"String", Reduce.59, NT.141, "^(1#dq)^(1#dq)")
 , {141} tableEntry(NT.T'.180, 1#"Declare", NT.142, T'.144, "{} any")
 , {142} tableEntry(NT.226, 1#"Declare'", NT.143, T'.144, "any")
 , {143} tableEntry(NT.40, 1#"E", Reduce.60, T'.144, "any")
 , {144} tableEntry(T', 1#"if", NT.145, NT.151, "if any then any else any")
 , {145} tableEntry(NT.40, 1#"E", T.146, NT.151, "any then any else any")
 , {146} tableEntry(T, 1#"then", NT.147, NT.151, "then any else any")
 , {147} tableEntry(NT.40, 1#"E", NT.148, NT.151, "any else any")
 , {148} tableEntry(NT.T.171, 1#"IF", T.149, NT.151, "else any")
 , {149} tableEntry(T, 1#"else", NT.150, NT.151, "else any")
 , {150} tableEntry(NT.40, 1#"E", Reduce.61, NT.151, "any")
 , {151} tableEntry(NT.157, 1#"Name", T.152, Fail, "any (any)")
 , {152} tableEntry(T, 1#"(", NT.153, NT.156, "(any)")
 , {153} tableEntry(NT.40, 1#"E", NT.154, NT.156, "any)")
 , {154} tableEntry(NT.T.41, 1#"EL'", T.155, NT.156, ")")
 , {155} tableEntry(T, 1#")", Reduce.62, NT.156, ")")
 , {156} tableEntry(NT.157, 1#"Name", Reduce.63, Fail, "any")
 , {157} tableEntry(NT.!T.161, 1#"Id", T.158, Fail, "any:any")
 , {158} tableEntry(T, 1#":", NT.159, NT.160, ":any")
 , {159} tableEntry(NT.176, 1#"Type", Reduce.64, NT.160, "any")
 , {160} tableEntry(NT.!T.161, 1#"Id", Reduce.65, Fail, "any")
 ...
 , {171} tableEntry(T, 1#"else", T.172, Success*, "else if any then any")
 , {172} tableEntry(T, 1#"if", NT.173, Success*, "if any then any")
 , {173} tableEntry(NT.40, 1#"E", T.174, Success*, "any then any")
 , {174} tableEntry(T, 1#"then", NT.175, Success*, "then any")
 , {175} tableEntry(NT.40, 1#"E", Reduce*(69, T.171), Success*, "any")
 , {176} tableEntry(NT.!T.161, 1#"Id", T.177, Fail, "any.any")
 , {177} tableEntry(T, 1#".", NT.178, NT.179, ".any")
 , {178} tableEntry(NT.176, 1#"Type", Reduce.70, NT.179, "any")
 , {179} tableEntry(NT.!T.161, 1#"Id", Reduce.71, Fail, "any")
 , {180} tableEntry(T', 1#"let", MatchAny.181, T'.185, "let any = any")
 , {181} tableEntry(MatchAny, 1#"?", T.182, T'.185, "any = any")
 , {182} tableEntry(T, 1#"=", NT.183, T'.185, "= any")
 , {183} tableEntry(NT.40, 1#"E", NT.184, T'.185, "any")
 , {184} tableEntry(NT.T.170, 1#"comma?", Reduce.72, T'.185, "")
 , {185} tableEntry(T', 1#"assert", NT.186, T'.190, "assert any report any")
 , {186} tableEntry(NT.40, 1#"E", T.187, T'.190, "any report any")
 , {187} tableEntry(T, 1#"report", NT.188, T'.190, "report any")
 , {188} tableEntry(NT.40, 1#"E", NT.189, T'.190, "any")
 ...
 , {194} tableEntry(T', 1#"for", NT.195, Fail, "for any = any do any")
 , {195} tableEntry(NT.206, 1#"ForDeclare", T'.196, Fail, "any = any do any")
 , {196} tableEntry(T', 1#"do", NT.197, T.201, "do any")
 , {197} tableEntry(NT.40, 1#"E", NT.198, T.199, "any")
 , {198} tableEntry(NT.T.170, 1#"comma?", Reduce.75, T.199, "")
 , {199} tableEntry(T, 1#"for", NT.200, Fail, "for any = any while any do any")
 , {200} tableEntry(NT.206, 1#"ForDeclare", T.201, Fail, "any = any while any do any")
 , {201} tableEntry(T, 1#"while", NT.202, Fail, "while any do any")
 , {202} tableEntry(NT.40, 1#"E", T.203, Fail, "any do any")
 , {203} tableEntry(T, 1#"do", NT.204, Fail, "do any")
 , {204} tableEntry(NT.40, 1#"E", NT.205, Fail, "any")
 , {205} tableEntry(NT.T.170, 1#"comma?", Reduce.76, Fail, "")
 , {206} tableEntry(NT.!T.217, 1#"AccumList", T'.207, Fail, "any = any, any ∈ any")
 , {207} tableEntry(T', 1#",", MatchAny.208, T.212, ", any ∈ any")
 , {208} tableEntry(MatchAny, 1#"?", T'.209, NT.211, "any ∈ any")
 , {209} tableEntry(T', 1#"∈", NT.210, T.214, "∈ any")
 , {210} tableEntry(NT.40, 1#"E", Reduce.77, NT.211, "any")
 , {211} tableEntry(NT.!T.217, 1#"AccumList", T.212, Fail, "any = any, any /in any")
 , {212} tableEntry(T, 1#",", MatchAny.213, NT.216, ", any /in any")
 , {213} tableEntry(MatchAny, 1#"?", T.214, NT.216, "any /in any")
 , {214} tableEntry(T, 1#"/in", NT.215, NT.216, "/in any")
 , {215} tableEntry(NT.40, 1#"E", Reduce.78, NT.216, "any")
 , {216} tableEntry(NT.!T.217, 1#"AccumList", Reduce.79, Fail, "any = any")
 , {217} tableEntry(!T, 1#"while", Fail, MatchAny.218, "while any = any")
 , {218} tableEntry(MatchAny, 1#"?", T.219, Fail, "any = any")
 , {219} tableEntry(T, 1#"=", NT.220, Fail, "= any")
 , {220} tableEntry(NT.40, 1#"E", NT.221, Fail, "any")
 , {221} tableEntry(NT.T.222, 1#"AccumList'", Reduce.80, Fail, "")
 , {222} tableEntry(T, 1#",", MatchAny.223, Success*, ", any = any")
 , {223} tableEntry(MatchAny, 1#"?", T.224, Success*, "any = any")
 , {224} tableEntry(T, 1#"=", NT.225, Success*, "= any")
 , {225} tableEntry(NT.40, 1#"E", Reduce*(81, T.222), Success*, "any")
 , {226} tableEntry(NT.T'.180, 1#"Declare", Reduce*(82, NT.226), Success*, "{}")
 , {227} tableEntry(NT.MatchAny.231, 1#"FP", NT.228, Fail, "any")
 , {228} tableEntry(NT.T.229, 1#"FPL'", Reduce.83, Fail, "")
 , {229} tableEntry(T, 1#",", NT.230, Success*, ", any")
 , {230} tableEntry(NT.MatchAny.231, 1#"FP", Reduce*(84, T.229), Success*, "any")
 , {231} tableEntry(MatchAny, 1#"?", T.232, NT.234, "any:any")
 , {232} tableEntry(T, 1#":", NT.233, NT.234, ":any")
 , {233} tableEntry(NT.176, 1#"Type", Reduce.85, NT.234, "any")
 , {234} tableEntry(NT.176, 1#"Type", Reduce.86, Fail, "any")
