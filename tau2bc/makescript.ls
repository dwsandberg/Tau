...
if ext.outfn sub 1 ∈ "lib" ∧ (toseq.cmddef) sub 3 ∉ "shell" then
 ...
 do if ext.fn ∈ "libinfo" then dependlibs + "built /nsp / /nsp:(name.fn).bc" else dependlibs,
 lineinfo(
  outfn
  , infn
  , acc >> 3
 ...
 do
  ...
  if isempty.b then txt
   ...
   + if l sub 1 ∈ "define" then ""
    ...
    "/br:(
    shellparameters(toseq.def.b sub 1 << 3, fullnames.input.b sub 1, fullnames.output.b sub 1, line.b sub 1)
    )"
   else
