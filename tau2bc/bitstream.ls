...
Function tobitstream(s:seq.bits) bitstream bitstream(n.s * 64, 0x0, s)

Function bits(a:bitstream) seq.bits fullwords.a + endpart.a

Function toseqseqbyte(bs:bitstream) seq.seq.byte
toseqseqbyte(bits.bs, (length.bs + 7) / 8)
...
if length.x = 0 then "empty"
 ...
 part
 + for acc = "", @e ∈ reverse.fullwords.x do acc + %.@e,
 acc
...
if partbitsa = 0 ∧ length.a > 0 then
 ...
 if n.fullwords.b = 0 then
  ...
  let allwords =
   shiftleft(2, firstword.b >> steal, fullwords.b + endpart.b, partbitsa, firstpart),
...
Function +(a:bitstream, b:seq.byte) bitstream
for acc = a, @e ∈ b do acc + @e,
acc
