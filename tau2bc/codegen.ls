...
do
 ...
 if kind = kint then next(last, s, code, lmap, noblocks, regno, push(argstk, toint.C64.value.s), blocks)
  ...
  for no = 1, e ∈ reverse.toseq.blocks
  while kind.sym.e ∉ [kstart, kloop]
  do no + 1
  ...
  let startblk = push(blocks, Lcode(code, s, noblocks, regno, push(argstk, typ.resultllvmtype), blocks))
  ...
  if action = "CALL" sub 1 then
   ...
   let newcode = code + usetemplate(m, regno, empty:seq.int) + CALLFINISH(regno + 1, [-1] + args),
...
do
 ...
 if arg.m = 1 then
  let cond = Lcode(newcode, Br2(1, 2), noblks, newreg + 1, push(empty:stack.int,-newreg), blks),
  next(newreg + 1, noblks + 1, push(blks, cond))
 else
  let exitblock = Lcode(newcode, Exit, noblks, newreg, push(empty:stack.int,-newreg), blks),
...
for i = 1, code = emptyinternalbc, phi = empty:seq.int, tailphi = empty:seq.int, l ∈ blks << 1
do
 let kind = kind.sym.l,
 if kind = kexit then
  ...
  let new = SWITCH(r(regno.blks sub (i + 1) + 1), i64, slot.top.stk11, defaultBlk, switchArgs),
...
let new = SWITCH(r(regno.startBlk + 1), i64, slot.startargs sub 1, defaultBlk, switchArgs)
