...
let have =
 for acc = empty:set.encoding.word3, @e ∈ baselibwords do acc + encode.word3.@e,
 acc
let used =
 for acc = empty:set.encoding.word3, @e ∈ encodingdata:word3 do acc + encode.@e,
 acc,
toseq(used \ have)
...
let flds =
 for acc = empty:seq.slot, @e ∈ fldsin do acc + asi64.slot.@e,
 acc
