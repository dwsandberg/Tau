...
for acc = "", @e ∈ arithseq((finish - start + 2) / 2, 2, start)
do if isprime.@e then acc + toword.@e else acc,
acc

function countprimes(start:int, finish:int) int
for acc = 0, @e ∈ arithseq((finish - start + 2) / 2, 2, start)
do if isprime.@e then acc + 1 else acc,
acc
...
let r =
 ...
 do
  for acc2 = acc, j ∈ arithseq(16, 4, 0)
  do acc2 + (toint(tobits.i >> j ∧ 0xF) + 4 * j),
  acc2,
 sort.acc
