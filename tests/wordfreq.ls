...
/br the word, occurs 460 times.' then
 "PASS wordfref"
...
≠"subpass2"_1 then optB(ty, s)else ty expandresult(nextvar.a, t2, flags.a)else a let newoptions1=if between(length 
.code.t, 1, 21)∧ Callself ∉ flags.t ∧ Hasfor ∉ flags.t ∧"NOINLINE"_1 ∉ options then if isverysimple(nopara.s, code.t)
...
next(flags, result >> 1, nextvar, map)else if length.result > 2 ∧ isconst.last.result ∧(sym=symbol(moduleref("stdlib 
seq", typeint), "∈", [typeint, seqof.typeint], typeboolean)∨ sym=symbol(moduleref("stdlib seq", typeword), "∈"
...
(last.result, Local.nextvar), nextvar, map)else if wordname.sym ∈"forexp"∧ isBuiltin.sym then let noop=forexpisnoop 
(sym, result)if not.isempty.noop then next(flags, noop, nextvar, map)else next(flags ∨ Hasfor, result+sym, nextvar 
, map)else if wordname.sym ∈"indexseq45"∧ isInternal.sym then next(flags ∨ Hasfor, result+sym, nextvar, map)else if 
sym=self then next(flags ∨ Callself, result+sym, nextvar, map)else let nopara=nopara.sym let dd=getCode(p, sym)let 
options=getoption.dd let ct=if first."COMPILETIME"∈ options then prepareargs(subseq(result, len-nopara+1, len 
), sym)else empty:seq.symbol if{COMPILE TIME}not.isempty.ct then if sym=symbol(moduleref."stdlib words", "decodeword 
...
.newcode next(flags, result >> nopara+newconst, nextvar, map)else if first."VERYSIMPLE"∈ options then next(flags 
, result+removeoptions.dd << nopara.sym, nextvar, map)else if"INLINE"_1 ∉ options then let newflags=if"STATE"_1 ∈ 
options ∨{wordname.sym ∈"setfld"∨}isGlobal.sym then State ∨ flags else flags next(newflags, result+sym, nextvar, 
map)else let code=removeoptions.dd if isempty.code then next(flags, result+sym, nextvar, map)else if length.code 
=1 ∧ code=[Local.1]∧ nopara=1 then{function just returns result}next(flags, result, nextvar, map)else let t=backparse2 
(result, len, nopara, empty:seq.int)+[len+1]{assert length.t=nopara+1 report"INLINE PARA PROBLEM"}let new=expandinline 
...
(result, length.result, 2, empty:seq.int)let index=subseq(result, t_2, length.code)let theseq=subseq(result, 
t_1, t_2-1)let theseq2=if isconstorlocal.theseq then first.theseq else Local(nextvar+1)let newcode=subseq(result 
, 1, t_1-1)+if isconstorlocal.theseq then empty:seq.symbol else theseq+Define(nextvar+1)/if+[theseq2, GetSeqType 
, Define.nextvar]+index+[Lit.1, PlusOp, Define(nextvar+2)]+indexseqcode(Local.nextvar, theseq2, Local(nextvar 
...
=resulttype.forsym ∧ code_(-2)=Littrue ∧ isseq.resulttype.last.code ∧ wordname.code_(-3)="+"_1 ∧ name.module.
code_(-3)∈"seq"∧ isSequence.code_(-4)∧ nopara.code_(-4)=1 ∧ last.code=code_(-8)∧ last.code=code_(-6)∧ code_
(-7)=code_(-5)then let t2=backparse2(code, length.code-8, 2, empty:seq.int)let initacc=subseq(code, t2_1, t2_
2-1)if length.initacc=1 ∧ isrecordconstant.initacc_1 ∧ constantcode.initacc_1=[Lit.0, Lit.0]then subseq(code 
, 1, t2_1-1)+subseq(code, t2_2, length.code-8)else empty:seq.symbol else empty:seq.symbol '
...
=typebit let callidx=symbol(internalmod, "callidx", [seqof.elementtype, typeint], elementtype)[Start.elementtype 
, seqtype, Lit.1, GtOp, Br2(1, 2)]+[theseq, masteridx, callidx, Exit]+if boundscheck then[masteridx, theseq, GetSeqLength 
, GtOp, Br2(1, 2), symbol(modTausupport, "outofbounds", seqof.typeword), abortsymbol.elementtype, Exit]else empty 
...
pele let options=getoption.fullcode let code=removeoptions.fullcode if isempty.code ∨"VERYSIMPLE"_1 ∈ options then 
next(big, small, pele ∪ core)else if"COMPILETIME"_1 ∈ options then let code4=firstopt(core, s, fullcode, options, true 
)next(big, small, symdef(s, code4)∪ core)else if length.code < 30 then let t=firstopt(core, s, fullcode, options, true 
)if"INLINE"_1 ∈ getoption.t then next(big, small, symdef(s, t)∪ core)else next(big, symdef(s, t)∪ small, core)else 
...
(s, b-1, no-1, [b]+result)else let nopara=nopara.s_i let first=if nopara=0 then i else let args=backparse2(s, i-1, nopara 
, empty:seq.int)assert length.args=nopara report"back parse 3"+print.[s_i]+toword.nopara+"//"+for acc="", @e 
∈ args do acc+toword.@e /for(acc)args_1 let b=if first > 1 ∧ isdefine.s_(first-1)then let c=backparse2(s, first-2, 1 
, empty:seq.int)c_1 else first backparse2(s, b-1, no-1, [b]+result)'
] 