...
do
 let a = %.newanal(tosymbols(in.test, self.test), self.test),
  ...
  + 
   if a ≠ out.test then
    "Fail^(self.test) Input:^(tosymbols(in.test, self.test)) Expected:^(out.test)"
    + "got:^(a)"
   else "Pass^(self.test)",
acc
...
do
 ...
 next(
  [symbol4(moduleref("* seq", typeint), 1_"empty", seqof.typeint, empty:seq.mytype, seqof.typeint)]
  + acc
  , arg
  , typ
  , 0
 ...
 next([Define.value.1_acc] + acc << 1, arg, typ, 0)
 else if w ∈ "Continue" then
 next([continue.value.1_acc] + acc << 1, arg, typ, 0)
 else
  let j = findindex(X, w),
