...
let r =
 "GRAPH::(toseq.arcs.g)Succ:(toseq.successors(g, n2))Pred:(toseq.predecessors(g, n4))sinks:(sinks(g, asset.[n5]))sources:(sources.g)",
r
 = "GRAPH:"
...
for
 s = constantseq(100, 0)
 , i ∈ for acc = empty:seq.int, e ∈ randomseq(3456, 100001) do acc + (abs.e mod 100 + 1),
 acc
...
let s =
 ...
 + encodeUTF8.char.50
 + encodeUTF8.char.336
for acc = empty:seq.int, @e ∈ toseqbyte.s do acc + toint.@e
let z = tomyseq.acc
